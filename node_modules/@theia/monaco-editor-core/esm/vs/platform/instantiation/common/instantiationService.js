...
const instantiation_js_1 = require("./instantiation.js");
const serviceCollection_js_1 = require("./serviceCollection.js");
...
class InstantiationService {
    constructor(_services = new serviceCollection_js_1.ServiceCollection(), _strict = false, _parent, _enableTracing = _enableAllTracing) {
        ...
        this._services.set(instantiation_js_1.IInstantiationService, this);
        this._globalGraph = _enableTracing ? _parent?._globalGraph ?? new graph_js_1.Graph(e => e) : undefined;
    }
    dispose() {
        if (!this._isDisposed) {
            this._isDisposed = true;
            // dispose all child services
            (0, lifecycle_js_1.dispose)(this._children);
            this._children.clear();
            // dispose all services created by this service
            for (const candidate of this._servicesToMaybeDispose) {
                if ((0, lifecycle_js_1.isDisposable)(candidate)) {
                    candidate.dispose();
                }
            }
    ...
    invokeFunction(fn, ...args) {
        ...
        try {
            const accessor = {
                get: (id) => {
                    if (_done) {
                        throw (0, errors_js_1.illegalState)('service accessor is only valid during the invocation of its target method');
                    }
    ...
    createInstance(ctorOrDescriptor, ...rest) {
        ...
        let _trace;
        let result;
        if (ctorOrDescriptor instanceof descriptors_js_1.SyncDescriptor) {
    ...
    _createInstance(ctor, args = [], _trace) {
        ...
        const serviceDependencies = instantiation_js_1._util.getServiceDependencies(ctor).sort((a, b) => a.index - b.index);
        const serviceArgs = [];
    ...
    _setCreatedServiceInstance(id, instance) {
        if (this._services.get(id) instanceof descriptors_js_1.SyncDescriptor) {
    ...
    _getOrCreateServiceInstance(id, _trace) {
        ...
        if (thing instanceof descriptors_js_1.SyncDescriptor) {
    ...
    _createAndCacheServiceInstance(id, desc, _trace) {
        const graph = new graph_js_1.Graph(data => data.id.toString());
        let cycleCount = 0;
        ...
        while (stack.length) {
            ...
            for (const dependency of instantiation_js_1._util.getServiceDependencies(item.desc.ctor)) {
                ...
                if (instanceOrDesc instanceof descriptors_js_1.SyncDescriptor) {
    ...
    _createServiceInstance(id, ctor, args = [], supportsDelayedInstantiation, _trace, disposeBucket) {
        if (!supportsDelayedInstantiation) {
            ...
            const idle = new async_js_1.GlobalIdleValue(() => {
                ...
                get(target, key) {
                    if (!idle.isInitialized) {
                        ...
                        if (typeof key === 'string' && (key.startsWith('onDid') || key.startsWith('onWill'))) {
                            ...
                            if (!list) {
                                list = new linkedList_js_1.LinkedList();
