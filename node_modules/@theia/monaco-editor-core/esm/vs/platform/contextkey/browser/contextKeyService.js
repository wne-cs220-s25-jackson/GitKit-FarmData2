...
const contextkey_js_1 = require("../common/contextkey.js");
...
class ConfigAwareContextValuesContainer extends Context {
    ...
    constructor(id, _configurationService, emitter) {
        ...
        this._values = ternarySearchTree_js_1.TernarySearchTree.forConfigKeys();
            ...
            if (event.source === 7 /* ConfigurationTarget.DEFAULT */) {
                ...
                for (const configKey of event.affectedKeys) {
                    ...
                    if (cachedItems !== undefined) {
                        changedKeys.push(...iterator_js_1.Iterable.map(cachedItems, ([key]) => key));
...
class AbstractContextKeyService extends lifecycle_js_1.Disposable {
    constructor(myContextId) {
        super();
        this._onDidChangeContext = this._register(new event_js_1.PauseableEmitter({ merge: input => new CompositeContextKeyChangeEvent(input) }));
    ...
    createOverlay(overlay = iterator_js_1.Iterable.empty()) {
        if (this._isDisposed) {
            throw new Error(`AbstractContextKeyService has been disposed`);
        }
...
class ScopedContextKeyService extends AbstractContextKeyService {
    constructor(parent, domNode) {
        ...
        this._parentChangeListener = this._register(new lifecycle_js_1.MutableDisposable());
        this._parent = parent;
    ...
    updateParent(parentContextKeyService) {
        ...
        const allValuesDiff = {
            ...(0, objects_js_1.distinct)(oldAllValues, newAllValues),
            ...(0, objects_js_1.distinct)(newAllValues, oldAllValues)
        };
...
function stringifyURIs(contextValue) {
    return (0, objects_js_1.cloneAndChange)(contextValue, (obj) => {
        if (typeof obj === 'object' && obj.$mid === 1 /* MarshalledId.Uri */) {
            return uri_js_1.URI.revive(obj).toString();
        }
        if (obj instanceof uri_js_1.URI) {
            return obj.toString();
        }
        return undefined;
    });
}
commands_js_1.CommandsRegistry.registerCommand('_setContext', setContext);
commands_js_1.CommandsRegistry.registerCommand({
    id: 'getContextKeyInfo',
    handler() {
        return [...contextkey_js_1.RawContextKey.all()].sort((a, b) => a.key.localeCompare(b.key));
    },
    metadata: {
        description: (0, nls_js_1.localize)('getContextKeyInfo', "A command that returns information about context keys"),
        args: []
    }
});
commands_js_1.CommandsRegistry.registerCommand('_generateContextKeyInfo', function () {
    const result = [];
    const seen = new Set();
    for (const info of contextkey_js_1.RawContextKey.all()) {
