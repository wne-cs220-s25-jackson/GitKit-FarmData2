...
const toolbar_js_1 = require("../../../base/browser/ui/toolbar/toolbar.js");
...
const menuEntryActionViewItem_js_1 = require("./menuEntryActionViewItem.js");
const actions_js_2 = require("../common/actions.js");
const menuService_js_1 = require("../common/menuService.js");
...
const telemetry_js_1 = require("../../telemetry/common/telemetry.js");
const actionViewItemService_js_1 = require("./actionViewItemService.js");
 ...
 */
let WorkbenchToolBar = class WorkbenchToolBar extends toolbar_js_1.ToolBar {
    constructor(container, _options, _menuService, _contextKeyService, _contextMenuService, _keybindingService, _commandService, telemetryService) {
        ...
        this._sessionDisposables = this._store.add(new lifecycle_js_1.DisposableStore());
        // telemetry logic
    ...
    setActions(_primary, _secondary = [], menuIds) {
        ...
        if (this._options?.hiddenItemStrategy !== -1 /* HiddenItemStrategy.NoHide */) {
            for (let i = 0; i < primary.length; i++) {
                ...
                if (!(action instanceof actions_js_2.MenuItemAction) && !(action instanceof actions_js_2.SubmenuItemAction)) {
        ...
        if (this._options?.overflowBehavior !== undefined) {
            const exemptedIds = (0, collections_js_1.intersection)(new Set(this._options.overflowBehavior.exempted), iterator_js_1.Iterable.map(primary, a => a?.id));
        ...
        (0, arrays_js_1.coalesceInPlace)(primary);
        (0, arrays_js_1.coalesceInPlace)(extraSecondary);
        super.setActions(primary, actions_js_1.Separator.join(extraSecondary, secondary));
        // add context menu for toggle and configure keybinding actions
        if (toggleActions.length > 0 || primary.length > 0) {
            this._sessionDisposables.add((0, dom_js_1.addDisposableListener)(this.getElement(), 'contextmenu', e => {
                const event = new mouseEvent_js_1.StandardMouseEvent((0, dom_js_1.getWindow)(this.getElement()), e);
                ...
                if (action instanceof actions_js_2.MenuItemAction && action.menuKeybinding) {
                    primaryActions.push(action.menuKeybinding);
                }
                else if (!(action instanceof actions_js_2.SubmenuItemAction || action instanceof toolbar_js_1.ToggleMenuAction)) {
                    ...
                    primaryActions.push((0, menuService_js_1.createConfigureKeybindingAction)(this._commandService, this._keybindingService, action.id, undefined, supportsKeybindings));
                }
                // -- Hide Actions --
                if (toggleActions.length > 0) {
                    ...
                    if (toggleActionsCheckedCount === 1 && this._options?.hiddenItemStrategy === 0 /* HiddenItemStrategy.Ignore */) {
                        ...
                        for (let i = 0; i < toggleActions.length; i++) {
                            if (toggleActions[i].checked) {
                                toggleActions[i] = (0, actions_js_1.toAction)({
                                    id: action.id,
                                    label: action.label,
                                    checked: true,
                                    enabled: false,
                    ...
                    if (!noHide && (action instanceof actions_js_2.MenuItemAction || action instanceof actions_js_2.SubmenuItemAction)) {
                        if (!action.hideActions) {
                        ...
                        primaryActions.push((0, actions_js_1.toAction)({
                            id: 'label',
                            label: (0, nls_js_1.localize)('hide', "Hide"),
                            enabled: false,
                            run() { }
                        }));
                    }
                }
                const actions = actions_js_1.Separator.join(primaryActions, toggleActions);
                // add "Reset Menu" action
                if (this._options?.resetMenu && !menuIds) {
                    menuIds = [this._options.resetMenu];
                }
                if (someAreHidden && menuIds) {
                    actions.push(new actions_js_1.Separator());
                    actions.push((0, actions_js_1.toAction)({
                        id: 'resetThisMenu',
                        label: (0, nls_js_1.localize)('resetThisMenu', "Reset Menu"),
...
let MenuWorkbenchToolBar = class MenuWorkbenchToolBar extends WorkbenchToolBar {
    constructor(container, menuId, options, menuService, contextKeyService, contextMenuService, keybindingService, commandService, telemetryService, actionViewService, instaService) {
        super(container, {
            ...
            actionViewItemProvider: (action, opts) => {
                let provider = actionViewService.lookUp(menuId, action instanceof actions_js_2.SubmenuItemAction ? action.item.submenu.id : action.id);
                if (!provider) {
                ...
                return (0, menuEntryActionViewItem_js_1.createActionViewItem)(instaService, action, opts);
            }
        }, menuService, contextKeyService, contextMenuService, keybindingService, commandService, telemetryService);
        this._onDidChangeMenuItems = this._store.add(new event_js_1.Emitter());
        ...
        const updateToolbar = () => {
            const { primary, secondary } = (0, menuEntryActionViewItem_js_1.getActionBarActions)(menu.getActions(options?.menuOptions), options?.toolbarOptions?.primaryGroup, options?.toolbarOptions?.shouldInlineSubmenu, options?.toolbarOptions?.useSeparatorsInPrimaryActions);
     ...
     */
    setActions() {
        throw new errors_js_1.BugIndicatingError('This toolbar is populated from a menu.');
    }
};
...
exports.MenuWorkbenchToolBar = MenuWorkbenchToolBar = __decorate([
    ...
    __param(9, actionViewItemService_js_1.IActionViewItemService),
    __param(10, instantiation_js_1.IInstantiationService)
