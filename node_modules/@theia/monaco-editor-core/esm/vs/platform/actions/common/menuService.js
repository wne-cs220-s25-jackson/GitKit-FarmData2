...
const actions_js_2 = require("../../../base/common/actions.js");
    ...
    updateHidden(menu, commandId, hidden) {
        ...
        if (!hidden) {
            ...
            if (entries) {
                ...
                if (idx >= 0) {
                    (0, arrays_js_1.removeFastWithoutKeepingOrder)(entries, idx);
                }
                if (entries.length === 0) {
...
class MenuInfoSnapshot {
    ...
    refresh() {
        ...
        const menuItems = this._sort(actions_js_1.MenuRegistry.getMenuItems(this._id));
        let group;
    ...
    _collectContextKeysAndSubmenuIds(item) {
        ...
        if ((0, actions_js_1.isIMenuItem)(item)) {
            ...
            actions_js_1.MenuRegistry.getMenuItems(item.submenu).forEach(this._collectContextKeysAndSubmenuIds, this);
...
let MenuInfo = MenuInfo_1 = class MenuInfo extends MenuInfoSnapshot {
        ...
        for (const group of this._menuGroups) {
            ...
            for (const item of items) {
                if (this._contextKeyService.contextMatchesRules(item.when)) {
                    const isMenuItem = (0, actions_js_1.isIMenuItem)(item);
                    if (isMenuItem) {
                    ...
                    if (isMenuItem) {
                        ...
                        (activeActions ??= []).push(new actions_js_1.MenuItemAction(item.command, item.alt, options, menuHide, menuKeybinding, this._contextKeyService, this._commandService));
                    }
                    else {
                        // SubmenuItemAction
                        ...
                        const submenuActions = actions_js_2.Separator.join(...groups.map(g => g[1]));
                        if (submenuActions.length > 0) {
                            (activeActions ??= []).push(new actions_js_1.SubmenuItemAction(item, menuHide, submenuActions));
                        }
                    }
                }
            }
    ...
    static _compareMenuItems(a, b) {
        ...
        // sort on titles
        return MenuInfo_1._compareTitles((0, actions_js_1.isIMenuItem)(a) ? a.command.title : a.title, (0, actions_js_1.isIMenuItem)(b) ? b.command.title : b.title);
    }
...
let MenuImpl = class MenuImpl {
    constructor(id, hiddenStates, options, commandService, keybindingService, contextKeyService) {
        ...
        const rebuildMenuSoon = new async_js_1.RunOnceScheduler(() => {
            this._menuInfo.refresh();
            this._onDidChange.fire({ menu: this, isStructuralChange: true, isEnablementChange: true, isToggleChange: true });
        }, options.eventDebounceDelay);
        this._disposables.add(rebuildMenuSoon);
        this._disposables.add(actions_js_1.MenuRegistry.onDidChangeMenu(e => {
        ...
        const lazyListener = this._disposables.add(new lifecycle_js_1.DisposableStore());
        const merge = (events) => {
        ...
        this._onDidChange = new event_js_1.DebounceEmitter({
...
function createMenuHide(menu, command, states) {
    const id = (0, actions_js_1.isISubmenuItem)(command) ? command.submenu.id : command.id;
    const title = typeof command.title === 'string' ? command.title : command.title.value;
    const hide = (0, actions_js_2.toAction)({
        id: `hide/${menu.id}/${id}`,
        label: (0, nls_js_1.localize)('hide.label', 'Hide \'{0}\'', title),
        run() { states.updateHidden(menu, id, true); }
    });
    const toggle = (0, actions_js_2.toAction)({
...
function createConfigureKeybindingAction(commandService, keybindingService, commandId, when = undefined, enabled = true) {
    return (0, actions_js_2.toAction)({
        id: `configureKeybinding/${commandId}`,
        label: (0, nls_js_1.localize)('configure keybinding', "Configure Keybinding"),
        enabled,
        run() {
