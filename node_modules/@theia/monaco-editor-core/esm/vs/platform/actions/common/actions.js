...
exports.IMenuService = (0, instantiation_js_1.createDecorator)('menuService');
...
exports.MenuRegistry = new class {
    constructor() {
        ...
        this._onDidChangeMenu = new event_js_1.MicrotaskEmitter({
...
class SubmenuItemAction extends actions_js_1.SubmenuAction {
...
let MenuItemAction = MenuItemAction_1 = class MenuItemAction {
    ...
    constructor(item, alt, options, hideActions, menuKeybinding, contextKeyService, _commandService) {
        ...
        if (item.toggled) {
            ...
            if (this.checked && themables_js_1.ThemeIcon.isThemeIcon(toggled.icon)) {
                icon = toggled.icon;
            }
        ...
        if (!icon) {
            icon = themables_js_1.ThemeIcon.isThemeIcon(item.icon) ? item.icon : undefined;
        }
        this.item = item;
        ...
        this.class = icon && themables_js_1.ThemeIcon.asClassName(icon);
    }
    run(...args) {
        let runArgs = [];
...
function registerAction2(ctor) {
    ...
    if (commands_js_1.CommandsRegistry.getCommand(command.id)) {
        throw new Error(`Cannot register two commands with the same id: ${command.id}`);
    }
    // command
    disposables.push(commands_js_1.CommandsRegistry.registerCommand({
        id: command.id,
    ...
    if (Array.isArray(keybinding)) {
        for (const item of keybinding) {
            disposables.push(keybindingsRegistry_js_1.KeybindingsRegistry.registerKeybindingRule({
                ...item,
                id: command.id,
                when: command.precondition ? contextkey_js_1.ContextKeyExpr.and(command.precondition, item.when) : item.when
            }));
        }
    }
    else if (keybinding) {
        disposables.push(keybindingsRegistry_js_1.KeybindingsRegistry.registerKeybindingRule({
            ...keybinding,
            id: command.id,
            when: command.precondition ? contextkey_js_1.ContextKeyExpr.and(command.precondition, keybinding.when) : keybinding.when
        }));
    }
    return {
        dispose() {
