...
const json = require("../../../base/common/json.js");
const lifecycle_js_1 = require("../../../base/common/lifecycle.js");
const map_js_1 = require("../../../base/common/map.js");
const objects = require("../../../base/common/objects.js");
const types = require("../../../base/common/types.js");
const uri_js_1 = require("../../../base/common/uri.js");
const configuration_js_1 = require("./configuration.js");
const configurationRegistry_js_1 = require("./configurationRegistry.js");
...
class ConfigurationModel {
    ...
    getValue(section) {
        return section ? (0, configuration_js_1.getConfigurationValue)(this.contents, section) : this.contents;
    }
    ...
    getOverrideValue(section, overrideIdentifier) {
        ...
        return overrideContents
            ? section ? (0, configuration_js_1.getConfigurationValue)(overrideContents, section) : overrideContents
            : undefined;
    }
    ...
    removeValue(key) {
        ...
        (0, configuration_js_1.removeFromValueTree)(this.contents, key);
        if (configurationRegistry_js_1.OVERRIDE_PROPERTY_REGEX.test(key)) {
            this.overrides.splice(this.overrides.findIndex(o => arrays.equals(o.identifiers, (0, configurationRegistry_js_1.overrideIdentifiersFromKey)(key))), 1);
        }
    }
    updateValue(key, value, add) {
        (0, configuration_js_1.addToValueTree)(this.contents, key, value, e => this.logService.error(e));
        add = add || this.keys.indexOf(key) === -1;
        if (add) {
            this.keys.push(key);
        }
        if (configurationRegistry_js_1.OVERRIDE_PROPERTY_REGEX.test(key)) {
            const identifiers = (0, configurationRegistry_js_1.overrideIdentifiersFromKey)(key);
            const override = {
                identifiers,
                keys: Object.keys(this.contents[key]),
                contents: (0, configuration_js_1.toValuesTree)(this.contents[key], message => this.logService.error(message)),
            };
...
class ConfigurationModelParser {
    ...
    doParseRaw(raw, options) {
        const configurationProperties = platform_js_1.Registry.as(configurationRegistry_js_1.Extensions.Configuration).getConfigurationProperties();
        const filtered = this.filter(raw, configurationProperties, true, options);
        raw = filtered.raw;
        const contents = (0, configuration_js_1.toValuesTree)(raw, message => this.logService.error(`Conflict in settings file ${this._name}: ${message}`));
    ...
    filter(properties, configurationProperties, filterOverriddenProperties, options) {
        ...
        for (const key in properties) {
            if (configurationRegistry_js_1.OVERRIDE_PROPERTY_REGEX.test(key) && filterOverriddenProperties) {
    ...
    toOverrides(raw, conflictReporter) {
        ...
        for (const key of Object.keys(raw)) {
            if (configurationRegistry_js_1.OVERRIDE_PROPERTY_REGEX.test(key)) {
                ...
                overrides.push({
                    identifiers: (0, configurationRegistry_js_1.overrideIdentifiersFromKey)(key),
                    keys: Object.keys(overrideRaw),
                    contents: (0, configuration_js_1.toValuesTree)(overrideRaw, conflictReporter)
                });
            }
        }
        return overrides;
...
class UserSettings extends lifecycle_js_1.Disposable {
    constructor(userSettingsResource, parseOptions, extUri, fileService, logService) {
        ...
        this.logService = logService;
        this._onDidChange = this._register(new event_js_1.Emitter());
...
class Configuration {
    constructor(_defaultConfiguration, _policyConfiguration, _applicationConfiguration, _localUserConfiguration, _remoteUserConfiguration, _workspaceConfiguration, _folderConfigurations, _memoryConfiguration, _memoryConfigurationByResource, logService) {
        ...
        this._foldersConsolidatedConfigurations = new map_js_1.ResourceMap();
    ...
    compareAndUpdateDefaultConfiguration(defaults, keys) {
        ...
        for (const key of keys) {
            for (const overrideIdentifier of (0, configurationRegistry_js_1.overrideIdentifiersFromKey)(key)) {
    ...
    static parse(data, logService) {
        ...
        const folders = data.folders.reduce((result, value) => {
            result.set(uri_js_1.URI.revive(value[0]), this.parseConfigurationModel(value[1], logService));
            return result;
        }, new map_js_1.ResourceMap());
        return new Configuration(defaultConfiguration, policyConfiguration, applicationConfiguration, userConfiguration, ConfigurationModel.createEmptyModel(logService), workspaceConfiguration, folders, ConfigurationModel.createEmptyModel(logService), new map_js_1.ResourceMap(), logService);
    }
...
function mergeChanges(...changes) {
    ...
    for (const change of changes) {
        ...
        change.overrides.forEach(([identifier, keys]) => {
            const result = (0, map_js_1.getOrSet)(overridesMap, identifier, new Set());
    ...
    if (to && from) {
        for (const key of from.keys) {
            if (to.keys.indexOf(key) !== -1) {
                const value1 = (0, configuration_js_1.getConfigurationValue)(from.contents, key);
                const value2 = (0, configuration_js_1.getConfigurationValue)(to.contents, key);
