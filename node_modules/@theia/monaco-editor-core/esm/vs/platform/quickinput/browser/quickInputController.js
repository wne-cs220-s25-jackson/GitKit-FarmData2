...
const button_js_1 = require("../../../base/browser/ui/button/button.js");
const countBadge_js_1 = require("../../../base/browser/ui/countBadge/countBadge.js");
const progressbar_js_1 = require("../../../base/browser/ui/progressbar/progressbar.js");
...
const quickInputBox_js_1 = require("./quickInputBox.js");
const quickInput_js_2 = require("./quickInput.js");
const layoutService_js_1 = require("../../layout/browser/layoutService.js");
const window_js_1 = require("../../../base/browser/window.js");
const instantiation_js_1 = require("../../instantiation/common/instantiation.js");
const quickInputTree_js_1 = require("./quickInputTree.js");
const contextkey_js_1 = require("../../contextkey/common/contextkey.js");
require("./quickInputActions.js");
const $ = dom.$;
let QuickInputController = class QuickInputController extends lifecycle_js_1.Disposable {
    ...
    constructor(options, layoutService, instantiationService, contextKeyService) {
        ...
        this.onShowEmitter = this._register(new event_js_1.Emitter());
        this.onShow = this.onShowEmitter.event;
        this.onHideEmitter = this._register(new event_js_1.Emitter());
        this.onHide = this.onHideEmitter.event;
        this.inQuickInputContext = quickInput_js_2.InQuickInputContextKey.bindTo(this.contextKeyService);
        this.quickInputTypeContext = quickInput_js_2.QuickInputTypeContextKey.bindTo(this.contextKeyService);
        this.endOfQuickInputBoxContext = quickInput_js_2.EndOfQuickInputBoxContextKey.bindTo(this.contextKeyService);
        this.backButton = quickInput_js_2.backButton;
        this.idPrefix = options.idPrefix;
        this._container = options.container;
        this.styles = options.styles;
        this._register(event_js_1.Event.runAndSubscribe(dom.onDidRegisterWindow, ({ window, disposables }) => this.registerKeyModsListeners(window, disposables), { window: window_js_1.mainWindow, disposables: this._store }));
    ...
    getUI(showInActiveContainer) {
        ...
        const leftActionBar = this._register(new actionbar_js_1.ActionBar(titleBar, { hoverDelegate: this.options.hoverDelegate }));
        leftActionBar.domNode.classList.add('quick-input-left-action-bar');
        const title = dom.append(titleBar, $('.quick-input-title'));
        const rightActionBar = this._register(new actionbar_js_1.ActionBar(titleBar, { hoverDelegate: this.options.hoverDelegate }));
        rightActionBar.domNode.classList.add('quick-input-right-action-bar');
        const headerContainer = dom.append(container, $('.quick-input-header'));
        const checkAll = dom.append(headerContainer, $('input.quick-input-check-all'));
        checkAll.type = 'checkbox';
        checkAll.setAttribute('aria-label', (0, nls_js_1.localize)('quickInput.checkAll', "Toggle all checkboxes"));
        ...
        const inputBox = this._register(new quickInputBox_js_1.QuickInputBox(filterContainer, this.styles.inputBox, this.styles.toggle));
        inputBox.setAttribute('aria-describedby', `${this.idPrefix}message`);
        const visibleCountContainer = dom.append(filterContainer, $('.quick-input-visible-count'));
        visibleCountContainer.setAttribute('aria-live', 'polite');
        visibleCountContainer.setAttribute('aria-atomic', 'true');
        const visibleCount = this._register(new countBadge_js_1.CountBadge(visibleCountContainer, { countFormat: (0, nls_js_1.localize)({ key: 'quickInput.visibleCount', comment: ['This tells the user how many items are shown in a list of items to select from. The items can be anything. Currently not visible, but read by screen readers.'] }, "{0} Results") }, this.styles.countBadge));
        const countContainer = dom.append(filterContainer, $('.quick-input-count'));
        countContainer.setAttribute('aria-live', 'polite');
        const count = this._register(new countBadge_js_1.CountBadge(countContainer, { countFormat: (0, nls_js_1.localize)({ key: 'quickInput.countSelected', comment: ['This tells the user how many items are selected in a list of items to select from. The items can be anything.'] }, "{0} Selected") }, this.styles.countBadge));
        const inlineActionBar = this._register(new actionbar_js_1.ActionBar(headerContainer, { hoverDelegate: this.options.hoverDelegate }));
        inlineActionBar.domNode.classList.add('quick-input-inline-action-bar');
        const okContainer = dom.append(headerContainer, $('.quick-input-action'));
        const ok = this._register(new button_js_1.Button(okContainer, this.styles.button));
        ok.label = (0, nls_js_1.localize)('ok', "OK");
        this._register(ok.onDidClick(e => {
            this.onDidAcceptEmitter.fire();
        }));
        const customButtonContainer = dom.append(headerContainer, $('.quick-input-action'));
        const customButton = this._register(new button_js_1.Button(customButtonContainer, { ...this.styles.button, supportIcons: true }));
        customButton.label = (0, nls_js_1.localize)('custom', "Custom");
        this._register(customButton.onDidClick(e => {
            this.onDidCustomEmitter.fire();
        }));
        const message = dom.append(inputContainer, $(`#${this.idPrefix}message.quick-input-message`));
        const progressBar = this._register(new progressbar_js_1.ProgressBar(container, this.styles.progressBar));
        progressBar.getContainer().classList.add('quick-input-progress');
        const widget = dom.append(container, $('.quick-input-html-widget'));
        widget.tabIndex = -1;
        const description1 = dom.append(container, $('.quick-input-description'));
        const listId = this.idPrefix + 'list';
        const list = this._register(this.instantiationService.createInstance(quickInputTree_js_1.QuickInputTree, container, this.options.hoverDelegate, this.options.linkOpenerDelegate, listId));
        ...
        this._register(list.onLeave(() => {
            ...
            setTimeout(() => {
                ...
                inputBox.setFocus();
                if (this.controller instanceof quickInput_js_2.QuickPick && this.controller.canSelectMany) {
                    list.clearFocus();
                }
            }, 0);
        }));
        ...
        this._register(focusTracker.onDidBlur(() => {
            if (!this.getUI().ignoreFocusOut && !this.options.ignoreFocusOut()) {
                this.hide(quickInput_js_1.QuickInputHideReason.Blur);
            }
        ...
        this._register(dom.addStandardDisposableListener(container, dom.EventType.KEY_DOWN, (event) => {
            ...
            switch (event.keyCode) {
                ...
                case 9 /* KeyCode.Escape */:
                    dom.EventHelper.stop(event, true);
                    this.hide(quickInput_js_1.QuickInputHideReason.Gesture);
                    break;
                case 2 /* KeyCode.Tab */:
    ...
    pick(picks, options = {}, token = cancellation_js_1.CancellationToken.None) {
        return new Promise((doResolve, reject) => {
            ...
            const disposables = [
                ...
                input.onDidHide(() => {
                    (0, lifecycle_js_1.dispose)(disposables);
                    resolve(undefined);
                }),
            ];
    ...
    setValidationOnInput(input, validationResult) {
        if (validationResult && (0, types_js_1.isString)(validationResult)) {
            input.severity = severity_js_1.default.Error;
            input.validationMessage = validationResult;
        }
        else if (validationResult && !(0, types_js_1.isString)(validationResult)) {
            input.severity = validationResult.severity;
            input.validationMessage = validationResult.content;
        }
        else {
            input.severity = severity_js_1.default.Ignore;
            input.validationMessage = undefined;
        }
    }
    input(options = {}, token = cancellation_js_1.CancellationToken.None) {
        return new Promise((resolve) => {
            ...
            const onDidValueChange = event_js_1.Event.debounce(input.onDidChangeValue, (last, cur) => cur, 100);
            ...
            const disposables = [
                ...
                input.onDidAccept(() => {
                    ...
                    validation.then(result => {
                        if (!result || (!(0, types_js_1.isString)(result) && result.severity !== severity_js_1.default.Error)) {
                            resolve(value);
                            input.hide();
                        }
    ...
    createQuickPick(options = { useSeparators: false }) {
        ...
        return new quickInput_js_2.QuickPick(ui);
    }
    createInputBox() {
        const ui = this.getUI(true);
        return new quickInput_js_2.InputBox(ui);
    }
    createQuickWidget() {
        const ui = this.getUI(true);
        return new quickInput_js_2.QuickWidget(ui);
    }
    show(controller) {
        const ui = this.getUI(true);
        ...
        ui.inputBox.showDecoration(severity_js_1.default.Ignore);
        ...
        quickInput_js_2.backButton.tooltip = backKeybindingLabel ? (0, nls_js_1.localize)('quickInput.backWithKeybinding', "Back ({0})", backKeybindingLabel) : (0, nls_js_1.localize)('quickInput.back', "Back");
    ...
    toggle() {
        if (this.isVisible() && this.controller instanceof quickInput_js_2.QuickPick && this.controller.canSelectMany) {
            this.getUI().list.toggleCheckbox();
        }
    }
    navigate(next, quickNavigate) {
        if (this.isVisible() && this.getUI().list.displayed) {
            this.getUI().list.focus(next ? quickInput_js_1.QuickPickFocus.Next : quickInput_js_1.QuickPickFocus.Previous);
            if (quickNavigate && this.controller instanceof quickInput_js_2.QuickPick) {
...
exports.QuickInputController = QuickInputController = QuickInputController_1 = __decorate([
    __param(1, layoutService_js_1.ILayoutService),
