...
const actionViewItems_js_1 = require("./actionViewItems.js");
...
class ActionBar extends lifecycle_js_1.Disposable {
    constructor(container, options = {}) {
        super();
        this._actionRunnerDisposables = this._register(new lifecycle_js_1.DisposableStore());
        this.viewItemDisposables = this._register(new lifecycle_js_1.DisposableMap());
        // Trigger Key Tracking
        this.triggerKeyDown = false;
        this.focusable = true;
        this._onDidBlur = this._register(new event_js_1.Emitter());
        this.onDidBlur = this._onDidBlur.event;
        this._onDidCancel = this._register(new event_js_1.Emitter({ onWillAddFirstListener: () => this.cancelHasListener = true }));
        this.onDidCancel = this._onDidCancel.event;
        this.cancelHasListener = false;
        this._onDidRun = this._register(new event_js_1.Emitter());
        this.onDidRun = this._onDidRun.event;
        this._onWillRun = this._register(new event_js_1.Emitter());
        ...
        this._hoverDelegate = options.hoverDelegate ?? this._register((0, hoverDelegateFactory_js_1.createInstantHoverDelegate)());
        if (this.options.actionRunner) {
            this._actionRunner = this.options.actionRunner;
        }
        else {
            this._actionRunner = new actions_js_1.ActionRunner();
        ...
        this._register(DOM.addDisposableListener(this.domNode, DOM.EventType.KEY_DOWN, e => {
            ...
            else if (event.equals(2 /* KeyCode.Tab */) && focusedItem instanceof actionViewItems_js_1.BaseActionViewItem && focusedItem.trapsArrowNavigation) {
    ...
    setFocusable(focusable) {
        ...
        if (this.focusable) {
            const firstEnabled = this.viewItems.find(vi => vi instanceof actionViewItems_js_1.BaseActionViewItem && vi.isEnabled());
            if (firstEnabled instanceof actionViewItems_js_1.BaseActionViewItem) {
                firstEnabled.setFocusable(true);
            }
        }
        else {
            this.viewItems.forEach(vi => {
                if (vi instanceof actionViewItems_js_1.BaseActionViewItem) {
    ...
    push(arg, options = {}) {
        ...
        actions.forEach((action) => {
            ...
            if (!item) {
                item = new actionViewItems_js_1.ActionViewItem(this.context, action, viewItemOptions);
            }
            ...
            if (this.focusable && item instanceof actionViewItems_js_1.BaseActionViewItem && this.viewItems.length === 0) {
    ...
    pull(index) {
        if (index >= 0 && index < this.viewItems.length) {
            ...
            (0, lifecycle_js_1.dispose)(this.viewItems.splice(index, 1));
            this.refreshRole();
        }
    }
    clear() {
        ...
        this.viewItems = (0, lifecycle_js_1.dispose)(this.viewItems);
    ...
    focusNext(forceLoop, forceFocus) {
        ...
        } while (this.focusedItem !== startIndex && ((this.options.focusOnlyEnabledItems && !item.isEnabled()) || item.action.id === actions_js_1.Separator.ID));
    ...
    updateFocus(fromRight, preventScroll, forceFocus = false) {
        ...
        if (actionViewItem) {
            ...
            if (actionViewItem.action.id === actions_js_1.Separator.ID) {
                focusItem = false;
            }
            if (!focusItem) {
                this.actionsList.focus({ preventScroll });
    ...
    doTrigger(event) {
        ...
        if (actionViewItem instanceof actionViewItems_js_1.BaseActionViewItem) {
...
function prepareActions(actions) {
    ...
    for (let i = 0; i < actions.length; i++) {
        if (actions[i].id === actions_js_1.Separator.ID) {
            continue;
        }
    ...
    for (let h = actions.length - 1; h >= 0; h--) {
        const isSeparator = actions[h].id === actions_js_1.Separator.ID;
        if (isSeparator) {
            actions.splice(h, 1);
        }
        else {
    ...
    for (let k = actions.length - 1; k >= 0; k--) {
        const isSeparator = actions[k].id === actions_js_1.Separator.ID;
