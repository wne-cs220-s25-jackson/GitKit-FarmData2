...
class DataTree extends abstractTree_js_1.AbstractTree {
    ...
    setInput(input, viewState) {
        if (viewState && !this.identityProvider) {
            throw new tree_js_1.TreeError(this.user, 'Can\'t restore tree view state without an identity provider');
        }
        this.input = input;
        if (!input) {
            this.nodesByIdentity.clear();
            this.model.setChildren(null, iterator_js_1.Iterable.empty());
            return;
        }
        if (!viewState) {
            this._refresh(input);
    ...
    iterate(element, isCollapsed) {
        ...
        const elements = iterator_js_1.Iterable.map(children, element => {
