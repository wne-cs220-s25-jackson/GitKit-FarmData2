...
const sash_js_1 = require("../sash/sash.js");
...
const defaultStyles = {
    separatorBorder: color_js_1.Color.transparent
};
class ViewItem {
    set size(size) {
        this._size = size;
    ...
    setVisible(visible, size) {
        ...
        if (visible) {
            this.size = (0, numbers_js_1.clamp)(this._cachedVisibleSize, this.viewMinimumSize, this.viewMaximumSize);
 ...
 */
class SplitView extends lifecycle_js_1.Disposable {
    /**
    ...
    constructor(container, options = {}) {
        ...
        this._onDidSashChange = this._register(new event_js_2.Emitter());
        this._onDidSashReset = this._register(new event_js_2.Emitter());
        ...
        this.sashContainer = (0, dom_js_1.append)(this.el, (0, dom_js_1.$)('.sash-container'));
        this.viewContainer = (0, dom_js_1.$)('.split-view-container');
        this.scrollable = this._register(new scrollable_js_1.Scrollable({
            forceIntegerValues: true,
            smoothScrollDuration: 125,
            scheduleAtNextAnimationFrame: callback => (0, dom_js_1.scheduleAtNextAnimationFrame)((0, dom_js_1.getWindow)(this.el), callback),
        }));
        this.scrollableElement = this._register(new scrollableElement_js_1.SmoothScrollableElement(this.viewContainer, {
        ...
        const onDidScrollViewContainer = this._register(new event_js_1.DomEmitter(this.viewContainer, 'scroll')).event;
        ...
        (0, dom_js_1.append)(this.el, this.scrollableElement.getDomNode());
    ...
    layout(size, layoutContext) {
        ...
        if (!this.proportions) {
            const indexes = (0, arrays_js_1.range)(this.viewItems.length);
            ...
            for (let i = 0; i < this.viewItems.length; i++) {
                ...
                if (typeof proportion === 'number' && total > 0) {
                    item.size = (0, numbers_js_1.clamp)(Math.round(proportion * size / total), item.minimumSize, item.maximumSize);
                }
            }
        }
    ...
    onSashStart({ sash, start, alt }) {
        ...
        const disposable = (0, lifecycle_js_1.combinedDisposable)((0, dom_js_1.addDisposableListener)(this.el.ownerDocument.body, 'keydown', e => resetSashDragState(this.sashDragState.current, e.altKey)), (0, dom_js_1.addDisposableListener)(this.el.ownerDocument.body, 'keyup', () => resetSashDragState(this.sashDragState.current, false)));
        const resetSashDragState = (start, alt) => {
            ...
            let snapAfter;
            if (!alt) {
                const upIndexes = (0, arrays_js_1.range)(index, -1);
                const downIndexes = (0, arrays_js_1.range)(index + 1, this.viewItems.length);
    ...
    onViewChange(item, size) {
        ...
        size = (0, numbers_js_1.clamp)(size, item.minimumSize, item.maximumSize);
    ...
    resizeView(index, size) {
        ...
        this.state = State.Busy;
        try {
            const indexes = (0, arrays_js_1.range)(this.viewItems.length).filter(i => i !== index);
            const lowPriorityIndexes = [...indexes.filter(i => this.viewItems[i].priority === 1 /* LayoutPriority.Low */), index];
            const highPriorityIndexes = indexes.filter(i => this.viewItems[i].priority === 2 /* LayoutPriority.High */);
            const item = this.viewItems[index];
            size = Math.round(size);
            size = (0, numbers_js_1.clamp)(size, item.minimumSize, Math.min(item.maximumSize, this.size));
            item.size = size;
    ...
    distributeViewSizes() {
        ...
        for (const item of flexibleViewItems) {
            item.size = (0, numbers_js_1.clamp)(size, item.minimumSize, item.maximumSize);
        }
        const indexes = (0, arrays_js_1.range)(this.viewItems.length);
        const lowPriorityIndexes = indexes.filter(i => this.viewItems[i].priority === 1 /* LayoutPriority.Low */);
        const highPriorityIndexes = indexes.filter(i => this.viewItems[i].priority === 2 /* LayoutPriority.High */);
    ...
    doAddView(view, size, index = this.viewItems.length, skipLayout) {
        ...
        try {
            // Add view
            const container = (0, dom_js_1.$)('.split-view-view');
            ...
            const containerDisposable = (0, lifecycle_js_1.toDisposable)(() => container.remove());
            const disposable = (0, lifecycle_js_1.combinedDisposable)(onChangeDisposable, containerDisposable);
            let viewSize;
            if (typeof size === 'number') {
                viewSize = size;
            }
            ...
            if (this.viewItems.length > 1) {
                ...
                const sash = this.orientation === 0 /* Orientation.VERTICAL */
                    ? new sash_js_1.Sash(this.sashContainer, { getHorizontalSashTop: s => this.getSashPosition(s), getHorizontalSashWidth: this.getSashOrthogonalSize }, { ...opts, orientation: 1 /* Orientation.HORIZONTAL */ })
                    : new sash_js_1.Sash(this.sashContainer, { getVerticalSashLeft: s => this.getSashPosition(s), getVerticalSashHeight: this.getSashOrthogonalSize }, { ...opts, orientation: 0 /* Orientation.VERTICAL */ });
                ...
                const onStart = event_js_2.Event.map(sash.onDidStart, sashEventMapper);
                const onStartDisposable = onStart(this.onSashStart, this);
                const onChange = event_js_2.Event.map(sash.onDidChange, sashEventMapper);
                const onChangeDisposable = onChange(this.onSashChange, this);
                const onEnd = event_js_2.Event.map(sash.onDidEnd, () => this.sashItems.findIndex(item => item.sash === sash));
                ...
                const disposable = (0, lifecycle_js_1.combinedDisposable)(onStartDisposable, onChangeDisposable, onEndDisposable, onDidResetDisposable, sash);
    ...
    resize(index, delta, sizes = this.viewItems.map(i => i.size), lowPriorityIndexes, highPriorityIndexes, overloadMinDelta = Number.NEGATIVE_INFINITY, overloadMaxDelta = Number.POSITIVE_INFINITY, snapBefore, snapAfter) {
        ...
        if (highPriorityIndexes) {
            for (const index of highPriorityIndexes) {
                (0, arrays_js_1.pushToStart)(upIndexes, index);
                (0, arrays_js_1.pushToStart)(downIndexes, index);
            }
        }
        if (lowPriorityIndexes) {
            for (const index of lowPriorityIndexes) {
                (0, arrays_js_1.pushToEnd)(upIndexes, index);
                (0, arrays_js_1.pushToEnd)(downIndexes, index);
            }
        }
        ...
        delta = (0, numbers_js_1.clamp)(delta, minDelta, maxDelta);
        for (let i = 0, deltaUp = delta; i < upItems.length; i++) {
            const item = upItems[i];
            const size = (0, numbers_js_1.clamp)(upSizes[i] + deltaUp, item.minimumSize, item.maximumSize);
        ...
        for (let i = 0, deltaDown = delta; i < downItems.length; i++) {
            ...
            const size = (0, numbers_js_1.clamp)(downSizes[i] - deltaDown, item.minimumSize, item.maximumSize);
    ...
    distributeEmptySpace(lowPriorityIndex) {
        ...
        const indexes = (0, arrays_js_1.range)(this.viewItems.length - 1, -1);
        const lowPriorityIndexes = indexes.filter(i => this.viewItems[i].priority === 1 /* LayoutPriority.Low */);
        const highPriorityIndexes = indexes.filter(i => this.viewItems[i].priority === 2 /* LayoutPriority.High */);
        for (const index of highPriorityIndexes) {
            (0, arrays_js_1.pushToStart)(indexes, index);
        }
        for (const index of lowPriorityIndexes) {
            (0, arrays_js_1.pushToEnd)(indexes, index);
        }
        if (typeof lowPriorityIndex === 'number') {
            (0, arrays_js_1.pushToEnd)(indexes, lowPriorityIndex);
        }
        for (let i = 0; emptyDelta !== 0 && i < indexes.length; i++) {
            const item = this.viewItems[indexes[i]];
            const size = (0, numbers_js_1.clamp)(item.size + emptyDelta, item.minimumSize, item.maximumSize);
    ...
    dispose() {
        this.sashDragState?.disposable.dispose();
        (0, lifecycle_js_1.dispose)(this.viewItems);
        this.viewItems = [];
