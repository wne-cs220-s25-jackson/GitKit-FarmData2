...
const listView_js_1 = require("../list/listView.js");
const abstractTree_js_1 = require("./abstractTree.js");
const indexTreeModel_js_1 = require("./indexTreeModel.js");
const objectTree_js_1 = require("./objectTree.js");
...
const iterator_js_1 = require("../../../common/iterator.js");
const lifecycle_js_1 = require("../../../common/lifecycle.js");
const types_js_1 = require("../../../common/types.js");
const cancellation_js_1 = require("../../../common/cancellation.js");
...
class AsyncDataTreeRenderer {
    ...
    renderTwistie(element, twistieElement) {
        if (element.slow) {
            twistieElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.treeItemLoading));
            return true;
        }
        else {
            twistieElement.classList.remove(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.treeItemLoading));
            return false;
        }
    }
...
class AsyncDataTreeElementsDragAndDropData extends listView_js_1.ElementsDragAndDropData {
    set context(context) {
...
function asAsyncDataTreeDragAndDropData(data) {
    if (data instanceof listView_js_1.ElementsDragAndDropData) {
...
class AsyncFindFilter extends abstractTree_js_1.FindFilter {
    ...
    filter(element, parentVisibility) {
        ...
        if (!this.isFindSessionActive || this.findMode === abstractTree_js_1.TreeFindMode.Highlight || !this.findProvider.isVisible) {
            return filterResult;
        }
        const visibility = (0, indexTreeModel_js_1.isFilterResult)(filterResult) ? filterResult.visibility : filterResult;
        if ((0, indexTreeModel_js_1.getVisibleState)(visibility) === 0 /* TreeVisibility.Hidden */) {
...
// TODO Fix types
class AsyncFindController extends abstractTree_js_1.FindController {
    constructor(tree, findProvider, filter, contextViewProvider, options) {
        ...
        this.taskQueue = new async_js_1.ThrottledDelayer(250);
        // Always make sure to end the session before disposing
        this.disposables.add((0, lifecycle_js_1.toDisposable)(async () => {
    ...
    applyPattern(_pattern) {
        ...
        this.activeTokenSource = new cancellation_js_1.CancellationTokenSource();
    ...
    onDidToggleChange(e) {
        ...
        this.placeholder = this.mode === abstractTree_js_1.TreeFindMode.Filter ? (0, nls_js_1.localize)('type to filter', "Type to filter") : (0, nls_js_1.localize)('type to search', "Type to search");
    ...
    shouldFocusWhenNavigating(node) {
        ...
        return !filters_js_1.FuzzyScore.isDefault(node.filterData);
    }
}
...
class AsyncDataTree {
    ...
    get onDidChangeFocus() { return event_js_1.Event.map(this.tree.onDidChangeFocus, asTreeEvent); }
    get onDidChangeSelection() { return event_js_1.Event.map(this.tree.onDidChangeSelection, asTreeEvent); }
    get onKeyDown() { return this.tree.onKeyDown; }
    get onMouseClick() { return event_js_1.Event.map(this.tree.onMouseClick, asTreeMouseEvent); }
    get onMouseDblClick() { return event_js_1.Event.map(this.tree.onMouseDblClick, asTreeMouseEvent); }
    get onContextMenu() { return event_js_1.Event.map(this.tree.onContextMenu, asTreeContextMenuEvent); }
    get onTap() { return event_js_1.Event.map(this.tree.onTap, asTreeMouseEvent); }
    get onPointer() { return event_js_1.Event.map(this.tree.onPointer, asTreeMouseEvent); }
    ...
    constructor(user, container, delegate, renderers, dataSource, options = {}) {
        ...
        this._onDidRender = new event_js_1.Emitter();
        this._onDidChangeNodeSlowState = new event_js_1.Emitter();
        this.nodeMapper = new tree_js_1.WeakMapper(node => new AsyncDataTreeNodeWrapper(node));
        this.disposables = new lifecycle_js_1.DisposableStore();
        this.identityProvider = options.identityProvider;
        this.autoExpandSingleChildren = typeof options.autoExpandSingleChildren === 'undefined' ? false : options.autoExpandSingleChildren;
        this.sorter = options.sorter;
        this.getDefaultCollapseState = e => options.collapseByDefault ? (options.collapseByDefault(e) ? tree_js_1.ObjectTreeElementCollapseState.PreserveOrCollapsed : tree_js_1.ObjectTreeElementCollapseState.PreserveOrExpanded) : undefined;
    ...
    createTree(user, container, delegate, renderers, options) {
        const objectTreeDelegate = new abstractTree_js_1.ComposedTreeDelegate(delegate);
        const objectTreeRenderers = renderers.map(r => new AsyncDataTreeRenderer(r, this.nodeMapper, this._onDidChangeNodeSlowState.event));
        const objectTreeOptions = asObjectTreeOptions(options) || {};
        return new objectTree_js_1.ObjectTree(user, container, objectTreeDelegate, objectTreeRenderers, objectTreeOptions);
    }
    ...
    async _updateChildren(element = this.root.element, recursive = true, rerender = false, viewStateContext, options) {
        if (typeof this.root.element === 'undefined') {
            throw new tree_js_1.TreeError(this.user, 'Tree input not set');
        }
        if (this.root.refreshPromise) {
            await this.root.refreshPromise;
            await event_js_1.Event.toPromise(this._onDidRender.event);
        }
    ...
    async expandTo(element) {
        ...
        for (const element of iterator_js_1.Iterable.reverse(elements)) {
    ...
    getDataNode(element) {
        ...
        if (!node) {
            ...
            throw new tree_js_1.TreeError(this.user, `Data tree node not found${nodeIdentity ? `: ${nodeIdentity}` : ''}`);
        }
        return node;
    }
    ...
    async doRefreshSubTree(node, recursive, viewStateContext) {
        ...
        try {
            ...
            node.stale = false;
            await async_js_1.Promises.settled(childrenToRefresh.map(child => this.doRefreshSubTree(child, recursive, viewStateContext)));
        }
        finally {
            done();
        }
    ...
    async doRefreshNode(node, recursive, viewStateContext) {
        ...
        let childrenPromise;
        if (!node.hasChildren) {
            childrenPromise = Promise.resolve(iterator_js_1.Iterable.empty());
        }
        else {
            const children = this.doGetChildren(node);
            if ((0, types_js_1.isIterable)(children)) {
                childrenPromise = Promise.resolve(children);
            }
            else {
                const slowTimeout = (0, async_js_1.timeout)(800);
                slowTimeout.then(() => {
                    node.slow = true;
    ...
    doGetChildren(node) {
        ...
        if ((0, types_js_1.isIterable)(children)) {
            ...
            result = (0, async_js_1.createCancelablePromise)(async () => this.processChildren(await children));
    ...
    setChildren(node, childrenElementsIterable, recursive, viewStateContext) {
        ...
        const children = childrenElements.map(element => {
            ...
            if (!this.identityProvider) {
                ...
                if (hasChildren && asyncDataTreeNode.defaultCollapseState === tree_js_1.ObjectTreeElementCollapseState.PreserveOrExpanded) {
            ...
            else if (hasChildren && childAsyncDataTreeNode.defaultCollapseState === tree_js_1.ObjectTreeElementCollapseState.PreserveOrExpanded) {
                childrenToRefresh.push(childAsyncDataTreeNode);
            }
        ...
        (0, arrays_js_1.splice)(node.children, 0, node.children.length, children);
    ...
    asTreeElement(node, viewStateContext) {
        ...
        return {
            element: node,
            children: node.hasChildren ? iterator_js_1.Iterable.map(node.children, child => this.asTreeElement(child, viewStateContext)) : [],
            collapsible: node.hasChildren,
            collapsed
        };
    }
    ...
    getViewState() {
        if (!this.identityProvider) {
            throw new tree_js_1.TreeError(this.user, 'Can\'t get tree view state without an identity provider');
        }
...
class CompressibleAsyncDataTree extends AsyncDataTree {
    constructor(user, container, virtualDelegate, compressionDelegate, renderers, dataSource, options = {}) {
        ...
        this.compressibleNodeMapper = new tree_js_1.WeakMapper(node => new CompressibleAsyncDataTreeNodeWrapper(node));
    ...
    createTree(user, container, delegate, renderers, options) {
        ...
        return new objectTree_js_1.CompressibleObjectTree(user, container, objectTreeDelegate, objectTreeRenderers, objectTreeOptions);
    }
    asTreeElement(node, viewStateContext) {
        return {
    ...
    processChildren(children) {
        if (this.filter) {
            children = iterator_js_1.Iterable.filter(children, e => {
...
function getVisibility(filterResult) {
    ...
    else if ((0, indexTreeModel_js_1.isFilterResult)(filterResult)) {
        return (0, indexTreeModel_js_1.getVisibleState)(filterResult.visibility);
    }
    else {
        return (0, indexTreeModel_js_1.getVisibleState)(filterResult);
    }
}
