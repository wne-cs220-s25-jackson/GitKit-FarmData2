...
const toggle_js_1 = require("../toggle/toggle.js");
const inputBox_js_1 = require("../inputbox/inputBox.js");
...
const labels_js_1 = require("../../../common/labels.js");
const lifecycle_js_1 = require("../../../common/lifecycle.js");
const platform_js_1 = require("../../../common/platform.js");
require("./dialog.css");
const nls = require("../../../../nls.js");
class Dialog extends lifecycle_js_1.Disposable {
    constructor(container, message, buttons, options) {
        super();
        this.container = container;
        this.message = message;
        this.options = options;
        this.modalElement = this.container.appendChild((0, dom_js_1.$)(`.monaco-dialog-modal-block.dimmed`));
        this.shadowElement = this.modalElement.appendChild((0, dom_js_1.$)('.dialog-shadow'));
        this.element = this.shadowElement.appendChild((0, dom_js_1.$)('.monaco-dialog-box'));
        this.element.setAttribute('role', 'dialog');
        this.element.tabIndex = -1;
        (0, dom_js_1.hide)(this.element);
        ...
        const buttonsRowElement = this.element.appendChild((0, dom_js_1.$)('.dialog-buttons-row'));
        this.buttonsContainer = buttonsRowElement.appendChild((0, dom_js_1.$)('.dialog-buttons'));
        const messageRowElement = this.element.appendChild((0, dom_js_1.$)('.dialog-message-row'));
        this.iconElement = messageRowElement.appendChild((0, dom_js_1.$)('#monaco-dialog-icon.dialog-icon'));
        this.iconElement.setAttribute('aria-label', this.getIconAriaLabel());
        this.messageContainer = messageRowElement.appendChild((0, dom_js_1.$)('.dialog-message-container'));
        if (this.options.detail || this.options.renderBody) {
            const messageElement = this.messageContainer.appendChild((0, dom_js_1.$)('.dialog-message'));
            const messageTextElement = messageElement.appendChild((0, dom_js_1.$)('#monaco-dialog-message-text.dialog-message-text'));
            messageTextElement.innerText = this.message;
        }
        this.messageDetailElement = this.messageContainer.appendChild((0, dom_js_1.$)('#monaco-dialog-message-detail.dialog-message-detail'));
        if (this.options.detail || !this.options.renderBody) {
        ...
        if (this.options.renderBody) {
            const customBody = this.messageContainer.appendChild((0, dom_js_1.$)('#monaco-dialog-message-body.dialog-message-body'));
        ...
        if (this.options.inputs) {
            this.inputs = this.options.inputs.map(input => {
                const inputRowElement = this.messageContainer.appendChild((0, dom_js_1.$)('.dialog-message-input'));
                const inputBox = this._register(new inputBox_js_1.InputBox(inputRowElement, undefined, {
        ...
        if (this.options.checkboxLabel) {
            const checkboxRowElement = this.messageContainer.appendChild((0, dom_js_1.$)('.dialog-checkbox-row'));
            const checkbox = this.checkbox = this._register(new toggle_js_1.Checkbox(this.options.checkboxLabel, !!this.options.checkboxChecked, options.checkboxStyles));
            checkboxRowElement.appendChild(checkbox.domNode);
            const checkboxMessageElement = checkboxRowElement.appendChild((0, dom_js_1.$)('.dialog-checkbox-message'));
            checkboxMessageElement.innerText = this.options.checkboxLabel;
            this._register((0, dom_js_1.addDisposableListener)(checkboxMessageElement, dom_js_1.EventType.CLICK, () => checkbox.checked = !checkbox.checked));
        }
        const toolbarRowElement = this.element.appendChild((0, dom_js_1.$)('.dialog-toolbar-row'));
        this.toolbarContainer = toolbarRowElement.appendChild((0, dom_js_1.$)('.dialog-toolbar'));
        this.applyStyles();
    }
    getIconAriaLabel() {
    ...
    async show() {
        ...
        return new Promise((resolve) => {
            (0, dom_js_1.clearNode)(this.buttonsContainer);
            const close = () => {
                resolve({
            ...
            if (this.options.closeOnLinkClick) {
                for (const el of this.messageContainer.getElementsByTagName('a')) {
                    this._register((0, dom_js_1.addDisposableListener)(el, dom_js_1.EventType.CLICK, () => {
                        setTimeout(close); // HACK to ensure the link action is triggered before the dialog is closed
                    }));
                    this._register((0, dom_js_1.addDisposableListener)(el, dom_js_1.EventType.KEY_DOWN, (e) => {
            ...
            const buttonBar = this.buttonBar = this._register(new button_js_1.ButtonBar(this.buttonsContainer));
            ...
            buttonMap.forEach((entry, index) => {
                ...
                button.label = (0, labels_js_1.mnemonicButtonLabel)(buttonMap[index].label, true);
                if (button instanceof button_js_1.ButtonWithDescription) {
            ...
            const window = (0, dom_js_1.getWindow)(this.container);
            this._register((0, dom_js_1.addDisposableListener)(window, 'keydown', e => {
                const evt = new keyboardEvent_js_1.StandardKeyboardEvent(e);
                ...
                if (evt.equals(2 /* KeyCode.Tab */) || evt.equals(17 /* KeyCode.RightArrow */) || evt.equals(1024 /* KeyMod.Shift */ | 2 /* KeyCode.Tab */) || evt.equals(15 /* KeyCode.LeftArrow */)) {
                    ...
                    if (this.messageContainer) {
                        ...
                        for (const link of links) {
                            ...
                            if ((0, dom_js_1.isActiveElement)(link)) {
            ...
            this._register((0, dom_js_1.addDisposableListener)(window, 'keyup', e => {
                dom_js_1.EventHelper.stop(e, true);
                const evt = new keyboardEvent_js_1.StandardKeyboardEvent(e);
            ...
            // Detect focus out
            this._register((0, dom_js_1.addDisposableListener)(this.element, 'focusout', e => {
                if (!!e.relatedTarget && !!this.element) {
                    if (!(0, dom_js_1.isAncestor)(e.relatedTarget, this.element)) {
            ...
            this.iconElement.classList.remove(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogError), ...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogWarning), ...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogInfo), ...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.loading), spinModifierClassName);
            if (this.options.icon) {
                this.iconElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(this.options.icon));
            }
            else {
                switch (this.options.type) {
                    case 'error':
                        this.iconElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogError));
                        break;
                    case 'warning':
                        this.iconElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogWarning));
                        break;
                    case 'pending':
                        this.iconElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.loading), spinModifierClassName);
                        break;
                    case 'none':
                    ...
                    case 'question':
                    default:
                        this.iconElement.classList.add(...themables_js_1.ThemeIcon.asClassNameArray(codicons_js_1.Codicon.dialogInfo));
                        break;
                }
            }
            if (!this.options.disableCloseAction) {
                const actionBar = this._register(new actionbar_js_1.ActionBar(this.toolbarContainer, {}));
                const action = this._register(new actions_js_1.Action('dialog.close', nls.localize('dialogClose', "Close Dialog"), themables_js_1.ThemeIcon.asClassName(codicons_js_1.Codicon.dialogClose), true, async () => {
                    resolve({
                        button: this.options.cancelId || 0,
                        checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
                    });
    ...
    dispose() {
        ...
        if (this.focusToReturn && (0, dom_js_1.isAncestor)(this.focusToReturn, this.container.ownerDocument.body)) {
    ...
    rearrangeButtons(buttons, cancelId) {
        ...
        if (platform_js_1.isMacintosh || platform_js_1.isLinux) {
