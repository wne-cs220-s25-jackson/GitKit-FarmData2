...
const symbols_js_1 = require("../../../common/symbols.js");
const diff_js_1 = require("../../../common/diff/diff.js");
...
class IndexTreeModel {
    constructor(user, rootElement, options = {}) {
        this.user = user;
        this.rootRef = [];
        this.eventBufferer = new event_js_1.EventBufferer();
        this._onDidSpliceModel = new event_js_1.Emitter();
        this.onDidSpliceModel = this._onDidSpliceModel.event;
        this._onDidSpliceRenderedNodes = new event_js_1.Emitter();
        this.onDidSpliceRenderedNodes = this._onDidSpliceRenderedNodes.event;
        this._onDidChangeCollapseState = new event_js_1.Emitter();
        this.onDidChangeCollapseState = this.eventBufferer.wrapEvent(this._onDidChangeCollapseState.event);
        this._onDidChangeRenderNodeCount = new event_js_1.Emitter();
        this.onDidChangeRenderNodeCount = this.eventBufferer.wrapEvent(this._onDidChangeRenderNodeCount.event);
        this.refilterDelayer = new async_js_1.Delayer(symbols_js_1.MicrotaskDelay);
    ...
    splice(location, deleteCount, toInsert = iterator_js_1.Iterable.empty(), options = {}) {
        if (location.length === 0) {
            throw new tree_js_1.TreeError(this.user, 'Invalid tree location');
        }
    ...
    spliceSmart(identity, location, deleteCount, toInsertIterable = iterator_js_1.Iterable.empty(), options, recurseLevels = options.diffDepth ?? 0) {
        ...
        const diff = new diff_js_1.LcsDiff({ getElements: () => parentNode.lastDiffIds }, {
            getElements: () => [
        ...
        for (const change of diff.changes.sort((a, b) => b.originalStart - a.originalStart)) {
            ...
            this.spliceSimple([...locationPrefix, lastStartO], change.originalLength, iterator_js_1.Iterable.slice(toInsert, lastStartM, lastStartM + change.modifiedLength), options);
        }
        // at this point, startO === startM === count since any remaining prefix should match
        recurseSplice(lastStartO, lastStartM, lastStartO);
    }
    spliceSimple(location, deleteCount, toInsert = iterator_js_1.Iterable.empty(), { onDidCreateNode, onDidDeleteNode, diffIdentityProvider }) {
        const { parentNode, listIndex, revealed, visible } = this.getParentNodeWithListIndex(location);
        const treeListElementsToInsert = [];
        const nodesToInsertIterator = iterator_js_1.Iterable.map(toInsert, el => this.createTreeNode(el, parentNode, parentNode.visible ? 1 /* TreeVisibility.Visible */ : 0 /* TreeVisibility.Hidden */, revealed, treeListElementsToInsert, onDidCreateNode));
        ...
        const deletedNodes = (0, arrays_js_1.splice)(parentNode.children, lastIndex, deleteCount, nodesToInsert);
        if (!diffIdentityProvider) {
            parentNode.lastDiffIds = undefined;
        }
        else if (parentNode.lastDiffIds) {
            (0, arrays_js_1.splice)(parentNode.lastDiffIds, lastIndex, deleteCount, nodesToInsert.map(n => diffIdentityProvider.getId(n.element).toString()));
        }
        else {
    ...
    createTreeNode(treeElement, parent, parentVisibility, revealed, treeListElements, onDidCreateNode) {
        ...
        const childElements = treeElement.children || iterator_js_1.Iterable.empty();
    ...
    getParentNodeLocation(location) {
        if (location.length === 0) {
            ...
            return (0, arrays_js_1.tail2)(location)[0];
        }
    }
