...
const domStylesheets_js_1 = require("../../domStylesheets.js");
const cssValue_js_1 = require("../../cssValue.js");
const event_js_1 = require("../../event.js");
const keyboardEvent_js_1 = require("../../keyboardEvent.js");
const touch_js_1 = require("../../touch.js");
const aria_js_1 = require("../aria/aria.js");
const splice_js_1 = require("./splice.js");
...
const filters_js_1 = require("../../../common/filters.js");
...
const listView_js_1 = require("./listView.js");
const mouseEvent_js_1 = require("../../mouseEvent.js");
const observable_js_1 = require("../../../common/observable.js");
class TraitRenderer {
    constructor(trait) {
        this.trait = trait;
...
class Trait {
    ...
    constructor(_trait) {
        ...
        this.indexes = [];
        this.sortedIndexes = [];
        this._onChange = new event_js_2.Emitter();
    ...
    contains(index) {
        return (0, arrays_js_1.binarySearch)(this.sortedIndexes, index, numericSort) >= 0;
    }
    dispose() {
        (0, lifecycle_js_1.dispose)(this._onChange);
    }
}
__decorate([
...
class KeyboardController {
    get onKeyDown() {
        return event_js_2.Event.chain(this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keydown')).event, $ => $.filter(e => !(0, dom_js_1.isEditableElement)(e.target))
            .map(e => new keyboardEvent_js_1.StandardKeyboardEvent(e)));
    }
    constructor(list, view, options) {
        this.list = list;
        this.view = view;
        this.disposables = new lifecycle_js_1.DisposableStore();
        this.multipleSelectionDisposables = new lifecycle_js_1.DisposableStore();
    ...
    onCtrlA(e) {
        e.preventDefault();
        e.stopPropagation();
        this.list.setSelection((0, arrays_js_1.range)(this.list.length), e.browserEvent);
...
class TypeNavigationController {
    constructor(list, view, keyboardNavigationLabelProvider, keyboardNavigationEventFilter, delegate) {
        ...
        this.enabledDisposables = new lifecycle_js_1.DisposableStore();
        this.disposables = new lifecycle_js_1.DisposableStore();
    ...
    enable() {
        ...
        let typing = false;
        const onChar = event_js_2.Event.chain(this.enabledDisposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keydown')).event, $ => $.filter(e => !(0, dom_js_1.isEditableElement)(e.target))
            .filter(() => this.mode === TypeNavigationMode.Automatic || this.triggered)
            .map(event => new keyboardEvent_js_1.StandardKeyboardEvent(event))
            .filter(e => typing || this.keyboardNavigationEventFilter(e))
            .filter(e => this.delegate.mightProducePrintableCharacter(e))
            .forEach(e => dom_js_1.EventHelper.stop(e, true))
            .map(event => event.browserEvent.key));
        const onClear = event_js_2.Event.debounce(onChar, () => null, 800, undefined, undefined, undefined, this.enabledDisposables);
        const onInput = event_js_2.Event.reduce(event_js_2.Event.any(onChar, onClear), (r, i) => i === null ? null : ((r || '') + i), undefined, this.enabledDisposables);
    ...
    onClear() {
        ...
        if (focus.length > 0 && focus[0] === this.previouslyFocused) {
            ...
            if (typeof ariaLabel === 'string') {
                (0, aria_js_1.alert)(ariaLabel);
            }
            else if (ariaLabel) {
                (0, aria_js_1.alert)(ariaLabel.get());
            }
        }
        this.previouslyFocused = -1;
    }
        ...
        for (let i = 0; i < this.list.length; i++) {
            ...
            if (this.list.options.typeNavigationEnabled) {
                if (typeof labelStr !== 'undefined') {
                    ...
                    if ((0, filters_js_1.matchesPrefix)(word, labelStr)) {
                        this.previouslyFocused = start;
                        this.list.setFocus([index]);
                        this.list.reveal(index);
                        return;
                    }
                    const fuzzy = (0, filters_js_1.matchesFuzzy2)(word, labelStr);
                    if (fuzzy) {
            ...
            else if (typeof labelStr === 'undefined' || (0, filters_js_1.matchesPrefix)(word, labelStr)) {
                this.previouslyFocused = start;
                this.list.setFocus([index]);
                this.list.reveal(index);
                return;
...
class DOMFocusController {
    constructor(list, view) {
        ...
        this.view = view;
        this.disposables = new lifecycle_js_1.DisposableStore();
        const onKeyDown = event_js_2.Event.chain(this.disposables.add(new event_js_1.DomEmitter(view.domNode, 'keydown')).event, $ => $
            .filter(e => !(0, dom_js_1.isEditableElement)(e.target))
            .map(e => new keyboardEvent_js_1.StandardKeyboardEvent(e)));
        const onTab = event_js_2.Event.chain(onKeyDown, $ => $.filter(e => e.keyCode === 2 /* KeyCode.Tab */ && !e.ctrlKey && !e.metaKey && !e.shiftKey && !e.altKey));
    ...
    onTab(e) {
        ...
        if (!tabIndexElement || !((0, dom_js_1.isHTMLElement)(tabIndexElement)) || tabIndexElement.tabIndex === -1) {
            return;
        }
        const style = (0, dom_js_1.getWindow)(tabIndexElement).getComputedStyle(tabIndexElement);
...
function isMouseRightClick(event) {
    return (0, dom_js_1.isMouseEvent)(event) && event.button === 2;
}
...
class MouseController {
    constructor(list) {
        this.list = list;
        this.disposables = new lifecycle_js_1.DisposableStore();
        this._onPointer = new event_js_2.Emitter();
        ...
        if (this.mouseSupport) {
            ...
            this.disposables.add(touch_js_1.Gesture.addTarget(list.getHTMLElement()));
        }
        event_js_2.Event.any(list.onMouseClick, list.onMouseMiddleClick, list.onTap)(this.onViewPointer, this, this.disposables);
    }
    updateOptions(optionsUpdate) {
        if (optionsUpdate.multipleSelectionSupport !== undefined) {
    ...
    onMouseDown(e) {
        ...
        if ((0, dom_js_1.getActiveElement)() !== e.browserEvent.target) {
            this.list.domFocus();
        }
    }
    onContextMenu(e) {
        if ((0, dom_js_1.isEditableElement)(e.browserEvent.target) || isMonacoEditor(e.browserEvent.target)) {
            return;
        }
    ...
    changeSelection(e) {
        ...
        if (this.isSelectionRangeChangeEvent(e)) {
            ...
            const rangeSelection = (0, arrays_js_1.range)(min, max + 1);
...
class DefaultStyleController {
    ...
    style(styles) {
         ...
         * Outlines
         */
        const focusAndSelectionOutline = (0, cssValue_js_1.asCssValueWithDefault)(styles.listFocusAndSelectionOutline, (0, cssValue_js_1.asCssValueWithDefault)(styles.listSelectionOutline, styles.listFocusOutline ?? ''));
        ...
        const inactiveFocusAndSelectionOutline = (0, cssValue_js_1.asCssValueWithDefault)(styles.listSelectionOutline, styles.listInactiveFocusOutline ?? '');
...
exports.unthemedListStyles = {
    ...
    treeInactiveIndentGuidesStroke: color_js_1.Color.fromHex('#a9a9a9').transparent(0.4).toString(),
    tableColumnsBorder: color_js_1.Color.fromHex('#cccccc').transparent(0.2).toString(),
    tableOddRowsBackgroundColor: color_js_1.Color.fromHex('#cccccc').transparent(0.04).toString(),
...
class AccessibiltyRenderer {
    ...
    renderTemplate(container) {
        return { container, disposables: new lifecycle_js_1.DisposableStore() };
    }
    renderElement(element, index, data) {
        const ariaLabel = this.accessibilityProvider.getAriaLabel(element);
        const observable = (ariaLabel && typeof ariaLabel !== 'string') ? ariaLabel : (0, observable_js_1.constObservable)(ariaLabel);
        data.disposables.add((0, observable_js_1.autorun)(reader => {
...
class List {
    get onDidChangeFocus() {
        return event_js_2.Event.map(this.eventBufferer.wrapEvent(this.focus.onChange), e => this.toListEvent(e), this.disposables);
    }
    get onDidChangeSelection() {
        return event_js_2.Event.map(this.eventBufferer.wrapEvent(this.selection.onChange), e => this.toListEvent(e), this.disposables);
    }
    ...
    get onContextMenu() {
        ...
        const fromKeyDown = event_js_2.Event.chain(this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keydown')).event, $ => $.map(e => new keyboardEvent_js_1.StandardKeyboardEvent(e))
            .filter(e => didJustPressContextMenuKey = e.keyCode === 58 /* KeyCode.ContextMenu */ || (e.shiftKey && e.keyCode === 68 /* KeyCode.F10 */))
            .map(e => dom_js_1.EventHelper.stop(e, true))
            .filter(() => false));
        const fromKeyUp = event_js_2.Event.chain(this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keyup')).event, $ => $.forEach(() => didJustPressContextMenuKey = false)
            .map(e => new keyboardEvent_js_1.StandardKeyboardEvent(e))
        ...
        const fromMouse = event_js_2.Event.chain(this.view.onContextMenu, $ => $.filter(_ => !didJustPressContextMenuKey)
            .map(({ element, index, browserEvent }) => ({ element, index, anchor: new mouseEvent_js_1.StandardMouseEvent((0, dom_js_1.getWindow)(this.view.domNode), browserEvent), browserEvent })));
        return event_js_2.Event.any(fromKeyDown, fromKeyUp, fromMouse);
    }
    get onKeyDown() { return this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keydown')).event; }
    get onKeyUp() { return this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keyup')).event; }
    get onKeyPress() { return this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'keypress')).event; }
    get onDidFocus() { return event_js_2.Event.signal(this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'focus', true)).event); }
    get onDidBlur() { return event_js_2.Event.signal(this.disposables.add(new event_js_1.DomEmitter(this.view.domNode, 'blur', true)).event); }
    constructor(user, container, virtualDelegate, renderers, _options = DefaultOptions) {
        this.user = user;
        this._options = _options;
        this.focus = new Trait('focused');
        this.anchor = new Trait('anchor');
        this.eventBufferer = new event_js_2.EventBufferer();
        this._ariaLabel = '';
        this.disposables = new lifecycle_js_1.DisposableStore();
        this._onDidDispose = new event_js_2.Emitter();
        ...
        if (_options.styleController) {
            ...
            const styleElement = (0, domStylesheets_js_1.createStyleSheet)(this.view.domNode);
            this.styleController = new DefaultStyleController(styleElement, this.view.domId);
        }
        this.spliceable = new splice_js_1.CombinedSpliceable([
    ...
    createListView(container, virtualDelegate, renderers, viewOptions) {
        return new listView_js_1.ListView(container, virtualDelegate, renderers, viewOptions);
    }
    ...
    splice(start, deleteCount, elements = []) {
        if (start < 0 || start > this.view.length) {
            throw new list_js_1.ListError(this.user, `Invalid start index: ${start}`);
        }
        if (deleteCount < 0) {
            throw new list_js_1.ListError(this.user, `Invalid delete count: ${deleteCount}`);
        }
    ...
    setSelection(indexes, browserEvent) {
        for (const index of indexes) {
            if (index < 0 || index >= this.length) {
                throw new list_js_1.ListError(this.user, `Invalid index ${index}`);
            }
        }
    ...
    reveal(index, relativeTop, paddingTop = 0) {
        ...
        if ((0, types_js_1.isNumber)(relativeTop)) {
            // y = mx + b
            const m = elementHeight - this.view.renderHeight + paddingTop;
            this.view.setScrollTop(m * (0, numbers_js_1.clamp)(relativeTop, 0, 1) + elementTop - paddingTop);
        }
        else {
    ...
    isDOMFocused() {
        return (0, dom_js_1.isActiveElement)(this.view.domNode);
    }
    getHTMLElement() {
