...
const css = require("../../cssValue.js");
const highlightedLabel_js_1 = require("../highlightedlabel/highlightedLabel.js");
...
class FastLabelNode {
    ...
    set classNames(classNames) {
        if (this.disposed || (0, objects_js_1.equals)(classNames, this._classNames)) {
            return;
        }
...
class IconLabel extends lifecycle_js_1.Disposable {
    constructor(container, options) {
        ...
        this.hoverDelegate = options?.hoverDelegate ?? (0, hoverDelegateFactory_js_1.getDefaultHoverDelegate)('mouse');
    }
    get element() {
    ...
    setLabel(label, description, options) {
        ...
        if (description || this.descriptionNode) {
            ...
            if (descriptionNode instanceof highlightedLabel_js_1.HighlightedLabel) {
                ...
                descriptionNode.textContent = description && options?.labelEscapeNewLines ? highlightedLabel_js_1.HighlightedLabel.escapeNewLines(description, []) : (description || '');
                this.setupHover(descriptionNode.element, options?.descriptionTitle || '');
    ...
    setupHover(htmlElement, tooltip) {
        ...
        if (this.hoverDelegate.showNativeHover) {
            function setupNativeHover(htmlElement, tooltip) {
                if ((0, types_js_1.isString)(tooltip)) {
                    // Icons don't render in the native hover so we strip them out
                    htmlElement.title = (0, iconLabels_js_1.stripIcons)(tooltip);
                }
            ...
            const hoverDisposable = (0, hoverDelegate2_js_1.getBaseLayerHoverDelegate)().setupManagedHover(this.hoverDelegate, hoverTarget, tooltip);
            if (hoverDisposable) {
    ...
    getOrCreateDescriptionNode() {
        if (!this.descriptionNode) {
            ...
            if (this.creationOptions?.supportDescriptionHighlights) {
                this.descriptionNode = this._register(new highlightedLabel_js_1.HighlightedLabel(dom.append(descriptionContainer.element, dom.$('span.label-description')), { supportIcons: !!this.creationOptions.supportIcons }));
            }
            else {
...
class Label {
    ...
    setLabel(label, options) {
        if (this.label === label && (0, objects_js_1.equals)(this.options, options)) {
            return;
        }
        this.label = label;
...
function splitMatches(labels, separator, matches) {
    ...
    return labels.map(label => {
        ...
        const result = matches
            .map(match => range_js_1.Range.intersect(labelRange, match))
            .filter(range => !range_js_1.Range.isEmpty(range))
            .map(({ start, end }) => ({ start: start - labelStart, end: end - labelStart }));
        labelStart = labelRange.end + separator.length;
        return result;
    });
}
class LabelWithHighlights extends lifecycle_js_1.Disposable {
    ...
    setLabel(label, options) {
        ...
        if (typeof label === 'string') {
            if (!this.singleLabel) {
                this.container.innerText = '';
                this.container.classList.remove('multiple');
                this.singleLabel = this._register(new highlightedLabel_js_1.HighlightedLabel(dom.append(this.container, dom.$('a.label-name', { id: options?.domId })), { supportIcons: this.supportIcons }));
            }
            ...
            for (let i = 0; i < label.length; i++) {
                ...
                const highlightedLabel = this._register(new highlightedLabel_js_1.HighlightedLabel(dom.append(this.container, name), { supportIcons: this.supportIcons }));
