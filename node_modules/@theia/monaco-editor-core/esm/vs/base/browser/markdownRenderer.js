...
const idGenerator_js_1 = require("../common/idGenerator.js");
const lazy_js_1 = require("../common/lazy.js");
const lifecycle_js_1 = require("../common/lifecycle.js");
const marked = require("../common/marked/marked.js");
const marshalling_js_1 = require("../common/marshalling.js");
const network_js_1 = require("../common/network.js");
const objects_js_1 = require("../common/objects.js");
const resources_js_1 = require("../common/resources.js");
...
const formattedTextRenderer_js_1 = require("./formattedTextRenderer.js");
const keyboardEvent_js_1 = require("./keyboardEvent.js");
const mouseEvent_js_1 = require("./mouseEvent.js");
const iconLabels_js_2 = require("./ui/iconLabel/iconLabels.js");
const defaultMarkedRenderers = Object.freeze({
    image: ({ href, title, text }) => {
        let dimensions = [];
        let attributes = [];
        if (href) {
            ({ href, dimensions } = (0, htmlContent_js_1.parseHrefAndDimensions)(href));
            attributes.push(`src="${(0, htmlContent_js_1.escapeDoubleQuotes)(href)}"`);
        }
        if (text) {
            attributes.push(`alt="${(0, htmlContent_js_1.escapeDoubleQuotes)(text)}"`);
        }
        if (title) {
            attributes.push(`title="${(0, htmlContent_js_1.escapeDoubleQuotes)(title)}"`);
        }
    ...
    link({ href, title, tokens }) {
        ...
        if (href === text) { // raw link case
            text = (0, htmlContent_js_1.removeMarkdownEscapes)(text);
        }
        title = typeof title === 'string' ? (0, htmlContent_js_1.escapeDoubleQuotes)((0, htmlContent_js_1.removeMarkdownEscapes)(title)) : '';
        href = (0, htmlContent_js_1.removeMarkdownEscapes)(href);
        // HTML Encode href
        href = href.replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
...
function renderMarkdown(markdown, options = {}, markedOptions = {}) {
    ...
    const element = (0, formattedTextRenderer_js_1.createElement)(options);
    ...
    if (markdown.supportThemeIcons) {
        const elements = (0, iconLabels_js_2.renderLabelWithIcons)(renderedMarkdown);
    ...
    if (options.actionHandler) {
        const onClick = options.actionHandler.disposables.add(new event_js_2.DomEmitter(element, 'click'));
        const onAuxClick = options.actionHandler.disposables.add(new event_js_2.DomEmitter(element, 'auxclick'));
        options.actionHandler.disposables.add(event_js_1.Event.any(onClick.event, onAuxClick.event)(e => {
            const mouseEvent = new mouseEvent_js_1.StandardMouseEvent(DOM.getWindow(element), e);
...
function rewriteRenderedLinks(markdown, options, root) {
        ...
        if (src) {
            ...
            try {
                if (markdown.baseUri) { // absolute or relative local path, or file: uri
                    href = resolveWithBaseUri(uri_js_1.URI.from(markdown.baseUri), href);
                }
            }
            catch (err) { }
            el.setAttribute('src', massageHref(markdown, href, true));
            if (options.remoteImageIsAllowed) {
                const uri = uri_js_1.URI.parse(href);
                if (uri.scheme !== network_js_1.Schemas.file && uri.scheme !== network_js_1.Schemas.data && !options.remoteImageIsAllowed(uri)) {
    ...
    for (const el of root.querySelectorAll('a')) {
        ...
        if (!href
            ...
            if (markdown.baseUri) {
                resolvedHref = resolveWithBaseUri(uri_js_1.URI.from(markdown.baseUri), href);
            }
...
function createMarkdownRenderer(options, markdown) {
    ...
    if (options.codeBlockRendererSync) {
        renderer.code = ({ text, lang, raw }) => {
            const id = idGenerator_js_1.defaultGenerator.nextId();
            const value = options.codeBlockRendererSync(postProcessCodeBlockLanguageId(lang), text, raw);
            syncCodeBlocks.push([id, value]);
            return `<div class="code" data-code="${id}">${(0, strings_js_1.escape)(text)}</div>`;
        };
    }
    ...
    if (!markdown.supportHtml) {
        ...
        renderer.html = ({ text }) => {
            if (options.sanitizerOptions?.replaceWithPlaintext) {
                return (0, strings_js_1.escape)(text);
            }
...
function preprocessMarkdownString(markdown) {
    ...
    // escape theme icons
    if (markdown.supportThemeIcons) {
        value = (0, iconLabels_js_1.markdownEscapeEscapedIcons)(value);
    }
    return value;
}
...
function uriMassage(markdown, part) {
    let data;
    try {
        data = (0, marshalling_js_1.parse)(decodeURIComponent(part));
    }
    catch (e) {
        // ignore
    }
    ...
    data = (0, objects_js_1.cloneAndChange)(data, value => {
        if (markdown.uris && markdown.uris[value]) {
            return uri_js_1.URI.revive(markdown.uris[value]);
        }
        else {
            return undefined;
        }
...
function massageHref(markdown, href, isDomUri) {
    ...
    let uri = uri_js_1.URI.revive(data);
    if (isDomUri) {
        if (href.startsWith(network_js_1.Schemas.data + ':')) {
            return href;
        }
        if (!uri) {
            uri = uri_js_1.URI.parse(href);
        }
        // this URI will end up as "src"-attribute of a dom node
        // and because of that special rewriting needs to be done
        // so that the URI uses a protocol that's understood by
        // browsers (like http or https)
        return network_js_1.FileAccess.uriToBrowserUri(uri).toString(true);
    }
    if (!uri) {
        return href;
    }
    if (uri_js_1.URI.parse(href).toString() === uri.toString()) {
...
function resolveWithBaseUri(baseUri, href) {
    ...
    if (baseUri.path.endsWith('/')) {
        return (0, resources_js_1.resolvePath)(baseUri, href).toString();
    }
    else {
        return (0, resources_js_1.resolvePath)((0, resources_js_1.dirname)(baseUri), href).toString();
    }
}
...
function sanitizeRenderedMarkdown(options, renderedMarkdown) {
    ...
    try {
        return dompurify_js_1.default.sanitize(renderedMarkdown, { ...config, RETURN_TRUSTED_TYPE: true });
    }
    finally {
        store.dispose();
    }
...
function getSanitizerOptions(options) {
    const allowedSchemes = [
        ...
        network_js_1.Schemas.mailto,
        network_js_1.Schemas.data,
        network_js_1.Schemas.file,
        network_js_1.Schemas.vscodeFileResource,
        network_js_1.Schemas.vscodeRemote,
        network_js_1.Schemas.vscodeRemoteResource,
    ];
    if (options.isTrusted) {
        allowedSchemes.push(network_js_1.Schemas.command);
    }
    return {
        config: {
...
const plainTextRenderer = new lazy_js_1.Lazy(createRenderer);
const plainTextWithCodeBlocksRenderer = new lazy_js_1.Lazy(() => {
    const renderer = createRenderer();
    renderer.code = ({ text }) => {
        return `\n\`\`\`\n${(0, strings_js_1.escape)(text)}\n\`\`\`\n`;
    };
    return renderer;
});
...
function addDompurifyHook(hook, cb) {
    dompurify_js_1.default.addHook(hook, cb);
    return (0, lifecycle_js_1.toDisposable)(() => dompurify_js_1.default.removeHook(hook));
}
