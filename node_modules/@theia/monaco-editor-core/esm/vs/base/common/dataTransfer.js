...
function createFileDataTransferItem(fileName, uri, data) {
    const file = { id: (0, uuid_js_1.generateUuid)(), name: fileName, uri, data };
    return {
        asString: async () => '',
        asFile: () => file,
        value: undefined,
...
class VSDataTransfer {
    ...
    matches(pattern) {
        ...
        if (iterator_js_1.Iterable.some(this, ([_, item]) => item.asFile())) {
            mimes.push('files');
        }
...
exports.UriList = Object.freeze({
    ...
    create: (entries) => {
        return (0, arrays_js_1.distinct)(entries.map(x => x.toString())).join('\r\n');
    },
    split: (str) => {
        return str.split('\r\n');
    },
