...
const extpath_js_1 = require("./extpath.js");
...
function getPathLabel(resource, formatting) {
    ...
    if (os === 1 /* OperatingSystem.Windows */ && !platform_js_1.isWindows) {
        absolutePath = absolutePath.replace(/\//g, '\\');
    }
    else if (os !== 1 /* OperatingSystem.Windows */ && platform_js_1.isWindows) {
        absolutePath = absolutePath.replace(/\\/g, '/');
    }
    ...
    if (os !== 1 /* OperatingSystem.Windows */ && tildifier?.userHome) {
        ...
        if (resource.scheme !== tildifier.userHome.scheme && resource.path[0] === path_js_1.posix.sep && resource.path[1] !== path_js_1.posix.sep) {
    ...
    // normalize
    const pathLib = os === 1 /* OperatingSystem.Windows */ ? path_js_1.win32 : path_js_1.posix;
...
function getRelativePathLabel(resource, relativePathProvider, os) {
    const pathLib = os === 1 /* OperatingSystem.Windows */ ? path_js_1.win32 : path_js_1.posix;
    const extUriLib = os === 3 /* OperatingSystem.Linux */ ? resources_js_1.extUri : resources_js_1.extUriIgnorePathCase;
    ...
    if (resource.scheme !== firstFolder.uri.scheme && resource.path[0] === path_js_1.posix.sep && resource.path[1] !== path_js_1.posix.sep) {
...
function normalizeDriveLetter(path, isWindowsOS = platform_js_1.isWindows) {
    if ((0, extpath_js_1.hasDriveLetter)(path, isWindowsOS)) {
        return path.charAt(0).toUpperCase() + path.slice(1);
    }
    return path;
}
let normalizedUserHomeCached = Object.create(null);
function tildify(path, userHome, os = platform_js_1.OS) {
    ...
    if (!normalizedUserHome) {
        ...
        if (platform_js_1.isWindows) {
            normalizedUserHome = (0, extpath_js_1.toSlashes)(normalizedUserHome); // make sure that the path is POSIX normalized on Windows
        }
        normalizedUserHome = `${(0, strings_js_1.rtrim)(normalizedUserHome, path_js_1.posix.sep)}${path_js_1.posix.sep}`;
        normalizedUserHomeCached = { original: userHome, normalized: normalizedUserHome };
    }
    let normalizedPath = path;
    if (platform_js_1.isWindows) {
        normalizedPath = (0, extpath_js_1.toSlashes)(normalizedPath); // make sure that the path is POSIX normalized on Windows
    }
    // Linux: case sensitive, macOS: case insensitive
    if (os === 3 /* OperatingSystem.Linux */ ? normalizedPath.startsWith(normalizedUserHome) : (0, strings_js_1.startsWithIgnoreCase)(normalizedPath, normalizedUserHome)) {
...
const unc = '\\\\';
const home = '~';
function shorten(paths, pathSeparator = path_js_1.sep) {
...
function mnemonicMenuLabel(label, forceDisableMnemonics) {
    if (platform_js_1.isMacintosh || forceDisableMnemonics) {
        return label.replace(/\(&&\w\)|&&/g, '').replace(/&/g, platform_js_1.isMacintosh ? '&' : '&&');
    }
...
function splitName(fullPath) {
    const p = fullPath.indexOf('/') !== -1 ? path_js_1.posix : path_js_1.win32;
