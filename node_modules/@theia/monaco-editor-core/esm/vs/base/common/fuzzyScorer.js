...
const comparers_js_1 = require("./comparers.js");
...
function computeCharScore(queryCharAtIndex, queryLowerCharAtIndex, target, targetLower, targetIndex, matchesSequenceLength) {
    ...
    if (targetIndex === 0) {
        ...
        // HTTP => HTTP = not boost
        else if ((0, filters_js_1.isUpper)(target.charCodeAt(targetIndex)) && matchesSequenceLength === 0) {
            score += 2;
            // if (DEBUG) {
...
function doScoreFuzzy2Single(target, query, patternStart, wordStart) {
    const score = (0, filters_js_1.fuzzyScore)(query.original, query.originalLowercase, patternStart, target, target.toLowerCase(), wordStart, { firstMatchCanBeWeak: true, boostFullMatch: true });
    if (!score) {
        return NO_SCORE2;
    }
    return [score[0], (0, filters_js_1.createMatches)(score)];
}
...
function getCacheHash(label, description, allowNonContiguousMatches, query) {
    ...
    const cacheHash = (0, hash_js_1.hash)({
        [query.normalized]: {
...
function doScoreItemFuzzy(label, description, path, query, allowNonContiguousMatches) {
    ...
    if (path && (platform_js_1.isLinux ? query.pathNormalized === path : (0, strings_js_1.equalsIgnoreCase)(query.pathNormalized, path))) {
...
function doScoreItemFuzzySingle(label, description, path, query, preferLabelMatches, allowNonContiguousMatches) {
    ...
    if (preferLabelMatches || !description) {
        ...
        if (labelScore) {
            ...
            // beginning.
            const labelPrefixMatch = (0, filters_js_1.matchesPrefix)(query.normalized, label);
            let baseScore;
            if (labelPrefixMatch) {
    ...
    if (description) {
        ...
        if (!!path) {
            descriptionPrefix = `${description}${path_js_1.sep}`; // assume this is a file path
        }
...
function fallbackCompare(itemA, itemB, query, accessor) {
    ...
    // compare by label
    if (labelA !== labelB) {
        return (0, comparers_js_1.compareAnything)(labelA, labelB, query.normalized);
    }
    // compare by description
    if (descriptionA && descriptionB && descriptionA !== descriptionB) {
        return (0, comparers_js_1.compareAnything)(descriptionA, descriptionB, query.normalized);
    }
    // compare by path
    if (pathA && pathB && pathA !== pathB) {
        return (0, comparers_js_1.compareAnything)(pathA, pathB, query.normalized);
    }
    // equal
    return 0;
}
...
function prepareQuery(original) {
    ...
    const containsPathSeparator = pathNormalized.indexOf(path_js_1.sep) >= 0;
...
function normalizeQuery(original) {
    ...
    if (platform_js_1.isWindows) {
        pathNormalized = original.replace(/\//g, path_js_1.sep); // Help Windows users to search for paths when using slash
    }
    else {
        pathNormalized = original.replace(/\\/g, path_js_1.sep); // Help macOS/Linux users to search for paths when using backslash
    }
    // we remove quotes here because quotes are used for exact match search
    const normalized = (0, strings_js_1.stripWildcards)(pathNormalized).replace(/\s|"/g, '');
    return {
        pathNormalized,
        normalized,
