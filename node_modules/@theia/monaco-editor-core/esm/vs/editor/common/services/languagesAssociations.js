...
const glob_js_1 = require("../../../base/common/glob.js");
...
function toLanguageAssociationItem(association, userConfigured) {
    return {
        ...
        filepatternLowercase: association.filepattern ? (0, glob_js_1.parse)(association.filepattern.toLowerCase()) : undefined,
...
function getAssociations(resource, firstLine) {
    ...
    if (resource) {
        switch (resource.scheme) {
            case network_js_1.Schemas.file:
                path = resource.fsPath;
                break;
            case network_js_1.Schemas.data: {
                const metadata = resources_js_1.DataUri.parseMetaData(resource);
                path = metadata.get(resources_js_1.DataUri.META_DATA_LABEL);
                break;
            }
            case network_js_1.Schemas.vscodeNotebookCell:
    ...
    if (!path) {
        return [{ id: 'unknown', mime: mime_js_1.Mimes.unknown }];
    }
    path = path.toLowerCase();
    const filename = (0, path_js_1.basename)(path);
    // 1.) User configured mappings have highest priority
    const configuredLanguage = getAssociationByPath(path, filename, userRegisteredAssociations);
    if (configuredLanguage) {
        return [configuredLanguage, { id: modesRegistry_js_1.PLAINTEXT_LANGUAGE_ID, mime: mime_js_1.Mimes.text }];
    }
    // 2.) Registered mappings have middle priority
    const registeredLanguage = getAssociationByPath(path, filename, nonUserRegisteredAssociations);
    if (registeredLanguage) {
        return [registeredLanguage, { id: modesRegistry_js_1.PLAINTEXT_LANGUAGE_ID, mime: mime_js_1.Mimes.text }];
    }
    // 3.) Firstline has lowest priority
    if (firstLine) {
        const firstlineLanguage = getAssociationByFirstline(firstLine);
        if (firstlineLanguage) {
            return [firstlineLanguage, { id: modesRegistry_js_1.PLAINTEXT_LANGUAGE_ID, mime: mime_js_1.Mimes.text }];
        }
    }
    return [{ id: 'unknown', mime: mime_js_1.Mimes.unknown }];
}
...
function getAssociationByFirstline(firstLine) {
    if ((0, strings_js_1.startsWithUTF8BOM)(firstLine)) {
