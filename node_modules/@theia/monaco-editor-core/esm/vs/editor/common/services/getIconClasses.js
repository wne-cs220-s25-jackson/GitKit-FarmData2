...
exports.fileIconSelectorEscape = fileIconSelectorEscape;
...
function getIconClasses(modelService, languageService, resource, fileKind, icon) {
    if (themables_js_1.ThemeIcon.isThemeIcon(icon)) {
        return [`codicon-${icon.id}`, 'predefined-file-icon'];
    }
    if (uri_js_1.URI.isUri(icon)) {
        return [];
    }
    // we always set these base classes even if we do not have a path
    const classes = fileKind === files_js_1.FileKind.ROOT_FOLDER ? ['rootfolder-icon'] : fileKind === files_js_1.FileKind.FOLDER ? ['folder-icon'] : ['file-icon'];
    if (resource) {
        // Get the path and name of the resource. For data-URIs, we need to parse specially
        let name;
        if (resource.scheme === network_js_1.Schemas.data) {
            const metadata = resources_js_1.DataUri.parseMetaData(resource);
            name = metadata.get(resources_js_1.DataUri.META_DATA_LABEL);
        }
        else {
        ...
        // Root Folders
        if (fileKind === files_js_1.FileKind.ROOT_FOLDER) {
            classes.push(`${name}-root-name-folder-icon`);
        }
        // Folders
        else if (fileKind === files_js_1.FileKind.FOLDER) {
...
function detectLanguageId(modelService, languageService, resource) {
    ...
    if (resource.scheme === network_js_1.Schemas.data) {
        const metadata = resources_js_1.DataUri.parseMetaData(resource);
        const mime = metadata.get(resources_js_1.DataUri.META_DATA_MIME);
        if (mime) {
    ...
    if (languageId && languageId !== modesRegistry_js_1.PLAINTEXT_LANGUAGE_ID) {
        return languageId;
    }
