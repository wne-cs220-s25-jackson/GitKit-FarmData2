...
const sparseMultilineTokens_js_1 = require("../tokens/sparseMultilineTokens.js");
...
let SemanticTokensProviderStyling = class SemanticTokensProviderStyling {
    ...
    getMetadata(tokenTypeIndex, tokenModifierSet, languageId) {
        ...
        if (entry) {
            ...
            if (ENABLE_TRACE && this._logService.getLevel() === log_js_1.LogLevel.Trace) {
                this._logService.trace(`SemanticTokensProviderStyling [CACHED] ${tokenTypeIndex} / ${tokenModifierSet}: foreground ${encodedTokenAttributes_js_1.TokenMetadata.getForeground(metadata)}, fontStyle ${encodedTokenAttributes_js_1.TokenMetadata.getFontStyle(metadata).toString(2)}`);
            }
        }
        else {
            ...
            if (tokenType) {
                ...
                if (ENABLE_TRACE && modifierSet > 0 && this._logService.getLevel() === log_js_1.LogLevel.Trace) {
            ...
            if (ENABLE_TRACE && this._logService.getLevel() === log_js_1.LogLevel.Trace) {
                this._logService.trace(`SemanticTokensProviderStyling ${tokenTypeIndex} (${tokenType}) / ${tokenModifierSet} (${tokenModifiers.join(' ')}): foreground ${encodedTokenAttributes_js_1.TokenMetadata.getForeground(metadata)}, fontStyle ${encodedTokenAttributes_js_1.TokenMetadata.getFontStyle(metadata).toString(2)}`);
            }
        }
        return metadata;
    }
...
function toMultilineTokens2(tokens, styling, languageId) {
    ...
    while (tokenIndex < tokenCount) {
        ...
        const tokens = sparseMultilineTokens_js_1.SparseMultilineTokens.create(areaLine, destData);
        result.push(tokens);
    }
    return result;
}
