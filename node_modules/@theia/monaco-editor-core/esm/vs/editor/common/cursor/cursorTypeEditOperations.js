...
const replaceCommand_js_1 = require("../commands/replaceCommand.js");
...
const wordCharacterClassifier_js_1 = require("../core/wordCharacterClassifier.js");
const range_js_1 = require("../core/range.js");
const position_js_1 = require("../core/position.js");
const languageConfiguration_js_1 = require("../languages/languageConfiguration.js");
const languageConfigurationRegistry_js_1 = require("../languages/languageConfigurationRegistry.js");
const supports_js_1 = require("../languages/supports.js");
const autoIndent_js_1 = require("../languages/autoIndent.js");
const enterAction_js_1 = require("../languages/enterAction.js");
class AutoIndentOperation {
    ...
    static _findActualIndentationForSelection(config, model, selection, ch) {
        const actualIndentation = (0, autoIndent_js_1.getIndentActionForType)(config, model, selection, ch, {
        ...
        const currentIndentation = (0, languageConfigurationRegistry_js_1.getIndentationAtPosition)(model, selection.startLineNumber, selection.startColumn);
    ...
    static _getIndentationAndAutoClosingPairEdits(config, model, indentationForSelections, ch, autoClosingPairClose) {
        ...
        return new cursorCommon_js_1.EditOperationResult(4 /* EditOperationType.TypingOther */, commands, editOptions);
    }
    static _getEditFromIndentationAndSelection(config, model, indentation, selection, ch, includeChInEdit = true) {
        ...
        const range = new range_js_1.Range(startLineNumber, 1, selection.endLineNumber, selection.endColumn);
        return { range, text };
    }
}
...
class AutoClosingOvertypeOperation {
    ...
    static _runAutoClosingOvertype(prevEditOperationType, selections, ch) {
        ...
        for (let i = 0, len = selections.length; i < len; i++) {
            ...
            const typeSelection = new range_js_1.Range(position.lineNumber, position.column, position.lineNumber, position.column + 1);
            commands[i] = new replaceCommand_js_1.ReplaceCommand(typeSelection, ch);
        }
...
class AutoClosingOvertypeWithInterceptorsOperation {
    static getEdits(config, model, selections, autoClosedCharacters, ch) {
        if (isAutoClosingOvertype(config, model, selections, autoClosedCharacters, ch)) {
            ...
            const commands = selections.map(s => new replaceCommand_js_1.ReplaceCommand(new range_js_1.Range(s.positionLineNumber, s.positionColumn, s.positionLineNumber, s.positionColumn + 1), '', false));
            return new cursorCommon_js_1.EditOperationResult(4 /* EditOperationType.TypingOther */, commands, {
...
class AutoClosingOpenCharTypeOperation {
    ...
    static getAutoClosingPairClose(config, model, selections, ch, chIsAlreadyTyped) {
        ...
        const pair = this._findAutoClosingPairOpen(config, model, positions.map(p => new position_js_1.Position(p.lineNumber, p.beforeColumn)), ch);
        if (!pair) {
            return null;
        }
        let autoCloseConfig;
        let shouldAutoCloseBefore;
        const chIsQuote = (0, cursorCommon_js_1.isQuote)(ch);
        if (chIsQuote) {
        ...
        for (const position of positions) {
            ...
            if (pair.open.length === 1 && (ch === '\'' || ch === '"') && autoCloseConfig !== 'always') {
                const wordSeparators = (0, wordCharacterClassifier_js_1.getMapForWordSeparators)(config.wordSeparators, []);
            ...
            const scopedLineTokens = (0, supports_js_1.createScopedLineTokens)(lineTokens, beforeColumn - 1);
    ...
    static _findAutoClosingPairOpen(config, model, positions, ch) {
        ...
        for (const candidate of candidates) {
            if (result === null || candidate.open.length > result.open.length) {
                ...
                for (const position of positions) {
                    const relevantText = model.getValueInRange(new range_js_1.Range(position.lineNumber, position.column - candidate.open.length + 1, position.lineNumber, position.column));
...
class CompositionEndOvertypeOperation {
    static getEdits(config, compositions) {
        ...
        const commands = compositions.map(composition => new replaceCommand_js_1.ReplaceOvertypeCommandOnCompositionEnd(composition.insertedTextRange));
        return new cursorCommon_js_1.EditOperationResult(4 /* EditOperationType.TypingOther */, commands, {
            shouldPushStackElementBefore: true,
            shouldPushStackElementAfter: false
        });
...
exports.CompositionEndOvertypeOperation = CompositionEndOvertypeOperation;
class SurroundSelectionOperation {
    ...
    static _runSurroundSelectionType(config, selections, ch) {
        ...
        for (let i = 0, len = selections.length; i < len; i++) {
            ...
            commands[i] = new surroundSelectionCommand_js_1.SurroundSelectionCommand(selection, ch, closeCharacter);
        }
        return new cursorCommon_js_1.EditOperationResult(0 /* EditOperationType.Other */, commands, {
            shouldPushStackElementBefore: true,
    ...
    static _isSurroundSelectionType(config, model, selections, ch) {
        ...
        for (const selection of selections) {
            ...
            if (isTypingAQuoteCharacter && selection.startLineNumber === selection.endLineNumber && selection.startColumn + 1 === selection.endColumn) {
                ...
                if ((0, cursorCommon_js_1.isQuote)(selectionText)) {
    ...
    static _typeInterceptorElectricChar(prevEditOperationType, config, model, selection, ch) {
        ...
        if (electricAction.matchOpenBracket) {
            ...
            if (match) {
                ...
                const typeSelection = new range_js_1.Range(position.lineNumber, 1, position.lineNumber, position.column);
                const command = new replaceCommand_js_1.ReplaceCommand(typeSelection, typeText);
                return new cursorCommon_js_1.EditOperationResult(getTypingOperation(typeText, prevEditOperationType), [command], {
...
class SimpleCharacterTypeOperation {
    static getEdits(config, prevEditOperationType, selections, ch, isDoingComposition) {
        ...
        const commands = [];
        for (let i = 0, len = selections.length; i < len; i++) {
            const ChosenReplaceCommand = config.inputMode === 'overtype' && !isDoingComposition ? replaceCommand_js_1.ReplaceOvertypeCommand : replaceCommand_js_1.ReplaceCommand;
            commands[i] = new ChosenReplaceCommand(selections[i], ch);
        }
        const opType = getTypingOperation(ch, prevEditOperationType);
        return new cursorCommon_js_1.EditOperationResult(opType, commands, {
...
class EnterOperation {
    ...
    static _enter(config, model, keepPosition, range) {
        ...
        const r = (0, enterAction_js_1.getEnterAction)(config.autoIndent, model, range, config.languageConfigurationService);
        if (r) {
            if (r.indentAction === languageConfiguration_js_1.IndentAction.None) {
                // Nothing special
                return typeCommand(range, '\n' + config.normalizeIndentation(r.indentation + r.appendText), keepPosition);
            }
            else if (r.indentAction === languageConfiguration_js_1.IndentAction.Indent) {
                // Indent once
                return typeCommand(range, '\n' + config.normalizeIndentation(r.indentation + r.appendText), keepPosition);
            }
            else if (r.indentAction === languageConfiguration_js_1.IndentAction.IndentOutdent) {
                // Ultra special
                const normalIndent = config.normalizeIndentation(r.indentation);
                const increasedIndent = config.normalizeIndentation(r.indentation + r.appendText);
                const typeText = '\n' + increasedIndent + '\n' + normalIndent;
                if (keepPosition) {
                    return new replaceCommand_js_1.ReplaceCommandWithoutChangingPosition(range, typeText, true);
                }
                else {
                    return new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(range, typeText, -1, increasedIndent.length - normalIndent.length, true);
                }
            }
            else if (r.indentAction === languageConfiguration_js_1.IndentAction.Outdent) {
        ...
        const indentation = strings.getLeadingWhitespace(lineText).substring(0, range.startColumn - 1);
        if (config.autoIndent >= 4 /* EditorAutoIndentStrategy.Full */) {
            const ir = (0, autoIndent_js_1.getIndentForEnter)(config.autoIndent, model, range, {
            ...
            if (ir) {
                ...
                if (keepPosition) {
                    return new replaceCommand_js_1.ReplaceCommandWithoutChangingPosition(range, '\n' + config.normalizeIndentation(ir.afterEnter), true);
                }
                else {
                    let offset = 0;
                    ...
                    return new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(range, '\n' + config.normalizeIndentation(ir.afterEnter), 0, offset, true);
                }
            }
        }
        return typeCommand(range, '\n' + config.normalizeIndentation(indentation), keepPosition);
    ...
    static lineInsertBefore(config, model, selections) {
        ...
        for (let i = 0, len = selections.length; i < len; i++) {
            ...
            if (lineNumber === 1) {
                commands[i] = new replaceCommand_js_1.ReplaceCommandWithoutChangingPosition(new range_js_1.Range(1, 1, 1, 1), '\n');
            }
            else {
                lineNumber--;
                const column = model.getLineMaxColumn(lineNumber);
                commands[i] = this._enter(config, model, false, new range_js_1.Range(lineNumber, column, lineNumber, column));
            }
        }
        return commands;
    }
...
class PasteOperation {
    static getEdits(config, model, selections, text, pasteOnNewLine, multicursorText) {
        ...
        if (distributedPaste) {
            selections = selections.sort(range_js_1.Range.compareRangesUsingStarts);
    ...
    static _distributedPaste(config, model, selections, text) {
        ...
        for (let i = 0, len = selections.length; i < len; i++) {
            ...
            const ChosenReplaceCommand = shouldOvertypeOnPaste ? replaceCommand_js_1.ReplaceOvertypeCommand : replaceCommand_js_1.ReplaceCommand;
    ...
    static _simplePaste(config, model, selections, text, pasteOnNewLine) {
        ...
        for (let i = 0, len = selections.length; i < len; i++) {
            ...
            if (pasteOnNewLine) {
                ...
                const typeSelection = new range_js_1.Range(position.lineNumber, 1, position.lineNumber, 1);
                commands[i] = new replaceCommand_js_1.ReplaceCommandThatPreservesSelection(typeSelection, text, selection, true);
            }
            else {
                const shouldOvertypeOnPaste = config.overtypeOnPaste && config.inputMode === 'overtype';
                const ChosenReplaceCommand = shouldOvertypeOnPaste ? replaceCommand_js_1.ReplaceOvertypeCommand : replaceCommand_js_1.ReplaceCommand;
...
class CompositionOperation {
    ...
    static _compositionType(model, selection, text, replacePrevCharCnt, replaceNextCharCnt, positionDelta) {
        ...
        const range = new range_js_1.Range(pos.lineNumber, startColumn, pos.lineNumber, endColumn);
        return new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(range, text, 0, positionDelta);
    }
}
...
class TypeWithoutInterceptorsOperation {
    static getEdits(prevEditOperationType, selections, str) {
        const commands = [];
        for (let i = 0, len = selections.length; i < len; i++) {
            commands[i] = new replaceCommand_js_1.ReplaceCommand(selections[i], str);
        }
...
class TabOperation {
            ...
            if (selection.isEmpty()) {
                ...
                if (/^\s*$/.test(lineText) && model.tokenization.isCheapToTokenize(selection.startLineNumber)) {
                    ...
                    if (!lineText.startsWith(possibleTypeText)) {
                        commands[i] = new replaceCommand_js_1.ReplaceCommand(new range_js_1.Range(selection.startLineNumber, 1, selection.startLineNumber, lineText.length + 1), possibleTypeText, true);
                        continue;
                    }
                }
                ...
                commands[i] = new shiftCommand_js_1.ShiftCommand(selection, {
                    isUnshift: false,
    ...
    static _goodIndentForLine(config, model, lineNumber) {
        let action = null;
        let indentation = '';
        const expectedIndentAction = (0, autoIndent_js_1.getInheritIndentForLine)(config.autoIndent, model, lineNumber, false, config.languageConfigurationService);
        if (expectedIndentAction) {
            ...
            const expectedEnterAction = (0, enterAction_js_1.getEnterAction)(config.autoIndent, model, new range_js_1.Range(lastLineNumber, maxColumn, lastLineNumber, maxColumn), config.languageConfigurationService);
            if (expectedEnterAction) {
                indentation = expectedEnterAction.indentation + expectedEnterAction.appendText;
            }
        }
        if (action) {
            if (action === languageConfiguration_js_1.IndentAction.Indent) {
                indentation = shiftIndent(config, indentation);
            }
            if (action === languageConfiguration_js_1.IndentAction.Outdent) {
    ...
    static _replaceJumpToNextIndent(config, model, selection, insertsAutoWhitespace) {
        ...
        return new replaceCommand_js_1.ReplaceCommand(selection, typeText, insertsAutoWhitespace);
    }
}
exports.TabOperation = TabOperation;
class BaseTypeWithAutoClosingCommand extends replaceCommand_js_1.ReplaceCommandWithOffsetCursorState {
    ...
    _computeCursorStateWithRange(model, range, helper) {
        this.closeCharacterRange = new range_js_1.Range(range.startLineNumber, range.endColumn - this._closeCharacter.length, range.endLineNumber, range.endColumn);
        this.enclosingRange = new range_js_1.Range(range.startLineNumber, range.endColumn - this._openCharacter.length - this._closeCharacter.length, range.endLineNumber, range.endColumn);
...
function typeCommand(range, text, keepPosition) {
    if (keepPosition) {
        return new replaceCommand_js_1.ReplaceCommandWithoutChangingPosition(range, text, true);
    }
    else {
        return new replaceCommand_js_1.ReplaceCommand(range, text, true);
    }
}
function shiftIndent(config, indentation, count) {
    count = count || 1;
    return shiftCommand_js_1.ShiftCommand.shiftIndent(indentation, indentation.length + count, config.tabSize, config.indentSize, config.insertSpaces);
}
function unshiftIndent(config, indentation, count) {
    count = count || 1;
    return shiftCommand_js_1.ShiftCommand.unshiftIndent(indentation, indentation.length + count, config.tabSize, config.indentSize, config.insertSpaces);
}
function shouldSurroundChar(config, ch) {
    if ((0, cursorCommon_js_1.isQuote)(ch)) {
