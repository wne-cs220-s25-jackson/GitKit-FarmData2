...
class WordOperations {
    ...
    static moveWordLeft(wordSeparators, model, position, wordNavigationType, hasMulticursor) {
        ...
        let prevWordOnLine = WordOperations._findPreviousWordOnLine(wordSeparators, model, new position_js_1.Position(lineNumber, column));
        if (wordNavigationType === 0 /* WordNavigationType.WordStart */) {
            return new position_js_1.Position(lineNumber, prevWordOnLine ? prevWordOnLine.start + 1 : 1);
        }
        if (wordNavigationType === 1 /* WordNavigationType.WordStartFast */) {
            if (!hasMulticursor // avoid having multiple cursors stop at different locations when doing word start
                ...
                prevWordOnLine = WordOperations._findPreviousWordOnLine(wordSeparators, model, new position_js_1.Position(lineNumber, prevWordOnLine.start + 1));
            }
            return new position_js_1.Position(lineNumber, prevWordOnLine ? prevWordOnLine.start + 1 : 1);
        }
        ...
        return new position_js_1.Position(lineNumber, prevWordOnLine ? prevWordOnLine.end + 1 : 1);
    }
    static _moveWordPartLeft(model, position) {
        const lineNumber = position.lineNumber;
        const maxColumn = model.getLineMaxColumn(lineNumber);
        if (position.column === 1) {
            return (lineNumber > 1 ? new position_js_1.Position(lineNumber - 1, model.getLineMaxColumn(lineNumber - 1)) : position);
        }
    ...
    static moveWordRight(wordSeparators, model, position, wordNavigationType) {
        ...
        let nextWordOnLine = WordOperations._findNextWordOnLine(wordSeparators, model, new position_js_1.Position(lineNumber, column));
        if (wordNavigationType === 2 /* WordNavigationType.WordEnd */) {
            if (nextWordOnLine && nextWordOnLine.wordType === 2 /* WordType.Separator */) {
                if (nextWordOnLine.end - nextWordOnLine.start === 1 && nextWordOnLine.nextCharClass === 0 /* WordCharacterClass.Regular */) {
                    // Skip over a word made up of one single separator and followed by a regular character
                    nextWordOnLine = WordOperations._findNextWordOnLine(wordSeparators, model, new position_js_1.Position(lineNumber, nextWordOnLine.end + 1));
                }
            }
            if (nextWordOnLine) {
    ...
    static _moveWordPartRight(model, position) {
        ...
        if (position.column === maxColumn) {
            return (lineNumber < model.getLineCount() ? new position_js_1.Position(lineNumber + 1, 1) : position);
        }
        const lineContent = model.getLineContent(lineNumber);
    ...
    static _deleteWordLeftWhitespace(model, position) {
        ...
        if (lastNonWhitespace + 1 < startIndex) {
            return new range_js_1.Range(position.lineNumber, lastNonWhitespace + 2, position.lineNumber, position.column);
        }
        return null;
    }
    static deleteWordLeft(ctx, wordNavigationType) {
        ...
        if (cursorDeleteOperations_js_1.DeleteOperations.isAutoClosingPairDelete(ctx.autoClosingDelete, ctx.autoClosingBrackets, ctx.autoClosingQuotes, ctx.autoClosingPairs.autoClosingPairsOpenByEnd, ctx.model, [ctx.selection], ctx.autoClosedCharacters)) {
            const position = ctx.selection.getPosition();
            return new range_js_1.Range(position.lineNumber, position.column - 1, position.lineNumber, position.column + 1);
        }
        const position = new position_js_1.Position(selection.positionLineNumber, selection.positionColumn);
        let lineNumber = position.lineNumber;
        let column = position.column;
        ...
        return new range_js_1.Range(lineNumber, column, position.lineNumber, position.column);
    }
    ...
    static _deleteInsideWordWhitespace(model, position) {
        ...
        return new range_js_1.Range(position.lineNumber, leftIndex + 1, position.lineNumber, rightIndex + 2);
    }
    static _deleteInsideWordDetermineDeleteRange(wordSeparators, model, position) {
        ...
        if (lineLength === 0) {
            ...
            if (position.lineNumber > 1) {
                return new range_js_1.Range(position.lineNumber - 1, model.getLineMaxColumn(position.lineNumber - 1), position.lineNumber, 1);
            }
            else {
                if (position.lineNumber < model.getLineCount()) {
                    return new range_js_1.Range(position.lineNumber, 1, position.lineNumber + 1, 1);
                }
                else {
                    // empty model
                    return new range_js_1.Range(position.lineNumber, 1, position.lineNumber, 1);
                }
            }
        }
        ...
        const createRangeWithPosition = (startColumn, endColumn) => {
            ...
            return new range_js_1.Range(position.lineNumber, startColumn, position.lineNumber, endColumn);
        };
    ...
    static _deleteWordPartLeft(model, selection) {
        ...
        return new range_js_1.Range(pos.lineNumber, pos.column, toPosition.lineNumber, toPosition.column);
    }
    ...
    static _deleteWordRightWhitespace(model, position) {
        ...
        if (startIndex + 1 < firstNonWhitespace) {
            // bingo
            return new range_js_1.Range(position.lineNumber, position.column, position.lineNumber, firstNonWhitespace + 1);
        }
        return null;
    }
        ...
        if (wordNavigationType === 2 /* WordNavigationType.WordEnd */) {
            if (nextWordOnLine) {
                ...
                if (column < maxColumn || lineNumber === lineCount) {
                    ...
                    lineNumber++;
                    nextWordOnLine = WordOperations._findNextWordOnLine(wordSeparators, model, new position_js_1.Position(lineNumber, 1));
                    if (nextWordOnLine) {
                        column = nextWordOnLine.start + 1;
                    }
                    else {
    ...
    static _createWordAtPosition(model, lineNumber, word) {
        const range = new range_js_1.Range(lineNumber, word.start + 1, lineNumber, word.end + 1);
        return {
    ...
    static getWordAtPosition(model, _wordSeparators, _intlSegmenterLocales, position) {
        const wordSeparators = (0, wordCharacterClassifier_js_1.getMapForWordSeparators)(_wordSeparators, _intlSegmenterLocales);
    ...
    static word(config, model, cursor, inSelectionMode, position) {
        const wordSeparators = (0, wordCharacterClassifier_js_1.getMapForWordSeparators)(config.wordSeparators, config.wordSegmenterLocales);
        const prevWord = WordOperations._findPreviousWordOnLine(wordSeparators, model, position);
        const nextWord = WordOperations._findNextWordOnLine(wordSeparators, model, position);
        if (!inSelectionMode) {
            ...
            return new cursorCommon_js_1.SingleCursorState(new range_js_1.Range(position.lineNumber, startColumn, position.lineNumber, endColumn), 1 /* SelectionStartKind.Word */, 0, new position_js_1.Position(position.lineNumber, endColumn), 0);
        }
        let startColumn;
        let endColumn;
        if (prevWord && prevWord.wordType === 1 /* WordType.Regular */ && prevWord.start < position.column - 1 && position.column - 1 < prevWord.end) {
        ...
        if (cursor.selectionStart.containsPosition(position)) {
            ...
            column = startColumn;
            const possiblePosition = new position_js_1.Position(lineNumber, column);
...
class WordPartOperations extends WordOperations {
    static deleteWordPartLeft(ctx) {
        ...
        candidates.sort(range_js_1.Range.compareRangesUsingEnds);
        return candidates[2];
    }
    static deleteWordPartRight(ctx) {
        ...
        candidates.sort(range_js_1.Range.compareRangesUsingStarts);
        return candidates[0];
    }
    static moveWordPartLeft(wordSeparators, model, position, hasMulticursor) {
        ...
        candidates.sort(position_js_1.Position.compare);
        return candidates[2];
    }
