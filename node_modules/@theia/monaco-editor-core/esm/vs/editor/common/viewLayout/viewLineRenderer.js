...
const stringBuilder_js_1 = require("../core/stringBuilder.js");
const lineDecorations_js_1 = require("./lineDecorations.js");
const linePart_js_1 = require("./linePart.js");
class LineRange {
...
class RenderLineInput {
    constructor(useMonospaceOptimizations, canUseHalfwidthRightwardsArrow, lineContent, continuesWithWrappedLine, isBasicASCII, containsRTL, fauxIndentLength, lineTokens, lineDecorations, tabSize, startVisibleColumn, spaceWidth, middotWidth, wsmiddotWidth, stopRenderingLineAfter, renderWhitespace, renderControlCharacters, fontLigatures, selectionsOnLine) {
        ...
        this.lineDecorations = lineDecorations.sort(lineDecorations_js_1.LineDecoration.compare);
    ...
    equals(other) {
        return (this.useMonospaceOptimizations === other.useMonospaceOptimizations
            ...
            && lineDecorations_js_1.LineDecoration.equalsArr(this.lineDecorations, other.lineDecorations)
...
function transformAndRemoveOverflowing(lineContent, lineContainsRTL, tokens, fauxIndentLength, len) {
    ...
    if (fauxIndentLength > 0) {
        result[resultLen++] = new linePart_js_1.LinePart(fauxIndentLength, '', 0, false);
    }
    ...
    for (let tokenIndex = 0, tokensLen = tokens.getCount(); tokenIndex < tokensLen; tokenIndex++) {
        ...
        if (endIndex >= len) {
            ...
            result[resultLen++] = new linePart_js_1.LinePart(len, type, 0, tokenContainsRTL);
            break;
        }
        const tokenContainsRTL = (lineContainsRTL ? strings.containsRTL(lineContent.substring(startOffset, endIndex)) : false);
        result[resultLen++] = new linePart_js_1.LinePart(endIndex, type, 0, tokenContainsRTL);
            ...
            if (lastTokenEndIndex + 50 /* Constants.LongToken */ < tokenEndIndex) {
                ...
                for (let j = lastTokenEndIndex; j < tokenEndIndex; j++) {
                    ...
                    if (lastSpaceOffset !== -1 && j - currTokenStart >= 50 /* Constants.LongToken */) {
                        ...
                        result[resultLen++] = new linePart_js_1.LinePart(lastSpaceOffset + 1, tokenType, tokenMetadata, tokenContainsRTL);
                        currTokenStart = lastSpaceOffset + 1;
                        lastSpaceOffset = -1;
                    }
                }
                if (currTokenStart !== tokenEndIndex) {
                    result[resultLen++] = new linePart_js_1.LinePart(tokenEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                }
            }
            else {
        ...
        for (let i = 0, len = tokens.length; i < len; i++) {
            ...
            if (diff > 50 /* Constants.LongToken */) {
                ...
                for (let j = 1; j < piecesCount; j++) {
                    ...
                    result[resultLen++] = new linePart_js_1.LinePart(pieceEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                }
                result[resultLen++] = new linePart_js_1.LinePart(tokenEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
            }
            else {
...
function extractControlCharacters(lineContent, tokens) {
    const result = [];
    let lastLinePart = new linePart_js_1.LinePart(0, '', 0, false);
    let charOffset = 0;
        ...
        for (; charOffset < tokenEndIndex; charOffset++) {
            ...
            if (isControlCharacter(charCode)) {
                if (charOffset > lastLinePart.endIndex) {
                    ...
                    lastLinePart = new linePart_js_1.LinePart(charOffset, token.type, token.metadata, token.containsRTL);
                    result.push(lastLinePart);
                }
                lastLinePart = new linePart_js_1.LinePart(charOffset + 1, 'mtkcontrol', token.metadata, false);
                result.push(lastLinePart);
            }
        }
        if (charOffset > lastLinePart.endIndex) {
            // emit previous part if it has text
            lastLinePart = new linePart_js_1.LinePart(tokenEndIndex, token.type, token.metadata, token.containsRTL);
            result.push(lastLinePart);
        }
    }
    return result;
...
function _applyRenderWhitespace(input, lineContent, len, tokens) {
    ...
    for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
        ...
        if (wasInWhitespace) {
            ...
            if (!isInWhitespace || (!useMonospaceOptimizations && tmpIndent >= tabSize)) {
                ...
                if (generateLinePartForEachWhitespace) {
                    ...
                    for (let i = lastEndIndex + 1; i <= charIndex; i++) {
                        result[resultLen++] = new linePart_js_1.LinePart(i, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
                    }
                }
                else {
                    result[resultLen++] = new linePart_js_1.LinePart(charIndex, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
                }
            ...
            if (charIndex === tokenEndIndex || (isInWhitespace && charIndex > fauxIndentLength)) {
                result[resultLen++] = new linePart_js_1.LinePart(charIndex, tokenType, 0, tokenContainsRTL);
                tmpIndent = tmpIndent % tabSize;
            }
        }
    ...
    if (generateWhitespace) {
        if (generateLinePartForEachWhitespace) {
            ...
            result[resultLen++] = new linePart_js_1.LinePart(len, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
        }
    }
    else {
        result[resultLen++] = new linePart_js_1.LinePart(len, tokenType, 0, tokenContainsRTL);
    }
    return result;
}
/**
...
function _applyInlineDecorations(lineContent, len, tokens, _lineDecorations) {
    _lineDecorations.sort(lineDecorations_js_1.LineDecoration.compare);
    const lineDecorations = lineDecorations_js_1.LineDecorationsNormalizer.normalize(lineContent, _lineDecorations);
    ...
    for (let tokenIndex = 0, len = tokens.length; tokenIndex < len; tokenIndex++) {
        ...
        while (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset < tokenEndIndex) {
            ...
            if (lineDecoration.startOffset > lastResultEndIndex) {
                ...
                result[resultLen++] = new linePart_js_1.LinePart(lastResultEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
            }
            if (lineDecoration.endOffset + 1 <= tokenEndIndex) {
                // This line decoration ends before this token ends
                lastResultEndIndex = lineDecoration.endOffset + 1;
                result[resultLen++] = new linePart_js_1.LinePart(lastResultEndIndex, tokenType + ' ' + lineDecoration.className, tokenMetadata | lineDecoration.metadata, tokenContainsRTL);
                lineDecorationIndex++;
            }
            else {
    ...
    if (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset === lastTokenEndIndex) {
        while (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset === lastTokenEndIndex) {
            const lineDecoration = lineDecorations[lineDecorationIndex];
            result[resultLen++] = new linePart_js_1.LinePart(lastResultEndIndex, lineDecoration.className, lineDecoration.metadata, false);
            lineDecorationIndex++;
        }
    }
    return result;
