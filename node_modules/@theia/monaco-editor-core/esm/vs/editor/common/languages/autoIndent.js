...
function getInheritIndentForLine(autoIndent, model, lineNumber, honorIntentialIndent = true, languageConfigurationService) {
    ...
    const processedIndentRulesSupport = new indentationLineProcessor_js_1.ProcessedIndentRulesSupport(model, indentRulesSupport, languageConfigurationService);
    if (lineNumber <= 1) {
        return {
            indentation: '',
            action: null
    ...
    if (processedIndentRulesSupport.shouldIncrease(precedingUnIgnoredLine) || processedIndentRulesSupport.shouldIndentNextLine(precedingUnIgnoredLine)) {
        ...
        return {
            ...
            action: languageConfiguration_js_1.IndentAction.Indent,
...
function getGoodIndentForLine(autoIndent, virtualModel, languageId, lineNumber, indentConverter, languageConfigurationService) {
    ...
    const processedIndentRulesSupport = new indentationLineProcessor_js_1.ProcessedIndentRulesSupport(virtualModel, indentRulesSupport, languageConfigurationService);
    ...
    if (indent) {
        ...
        if (inheritLine !== undefined) {
            ...
            if (shouldApplyEnterRules) {
                ...
                if (enterResult) {
                    ...
                    if ((enterResult.indentAction === languageConfiguration_js_1.IndentAction.Indent) ||
                        (enterResult.indentAction === languageConfiguration_js_1.IndentAction.IndentOutdent)) {
                        indentation = indentConverter.shiftIndent(indentation);
                    }
                    else if (enterResult.indentAction === languageConfiguration_js_1.IndentAction.Outdent) {
        ...
        if (processedIndentRulesSupport.shouldDecrease(lineNumber)) {
            if (indent.action === languageConfiguration_js_1.IndentAction.Indent) {
...
function getIndentForEnter(autoIndent, model, range, indentConverter, languageConfigurationService) {
    ...
    const languageIsDifferentFromLineStart = (0, indentationLineProcessor_js_1.isLanguageDifferentFromLineStart)(model, range.getStartPosition());
    ...
    if (afterEnterAction.action === languageConfiguration_js_1.IndentAction.Indent) {
...
function getIndentActionForType(cursorConfig, model, range, ch, indentConverter, languageConfigurationService) {
    ...
    if (!indentRulesSupport.shouldDecrease(textAroundRange) && indentRulesSupport.shouldDecrease(textAroundRangeWithCharacter)) {
        ...
        if (r.action !== languageConfiguration_js_1.IndentAction.Indent) {
            indentation = indentConverter.unshiftIndent(indentation);
        }
        return indentation;
    }
