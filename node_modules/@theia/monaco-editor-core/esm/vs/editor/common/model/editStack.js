...
const buffer = require("../../../base/common/buffer.js");
...
class SingleModelEditStackData {
    ...
    append(model, textChanges, afterEOL, afterVersionId, afterCursorState) {
        if (textChanges.length > 0) {
            this.changes = (0, textChange_js_1.compressConsecutiveTextChanges)(this.changes, textChanges);
        }
        this.afterEOL = afterEOL;
        this.afterVersionId = afterVersionId;
        this.afterCursorState = afterCursorState;
    ...
    static _readSelections(b, offset, dest) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            offset += 4;
            dest.push(new selection_js_1.Selection(selectionStartLineNumber, selectionStartColumn, positionLineNumber, positionColumn));
        }
        return offset;
    }
    serialize() {
    ...
    static deserialize(source) {
        ...
        for (let i = 0; i < changeCount; i++) {
            offset = textChange_js_1.TextChange.read(b, offset, changes);
        }
...
class SingleModelEditStackElement {
    ...
    get resource() {
        if (uri_js_1.URI.isUri(this.model)) {
            return this.model;
        }
    ...
    matchesResource(resource) {
        const uri = (uri_js_1.URI.isUri(this.model) ? this.model : this.model.uri);
...
class MultiModelEditStackElement {
    ...
    getMissingModels() {
        const result = [];
        for (const editStackElement of this._editStackElementsArr) {
            if (uri_js_1.URI.isUri(editStackElement.model)) {
    ...
    setModel(model) {
        const key = uriGetComparisonKey(uri_js_1.URI.isUri(model) ? model : model.uri);
    ...
    toString() {
        const result = [];
        for (const editStackElement of this._editStackElementsArr) {
            result.push(`${(0, resources_js_1.basename)(editStackElement.resource)}: ${editStackElement}`);
        }
        return `{${result.join(', ')}}`;
    }
}
