...
class PairAstNode extends BaseAstNode {
    static create(openingBracket, child, closingBracket) {
        ...
        if (child) {
            length = (0, length_js_1.lengthAdd)(length, child.length);
        }
        if (closingBracket) {
            length = (0, length_js_1.lengthAdd)(length, closingBracket.length);
        }
        return new PairAstNode(length, openingBracket, child, closingBracket, child ? child.missingOpeningBracketIds : smallImmutableSet_js_1.SmallImmutableSet.getEmpty());
    }
    get kind() {
    ...
    */
    ...
    computeMinIndentation(offset, textModel) {
        return this.child ? this.child.computeMinIndentation((0, length_js_1.lengthAdd)(offset, this.openingBracket.length), textModel) : Number.MAX_SAFE_INTEGER;
    }
}
...
class ListAstNode extends BaseAstNode {
    ...
    */
    static create23(item1, item2, item3, immutable = false) {
        ...
        length = (0, length_js_1.lengthAdd)(length, item2.length);
        missingBracketIds = missingBracketIds.merge(item2.missingOpeningBracketIds);
        if (item3) {
            if (item1.listHeight !== item3.listHeight) {
                throw new Error('Invalid list heights');
            }
            length = (0, length_js_1.lengthAdd)(length, item3.length);
    ...
    static create(items, immutable = false) {
        if (items.length === 0) {
            ...
            for (let i = 1; i < items.length; i++) {
                length = (0, length_js_1.lengthAdd)(length, items[i].length);
    ...
    static getEmpty() {
        return new ImmutableArrayListAstNode(length_js_1.lengthZero, 0, [], smallImmutableSet_js_1.SmallImmutableSet.getEmpty());
    }
    get kind() {
    ...
    computeMinIndentation(offset, textModel) {
        ...
        for (let i = 0; i < this.childrenLength; i++) {
            ...
            if (child) {
                ...
                childOffset = (0, length_js_1.lengthAdd)(childOffset, child.length);
            }
        }
...
class TextAstNode extends ImmutableLeafAstNode {
    ...
    get missingOpeningBracketIds() {
        return smallImmutableSet_js_1.SmallImmutableSet.getEmpty();
    }
    canBeReused(_openedBracketIds) {
        return true;
    }
    computeMinIndentation(offset, textModel) {
        const start = (0, length_js_1.lengthToObj)(offset);
        // Text ast nodes don't have partial indentation (ensured by the tokenizer).
        // Thus, if this text node does not start at column 0, the first line cannot have any indentation at all.
        const startLineNumber = (start.columnCount === 0 ? start.lineCount : start.lineCount + 1) + 1;
        const endLineNumber = (0, length_js_1.lengthGetLineCount)((0, length_js_1.lengthAdd)(offset, this.length)) + 1;
        ...
        for (let lineNumber = startLineNumber; lineNumber <= endLineNumber; lineNumber++) {
            ...
            const visibleColumn = cursorColumns_js_1.CursorColumns.visibleColumnFromColumn(lineContent, firstNonWsColumn, textModel.getOptions().tabSize);
