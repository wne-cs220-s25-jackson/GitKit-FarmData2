...
const dynamicViewOverlay_js_1 = require("../../view/dynamicViewOverlay.js");
...
class AbstractLineHighlightOverlay extends dynamicViewOverlay_js_1.DynamicViewOverlay {
    constructor(context) {
        super();
    ...
    prepareRender(ctx) {
        ...
        if (this._wordWrap) {
            ...
            for (const cursorLineNumber of this._cursorLineNumbers) {
                ...
                const modelLineNumber = coordinatesConverter.convertViewPositionToModelPosition(new position_js_1.Position(cursorLineNumber, 1)).lineNumber;
                const firstViewLineNumber = coordinatesConverter.convertModelPositionToViewPosition(new position_js_1.Position(modelLineNumber, 1)).lineNumber;
                const lastViewLineNumber = coordinatesConverter.convertModelPositionToViewPosition(new position_js_1.Position(modelLineNumber, this._context.viewModel.model.getLineMaxColumn(modelLineNumber))).lineNumber;
...
(0, themeService_js_1.registerThemingParticipant)((theme, collector) => {
    const lineHighlight = theme.getColor(editorColorRegistry_js_1.editorLineHighlight);
    if (lineHighlight) {
        collector.addRule(`.monaco-editor .view-overlays .current-line { background-color: ${lineHighlight}; }`);
        collector.addRule(`.monaco-editor .margin-view-overlays .current-line-margin { background-color: ${lineHighlight}; border: none; }`);
    }
    if (!lineHighlight || lineHighlight.isTransparent() || theme.defines(editorColorRegistry_js_1.editorLineHighlightBorder)) {
        const lineHighlightBorder = theme.getColor(editorColorRegistry_js_1.editorLineHighlightBorder);
