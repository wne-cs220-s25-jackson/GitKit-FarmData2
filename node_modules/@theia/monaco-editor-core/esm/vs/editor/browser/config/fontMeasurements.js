...
class FontMeasurementsImpl extends lifecycle_js_1.Disposable {
    ...
    _ensureCache(targetWindow) {
        const windowId = (0, dom_js_1.getWindowId)(targetWindow);
    ...
    restoreFontInfo(targetWindow, savedFontInfos) {
        ...
        for (const savedFontInfo of savedFontInfos) {
            if (savedFontInfo.version !== fontInfo_js_1.SERIALIZED_FONT_INFO_VERSION) {
                // cannot use older version
                continue;
            }
            const fontInfo = new fontInfo_js_1.FontInfo(savedFontInfo, false);
    ...
    readFontInfo(targetWindow, bareFontInfo) {
        ...
        if (!cache.has(bareFontInfo)) {
            ...
            if (readConfig.typicalHalfwidthCharacterWidth <= 2 || readConfig.typicalFullwidthCharacterWidth <= 2 || readConfig.spaceWidth <= 2 || readConfig.maxDigitWidth <= 2) {
                ...
                readConfig = new fontInfo_js_1.FontInfo({
                    pixelRatio: pixelRatio_js_1.PixelRatio.getInstance(targetWindow).value,
    ...
    _createRequest(chr, type, all, monospace) {
        const result = new charWidthReader_js_1.CharWidthRequest(chr, type);
        all.push(result);
    ...
    _actualReadFontInfo(targetWindow, bareFontInfo) {
        ...
        (0, charWidthReader_js_1.readCharWidths)(targetWindow, bareFontInfo, all);
        const maxDigitWidth = Math.max(digit0.width, digit1.width, digit2.width, digit3.width, digit4.width, digit5.width, digit6.width, digit7.width, digit8.width, digit9.width);
        let isMonospace = (bareFontInfo.fontFeatureSettings === editorOptions_js_1.EditorFontLigatures.OFF);
        ...
        return new fontInfo_js_1.FontInfo({
            pixelRatio: pixelRatio_js_1.PixelRatio.getInstance(targetWindow).value,
