...
const diffProviderFactoryService_js_1 = require("./diffProviderFactoryService.js");
...
const defaultLinesDiffComputer_js_1 = require("../../../common/diff/defaultLinesDiffComputer/defaultLinesDiffComputer.js");
const rangeMapping_js_1 = require("../../../common/diff/rangeMapping.js");
const beforeEditPositionMapper_js_1 = require("../../../common/model/bracketPairsTextModelPart/bracketPairsTree/beforeEditPositionMapper.js");
const combineTextEditInfos_js_1 = require("../../../common/model/bracketPairsTextModelPart/bracketPairsTree/combineTextEditInfos.js");
const heuristicSequenceOptimizations_js_1 = require("../../../common/diff/defaultLinesDiffComputer/heuristicSequenceOptimizations.js");
...
let DiffEditorViewModel = class DiffEditorViewModel extends lifecycle_js_1.Disposable {
    ...
    constructor(model, _options, _diffProviderFactoryService) {
        ...
        this._isDiffUpToDate = (0, observable_js_1.observableValue)(this, false);
        this.isDiffUpToDate = this._isDiffUpToDate;
        this._diff = (0, observable_js_1.observableValue)(this, undefined);
        this.diff = this._diff;
        this._unchangedRegions = (0, observable_js_1.observableValue)(this, undefined);
        this.unchangedRegions = (0, observable_js_1.derived)(this, r => {
        ...
        this.movedTextToCompare = (0, observable_js_1.observableValue)(this, undefined);
        this._activeMovedText = (0, observable_js_1.observableValue)(this, undefined);
        this._hoveredMovedText = (0, observable_js_1.observableValue)(this, undefined);
        this.activeMovedText = (0, observable_js_1.derived)(this, r => this.movedTextToCompare.read(r) ?? this._hoveredMovedText.read(r) ?? this._activeMovedText.read(r));
        this._cancellationTokenSource = new cancellation_js_1.CancellationTokenSource();
        this._diffProvider = (0, observable_js_1.derived)(this, reader => {
            ...
            const onChangeSignal = (0, observable_js_1.observableSignalFromEvent)('onDidChange', diffProvider.onDidChange);
            return {
                diffProvider,
                onChangeSignal,
            };
        });
        this._register((0, lifecycle_js_1.toDisposable)(() => this._cancellationTokenSource.cancel()));
        const contentChangedSignal = (0, observable_js_1.observableSignal)('contentChangedSignal');
        const debouncer = this._register(new async_js_1.RunOnceScheduler(() => contentChangedSignal.trigger(undefined), 200));
        this._register((0, observable_js_1.autorun)(reader => {
            ...
            const lastUnchangedRegionsOrigRanges = lastUnchangedRegions.originalDecorationIds
                ...
                .map(r => r ? lineRange_js_1.LineRange.fromRangeInclusive(r) : undefined);
            ...
            const updatedLastUnchangedRegions = lastUnchangedRegions.regions.map((r, idx) => (!lastUnchangedRegionsOrigRanges[idx] || !lastUnchangedRegionsModRanges[idx]) ? undefined :
                new UnchangedRegion(lastUnchangedRegionsOrigRanges[idx].startLineNumber, lastUnchangedRegionsModRanges[idx].startLineNumber, lastUnchangedRegionsOrigRanges[idx].length, r.visibleLineCountTop.read(reader), r.visibleLineCountBottom.read(reader))).filter(types_js_1.isDefined);
            const newRanges = [];
            let didChange = false;
            for (const touching of (0, arrays_js_1.groupAdjacentBy)(updatedLastUnchangedRegions, (a, b) => a.getHiddenModifiedRange(reader).endLineNumberExclusive === b.getHiddenModifiedRange(reader).startLineNumber)) {
        ...
        const updateUnchangedRegions = (result, tx, reader) => {
            ...
            if (lastUnchangedRegions) {
                ...
                const lastUnchangedRegionsModRanges = lastUnchangedRegions.modifiedDecorationIds
                    .map(id => model.modified.getDecorationRange(id))
                    .map(r => r ? lineRange_js_1.LineRange.fromRangeInclusive(r) : undefined);
                const updatedLastUnchangedRegions = (0, utils_js_1.filterWithPrevious)(lastUnchangedRegions.regions
                    .map((r, idx) => {
                ...
                }).filter(types_js_1.isDefined), (cur, prev) => !prev || (cur.modifiedLineNumber >= prev.modifiedLineNumber + prev.lineCount && cur.originalLineNumber >= prev.originalLineNumber + prev.lineCount));
                let hiddenRegions = updatedLastUnchangedRegions.map(r => new rangeMapping_js_1.LineRangeMapping(r.getHiddenOriginalRange(reader), r.getHiddenModifiedRange(reader)));
                hiddenRegions = rangeMapping_js_1.LineRangeMapping.clip(hiddenRegions, lineRange_js_1.LineRange.ofLength(1, model.original.getLineCount()), lineRange_js_1.LineRange.ofLength(1, model.modified.getLineCount()));
                visibleRegions = rangeMapping_js_1.LineRangeMapping.inverse(hiddenRegions, model.original.getLineCount(), model.modified.getLineCount());
            }
        ...
        this._register(model.modified.onDidChangeContent((e) => {
            ...
            if (diff) {
                const textEdits = beforeEditPositionMapper_js_1.TextEditInfo.fromModelContentChanges(e.changes);
        ...
        this._register((0, observable_js_1.autorunWithStore)(async (reader, store) => {
            ...
            (0, hotReloadHelpers_js_1.readHotReloadableExport)(defaultLinesDiffComputer_js_1.DefaultLinesDiffComputer, reader);
            (0, hotReloadHelpers_js_1.readHotReloadableExport)(heuristicSequenceOptimizations_js_1.optimizeSequenceDiffs, reader);
            this._isDiffUpToDate.set(false, undefined);
            let originalTextEditInfos = [];
            store.add(model.original.onDidChangeContent((e) => {
                const edits = beforeEditPositionMapper_js_1.TextEditInfo.fromModelContentChanges(e.changes);
                originalTextEditInfos = (0, combineTextEditInfos_js_1.combineTextEditInfos)(originalTextEditInfos, edits);
            }));
            let modifiedTextEditInfos = [];
            store.add(model.modified.onDidChangeContent((e) => {
                const edits = beforeEditPositionMapper_js_1.TextEditInfo.fromModelContentChanges(e.changes);
                modifiedTextEditInfos = (0, combineTextEditInfos_js_1.combineTextEditInfos)(modifiedTextEditInfos, edits);
            }));
    ...
    async waitForDiff() {
        await (0, observable_js_1.waitForState)(this.isDiffUpToDate, s => s);
    }
    serializeState() {
    ...
    restoreSerializedState(state) {
        const ranges = state.collapsedRegions?.map(r => lineRange_js_1.LineRange.deserialize(r.range));
        const regions = this._unchangedRegions.get();
        if (!regions || !ranges) {
            return;
        }
...
exports.DiffEditorViewModel = DiffEditorViewModel = __decorate([
    __param(2, diffProviderFactoryService_js_1.IDiffProviderFactoryService)
], DiffEditorViewModel);
function normalizeDocumentDiff(diff, original, modified) {
    return {
        changes: diff.changes.map(c => new rangeMapping_js_1.DetailedLineRangeMapping(c.original, c.modified, c.innerChanges ? c.innerChanges.map(i => normalizeRangeMapping(i, original, modified)) : undefined)),
        moves: diff.moves,
...
function normalizeRangeMapping(rangeMapping, original, modified) {
    ...
    return new rangeMapping_js_1.RangeMapping(originalRange, modifiedRange);
}
class DiffState {
...
class UnchangedRegion {
    static fromDiffs(changes, originalLineCount, modifiedLineCount, minHiddenLineCount, minContext) {
        const inversedMappings = rangeMapping_js_1.DetailedLineRangeMapping.inverse(changes, originalLineCount, modifiedLineCount);
        const result = [];
    ...
    get originalUnchangedRange() {
        return lineRange_js_1.LineRange.ofLength(this.originalLineNumber, this.lineCount);
    }
    get modifiedUnchangedRange() {
        return lineRange_js_1.LineRange.ofLength(this.modifiedLineNumber, this.lineCount);
    }
    constructor(originalLineNumber, modifiedLineNumber, lineCount, visibleLineCountTop, visibleLineCountBottom) {
        ...
        this._visibleLineCountTop = (0, observable_js_1.observableValue)(this, 0);
        this.visibleLineCountTop = this._visibleLineCountTop;
        this._visibleLineCountBottom = (0, observable_js_1.observableValue)(this, 0);
        this.visibleLineCountBottom = this._visibleLineCountBottom;
        this._shouldHideControls = (0, observable_js_1.derived)(this, reader => /** @description isVisible */ this.visibleLineCountTop.read(reader) + this.visibleLineCountBottom.read(reader) === this.lineCount && !this.isDragged.read(reader));
        this.isDragged = (0, observable_js_1.observableValue)(this, undefined);
        ...
        (0, assert_js_1.softAssert)(visibleLineCountTop === visibleLineCountTop2);
        (0, assert_js_1.softAssert)(visibleLineCountBottom === visibleLineCountBottom2);
    ...
    setVisibleRanges(visibleRanges, tx) {
        const result = [];
        const hiddenModified = new lineRange_js_1.LineRangeSet(visibleRanges.map(r => r.modified)).subtractFrom(this.modifiedUnchangedRange);
    ...
    getHiddenOriginalRange(reader) {
        return lineRange_js_1.LineRange.ofLength(this.originalLineNumber + this._visibleLineCountTop.read(reader), this.lineCount - this._visibleLineCountTop.read(reader) - this._visibleLineCountBottom.read(reader));
    }
    getHiddenModifiedRange(reader) {
        return lineRange_js_1.LineRange.ofLength(this.modifiedLineNumber + this._visibleLineCountTop.read(reader), this.lineCount - this._visibleLineCountTop.read(reader) - this._visibleLineCountBottom.read(reader));
    }
