...
const scrollableElement_js_1 = require("../../../../../base/browser/ui/scrollbar/scrollableElement.js");
...
require("./accessibleDiffViewer.css");
const accessibleDiffViewerInsertIcon = (0, iconRegistry_js_1.registerIcon)('diff-review-insert', codicons_js_1.Codicon.add, (0, nls_js_1.localize)('accessibleDiffViewerInsertIcon', 'Icon for \'Insert\' in accessible diff viewer.'));
const accessibleDiffViewerRemoveIcon = (0, iconRegistry_js_1.registerIcon)('diff-review-remove', codicons_js_1.Codicon.remove, (0, nls_js_1.localize)('accessibleDiffViewerRemoveIcon', 'Icon for \'Remove\' in accessible diff viewer.'));
const accessibleDiffViewerCloseIcon = (0, iconRegistry_js_1.registerIcon)('diff-review-close', codicons_js_1.Codicon.close, (0, nls_js_1.localize)('accessibleDiffViewerCloseIcon', 'Icon for \'Close\' in accessible diff viewer.'));
let AccessibleDiffViewer = class AccessibleDiffViewer extends lifecycle_js_1.Disposable {
    static { this._ttPolicy = (0, trustedTypes_js_1.createTrustedTypesPolicy)('diffReview', { createHTML: value => value }); }
...
let ViewModel = class ViewModel extends lifecycle_js_1.Disposable {
    constructor(_diffs, _models, _setVisible, canClose, _accessibilitySignalService) {
        ...
        this._groups = (0, observable_js_1.observableValue)(this, []);
        this._currentGroupIdx = (0, observable_js_1.observableValue)(this, 0);
        this._currentElementIdx = (0, observable_js_1.observableValue)(this, 0);
        ...
        this._register((0, observable_js_1.autorun)(reader => {
            ...
            if (currentViewItem?.type === LineType.Deleted) {
                this._accessibilitySignalService.playSignal(accessibilitySignalService_js_1.AccessibilitySignal.diffLineDeleted, { source: 'accessibleDiffViewer.currentElementChanged' });
            }
            else if (currentViewItem?.type === LineType.Added) {
                this._accessibilitySignalService.playSignal(accessibilitySignalService_js_1.AccessibilitySignal.diffLineInserted, { source: 'accessibleDiffViewer.currentElementChanged' });
            }
        }));
        this._register((0, observable_js_1.autorun)(reader => {
            ...
            if (currentViewItem && currentViewItem.type !== LineType.Header) {
                ...
                this._models.modifiedSetSelection(range_js_1.Range.fromPositions(new position_js_1.Position(lineNumber, 1)));
            }
        }));
    }
    _goToGroupDelta(delta, tx) {
        ...
        (0, observable_js_1.subtransaction)(tx, tx => {
            this._currentGroupIdx.set(offsetRange_js_1.OffsetRange.ofLength(groups.length).clipCyclic(this._currentGroupIdx.get() + delta), tx);
    ...
    _goToLineDelta(delta) {
        ...
        (0, observable_js_1.transaction)(tx => {
            this._currentElementIdx.set(offsetRange_js_1.OffsetRange.ofLength(group.lines.length).clip(this._currentElementIdx.get() + delta), tx);
        });
    }
    ...
    revealCurrentElementInEditor() {
        ...
        if (curElem) {
            if (curElem.type === LineType.Deleted) {
                this._models.originalReveal(range_js_1.Range.fromPositions(new position_js_1.Position(curElem.originalLineNumber, 1)));
            }
            else {
                this._models.modifiedReveal(curElem.type !== LineType.Header
                    ? range_js_1.Range.fromPositions(new position_js_1.Position(curElem.modifiedLineNumber, 1))
                    : undefined);
            }
        }
    }
...
ViewModel = __decorate([
    __param(4, accessibilitySignalService_js_1.IAccessibilitySignalService)
], ViewModel);
const viewElementGroupLineMargin = 3;
function computeViewElementGroups(diffs, originalLineCount, modifiedLineCount) {
    const result = [];
    for (const g of (0, arrays_js_1.groupAdjacentBy)(diffs, (a, b) => (b.modified.startLineNumber - a.modified.endLineNumberExclusive < 2 * viewElementGroupLineMargin))) {
        const viewElements = [];
        viewElements.push(new HeaderViewElement());
        const origFullRange = new lineRange_js_1.LineRange(Math.max(1, g[0].original.startLineNumber - viewElementGroupLineMargin), Math.min(g[g.length - 1].original.endLineNumberExclusive + viewElementGroupLineMargin, originalLineCount + 1));
        const modifiedFullRange = new lineRange_js_1.LineRange(Math.max(1, g[0].modified.startLineNumber - viewElementGroupLineMargin), Math.min(g[g.length - 1].modified.endLineNumberExclusive + viewElementGroupLineMargin, modifiedLineCount + 1));
        (0, arrays_js_1.forEachAdjacent)(g, (a, b) => {
            const origRange = new lineRange_js_1.LineRange(a ? a.original.endLineNumberExclusive : origFullRange.startLineNumber, b ? b.original.startLineNumber : origFullRange.endLineNumberExclusive);
            const modifiedRange = new lineRange_js_1.LineRange(a ? a.modified.endLineNumberExclusive : modifiedFullRange.startLineNumber, b ? b.modified.startLineNumber : modifiedFullRange.endLineNumberExclusive);
        ...
        result.push(new ViewElementGroup(new rangeMapping_js_1.LineRangeMapping(modifiedRange, originalRange), viewElements));
    }
    return result;
}
var LineType;
...
let View = class View extends lifecycle_js_1.Disposable {
    constructor(_element, _model, _width, _height, _models, _languageService) {
        ...
        this._actionBar = this._register(new actionbar_js_1.ActionBar(actionBarContainer));
        this._register((0, observable_js_1.autorun)(reader => {
            /** @description update actions */
            this._actionBar.clear();
            if (this._model.canClose.read(reader)) {
                this._actionBar.push(new actions_js_1.Action('diffreview.close', (0, nls_js_1.localize)('label.close', "Close"), 'close-diff-review ' + themables_js_1.ThemeIcon.asClassName(accessibleDiffViewerCloseIcon), true, async () => _model.close()), { label: false, icon: true });
            }
        }));
        this._content = document.createElement('div');
        this._content.className = 'diff-review-content';
        this._content.setAttribute('role', 'code');
        this._scrollbar = this._register(new scrollableElement_js_1.DomScrollableElement(this._content, {}));
        (0, dom_js_1.reset)(this.domNode, this._scrollbar.getDomNode(), actionBarContainer);
        this._register((0, observable_js_1.autorun)(r => {
            this._height.read(r);
            this._width.read(r);
            this._scrollbar.scanDomNode();
        }));
        this._register((0, lifecycle_js_1.toDisposable)(() => { (0, dom_js_1.reset)(this.domNode); }));
        this._register((0, utils_js_1.applyStyle)(this.domNode, { width: this._width, height: this._height }));
        this._register((0, utils_js_1.applyStyle)(this._content, { width: this._width, height: this._height }));
        ...
        this._register((0, dom_js_1.addStandardDisposableListener)(this.domNode, 'keydown', (e) => {
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', (0, nls_js_1.localize)('ariaLabel', 'Accessible Diff Viewer. Use arrow up and down to navigate.'));
        (0, domFontInfo_js_1.applyFontInfo)(container, modifiedOptions.get(52 /* EditorOption.fontInfo */));
        (0, dom_js_1.reset)(this._content, container);
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? (0, nls_js_1.localize)('no_lines_changed', "no lines changed")
                    : lines === 1 ? (0, nls_js_1.localize)('one_line_changed', "1 line changed")
                        : (0, nls_js_1.localize)('more_lines_changed', "{0} lines changed", lines);
                ...
                header.setAttribute('aria-label', (0, nls_js_1.localize)({
                    key: 'header',
                    comment: [
            ...
            const isSelectedObs = (0, observable_js_1.derived)(reader => /** @description isSelected */ this._model.currentElement.read(reader) === viewItem);
            ...
            store.add((0, dom_js_1.addDisposableListener)(row, 'focus', () => {
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (spacerIcon) {
            ...
            spacerCodicon.className = themables_js_1.ThemeIcon.asClassName(spacerIcon);
        ...
        if (lineContent.length === 0) {
            lineContent = (0, nls_js_1.localize)('blankLine', "blank");
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = (0, nls_js_1.localize)({ key: 'unchangedLine', comment: ['The placeholders are contents of the line and should not be translated.'] }, "{0} unchanged line {1}", lineContent, item.originalLineNumber);
                }
                else {
                    ariaLabel = (0, nls_js_1.localize)('equalLine', "{0} original line {1} modified line {2}", lineContent, item.originalLineNumber, item.modifiedLineNumber);
                }
                break;
            case LineType.Added:
                ariaLabel = (0, nls_js_1.localize)('insertLine', "+ {0} modified line {1}", lineContent, item.modifiedLineNumber);
                break;
            case LineType.Deleted:
                ariaLabel = (0, nls_js_1.localize)('deleteLine', "- {0} original line {1}", lineContent, item.originalLineNumber);
                break;
        }
    ...
    _getLineHtml(model, options, tabSize, lineNumber, languageIdCodec) {
        ...
        const lineTokens = lineTokens_js_1.LineTokens.createEmpty(lineContent, languageIdCodec);
        const isBasicASCII = viewModel_js_1.ViewLineRenderingData.isBasicASCII(lineContent, model.mightContainNonBasicASCII());
        const containsRTL = viewModel_js_1.ViewLineRenderingData.containsRTL(lineContent, isBasicASCII, model.mightContainRTL());
        const r = (0, viewLineRenderer_js_1.renderViewLine2)(new viewLineRenderer_js_1.RenderLineInput((fontInfo.isMonospace && !options.get(33 /* EditorOption.disableMonospaceOptimizations */)), fontInfo.canUseHalfwidthRightwardsArrow, lineContent, false, isBasicASCII, containsRTL, 0, lineTokens, [], tabSize, 0, fontInfo.spaceWidth, fontInfo.middotWidth, fontInfo.wsmiddotWidth, options.get(122 /* EditorOption.stopRenderingLineAfter */), options.get(104 /* EditorOption.renderWhitespace */), options.get(99 /* EditorOption.renderControlCharacters */), options.get(53 /* EditorOption.fontLigatures */) !== editorOptions_js_1.EditorFontLigatures.OFF, null));
        return r.html;
    }
};
View = __decorate([
