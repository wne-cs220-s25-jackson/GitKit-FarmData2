...
const markdownRenderer_js_1 = require("../../widget/markdownRenderer/browser/markdownRenderer.js");
...
const $ = dom.$;
let HoverWidget = class HoverWidget extends widget_js_1.Widget {
    get _targetWindow() {
    ...
    constructor(options, _keybindingService, _configurationService, _openerService, _instantiationService, _accessibilityService) {
        ...
        this._messageListeners = new lifecycle_js_1.DisposableStore();
        ...
        this._onDispose = this._register(new event_js_1.Emitter());
        this._onRequestLayout = this._register(new event_js_1.Emitter());
        this._linkHandler = options.linkHandler || (url => {
            return (0, markdownRenderer_js_1.openLinkFromMarkdown)(this._openerService, url, (0, htmlContent_js_1.isMarkdownString)(options.content) ? options.content.isTrusted : undefined);
        });
        this._target = 'targetElements' in options.target ? options.target : new ElementHoverTarget(options.target);
        this._hoverPointer = options.appearance?.showPointer ? $('div.workbench-hover-pointer') : undefined;
        this._hover = this._register(new hoverWidget_js_1.HoverWidget(!options.appearance?.skipFadeInAnimation));
        ...
        this._hoverPosition = options.position?.hoverPosition === undefined
            ...
            : (0, types_js_1.isNumber)(options.position.hoverPosition)
        ...
        if (typeof options.content === 'string') {
            ...
            const mdRenderer = this._instantiationService.createInstance(markdownRenderer_js_1.MarkdownRenderer, { codeBlockFontFamily: this._configurationService.getValue('editor').fontFamily || editorOptions_js_1.EDITOR_FONT_DEFAULTS.fontFamily });
        ...
        if (options.actions && options.actions.length > 0) {
            ...
            options.actions.forEach(action => {
                ...
                hoverWidget_js_1.HoverAction.render(actionsElement, {
                    label: action.label,
        ...
        if (options.actions && options.actions.length > 0) {
            ...
            if (options.persistence?.hideOnHover === undefined) {
                ...
                hideOnHover = typeof options.content === 'string' ||
                    (0, htmlContent_js_1.isMarkdownString)(options.content) && !options.content.value.includes('](') && !options.content.value.includes('</a>');
            }
            else {
                // It's set explicitly
        ...
        if (options.appearance?.showHoverHint) {
            ...
            infoElement.textContent = (0, nls_js_1.localize)('hoverhint', 'Hold {0} key to mouse over', platform_js_1.isMacintosh ? 'Option' : 'Alt');
    ...
    render(container) {
        ...
        const accessibleViewHint = hoverFocused && (0, hoverWidget_js_1.getHoverAccessibleViewHint)(this._configurationService.getValue('accessibility.verbosity.hover') === true && this._accessibilityService.isScreenReaderOptimized(), this._keybindingService.lookupKeybinding('editor.action.accessibleView')?.getAriaLabel());
        if (accessibleViewHint) {
            (0, aria_js_1.status)(accessibleViewHint);
        }
        this.layout();
        this.addFocusTrap();
    }
...
exports.HoverWidget = HoverWidget = __decorate([
    __param(1, keybinding_js_1.IKeybindingService),
    __param(2, configuration_js_1.IConfigurationService),
    __param(3, opener_js_1.IOpenerService),
    __param(4, instantiation_js_1.IInstantiationService),
    __param(5, accessibility_js_1.IAccessibilityService)
], HoverWidget);
class CompositeMouseTracker extends widget_js_1.Widget {
    ...
    constructor(_elements, _eventDebounceDelay = 200) {
        ...
        this._mouseTimer = this._register(new lifecycle_js_1.MutableDisposable());
        this._onMouseOut = this._register(new event_js_1.Emitter());
    ...
    _onTargetMouseLeave() {
        ...
        this._mouseTimer.value = new async_js_1.TimeoutTimer(() => this._fireIfMouseOutside(), this._eventDebounceDelay);
    }
    _fireIfMouseOutside() {
        if (!this._isMouseIn) {
