...
const modelLineProjectionData_js_1 = require("../../common/modelLineProjectionData.js");
const textModelEvents_js_1 = require("../../common/textModelEvents.js");
const ttPolicy = (0, trustedTypes_js_1.createTrustedTypesPolicy)('domLineBreaksComputer', { createHTML: value => value });
class DOMLineBreaksComputerFactory {
    ...
    createLineBreaksComputer(fontInfo, tabSize, wrappingColumn, wrappingIndent, wordBreak) {
        ...
        return {
            ...
            finalize: () => {
                return createLineBreaks((0, types_js_1.assertIsDefined)(this.targetWindow.deref()), requests, fontInfo, tabSize, wrappingColumn, wrappingIndent, wordBreak, injectedTexts);
            }
        };
    }
}
...
function createLineBreaks(targetWindow, requests, fontInfo, tabSize, firstLineBreakColumn, wrappingIndent, wordBreak, injectedTextsPerLine) {
    function createEmptyLineBreakWithPossiblyInjectedText(requestIdx) {
        ...
        if (injectedTexts) {
            const lineText = textModelEvents_js_1.LineInjectedText.applyInjectedText(requests[requestIdx], injectedTexts);
            const injectionOptions = injectedTexts.map(t => t.options);
            const injectionOffsets = injectedTexts.map(text => text.column - 1);
            // creating a `LineBreakData` with an invalid `breakOffsetsVisibleColumn` is OK
            // because `breakOffsetsVisibleColumn` will never be used because it contains injected text
            return new modelLineProjectionData_js_1.ModelLineProjectionData(injectionOffsets, injectionOptions, [lineText.length], [], 0);
        }
        else {
            return null;
        }
    ...
    (0, domFontInfo_js_1.applyFontInfo)(containerDomNode, fontInfo);
    ...
    for (let i = 0; i < requests.length; i++) {
        const lineContent = textModelEvents_js_1.LineInjectedText.applyInjectedText(requests[i], injectedTextsPerLine[i]);
    ...
    for (let i = 0; i < requests.length; i++) {
        ...
        result[i] = new modelLineProjectionData_js_1.ModelLineProjectionData(injectionOffsets, injectionOptions, breakOffsets, breakOffsetsVisibleColumn, wrappedTextIndentLength);
    }
