...
const mouseEvent_js_1 = require("../../../base/browser/mouseEvent.js");
const lifecycle_js_1 = require("../../../base/common/lifecycle.js");
const platform = require("../../../base/common/platform.js");
const mouseTarget_js_1 = require("./mouseTarget.js");
const editorDom_js_1 = require("../editorDom.js");
...
class MouseHandler extends viewEventHandler_js_1.ViewEventHandler {
    constructor(context, viewController, viewHelper) {
        ...
        this.mouseTargetFactory = new mouseTarget_js_1.MouseTargetFactory(this._context, viewHelper);
        ...
        const mouseEvents = new editorDom_js_1.EditorMouseEventFactory(this.viewHelper.viewDomNode);
            ...
            if (!this._mouseLeaveMonitor) {
                this._mouseLeaveMonitor = dom.addDisposableListener(this.viewHelper.viewDomNode.ownerDocument, 'mousemove', (e) => {
                    if (!this.viewHelper.viewDomNode.contains(e.target)) {
                        ...
                        this._onMouseLeave(new editorDom_js_1.EditorMouseEvent(e, false, this.viewHelper.viewDomNode));
                    }
                });
            }
        }));
    ...
    _setupMouseWheelZoomListener() {
        const classifier = scrollableElement_js_1.MouseWheelClassifier.INSTANCE;
        let prevMouseWheelTime = 0;
        let gestureStartZoomLevel = editorZoom_js_1.EditorZoom.getZoomLevel();
        ...
        const onMouseWheel = (browserEvent) => {
            ...
            const e = new mouseEvent_js_1.StandardWheelEvent(browserEvent);
            classifier.acceptStandardWheelEvent(e);
            if (classifier.isPhysicalMouseWheel()) {
                if (hasMouseWheelZoomModifiers(browserEvent)) {
                    const zoomLevel = editorZoom_js_1.EditorZoom.getZoomLevel();
                    const delta = e.deltaY > 0 ? 1 : -1;
                    editorZoom_js_1.EditorZoom.setZoomLevel(zoomLevel + delta);
                    e.preventDefault();
                    e.stopPropagation();
                }
            }
                ...
                if (Date.now() - prevMouseWheelTime > 50) {
                    ...
                    gestureStartZoomLevel = editorZoom_js_1.EditorZoom.getZoomLevel();
                ...
                if (gestureHasZoomModifiers) {
                    editorZoom_js_1.EditorZoom.setZoomLevel(gestureStartZoomLevel + gestureAccumulatedDelta / 5);
                    e.preventDefault();
                    e.stopPropagation();
                }
            }
    ...
    getTargetAtClientPoint(clientX, clientY) {
        const clientPos = new editorDom_js_1.ClientCoordinates(clientX, clientY);
        const pos = clientPos.toPageCoordinates(dom.getWindow(this.viewHelper.viewDomNode));
        const editorPos = (0, editorDom_js_1.createEditorPagePosition)(this.viewHelper.viewDomNode);
        if (pos.y < editorPos.y || pos.y > editorPos.y + editorPos.height || pos.x < editorPos.x || pos.x > editorPos.x + editorPos.width) {
            return null;
        }
        const relativePos = (0, editorDom_js_1.createCoordinatesRelativeToEditor)(this.viewHelper.viewDomNode, editorPos, pos);
...
class MouseDownOperation extends lifecycle_js_1.Disposable {
    constructor(_context, _viewController, _viewHelper, _mouseTargetFactory, createMouseTarget, getMouseColumn) {
        ...
        this._mouseMoveMonitor = this._register(new editorDom_js_1.GlobalEditorPointerMoveMonitor(this._viewHelper.viewDomNode));
        ...
        this._currentSelection = new selection_js_1.Selection(1, 1, 1, 1);
    ...
    _getPositionOutsideEditor(e) {
        ...
        if (e.posy < editorContent.y) {
            ...
            const viewZoneData = mouseTarget_js_1.HitTestContext.getZoneAtCoord(this._context, verticalOffset);
            if (viewZoneData) {
                const newPosition = this._helpPositionJumpOverViewZone(viewZoneData);
                if (newPosition) {
                    return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, newPosition, 'above', outsideDistance);
                }
            }
            const aboveLineNumber = viewLayout.getLineNumberAtVerticalOffset(verticalOffset);
            return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, new position_js_1.Position(aboveLineNumber, 1), 'above', outsideDistance);
        }
        if (e.posy > editorContent.y + editorContent.height) {
            ...
            if (viewZoneData) {
                const newPosition = this._helpPositionJumpOverViewZone(viewZoneData);
                if (newPosition) {
                    return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, newPosition, 'below', outsideDistance);
                }
            }
            const belowLineNumber = viewLayout.getLineNumberAtVerticalOffset(verticalOffset);
            return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, new position_js_1.Position(belowLineNumber, model.getLineMaxColumn(belowLineNumber)), 'below', outsideDistance);
        }
        const possibleLineNumber = viewLayout.getLineNumberAtVerticalOffset(viewLayout.getCurrentScrollTop() + e.relativePos.y);
        if (e.posx < editorContent.x) {
            const outsideDistance = editorContent.x - e.posx;
            return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, new position_js_1.Position(possibleLineNumber, 1), 'left', outsideDistance);
        }
        if (e.posx > editorContent.x + editorContent.width) {
            const outsideDistance = e.posx - editorContent.x - editorContent.width;
            return mouseTarget_js_1.MouseTarget.createOutsideEditor(mouseColumn, new position_js_1.Position(possibleLineNumber, model.getLineMaxColumn(possibleLineNumber)), 'right', outsideDistance);
        }
        return null;
    }
    _findMousePosition(e, testEventTarget) {
        ...
        if (t.type === 8 /* MouseTargetType.CONTENT_VIEW_ZONE */ || t.type === 5 /* MouseTargetType.GUTTER_VIEW_ZONE */) {
            ...
            if (newPosition) {
                return mouseTarget_js_1.MouseTarget.createViewZone(t.type, t.element, t.mouseColumn, newPosition, t.detail);
            }
        }
        return t;
    }
    _helpPositionJumpOverViewZone(viewZoneData) {
        ...
        const selectionStart = new position_js_1.Position(this._currentSelection.selectionStartLineNumber, this._currentSelection.selectionStartColumn);
...
class TopBottomDragScrolling extends lifecycle_js_1.Disposable {
...
class TopBottomDragScrollingOperation extends lifecycle_js_1.Disposable {
    ...
    _execute() {
        ...
        let mouseTarget;
        {
            const editorPos = (0, editorDom_js_1.createEditorPagePosition)(this._viewHelper.viewDomNode);
            ...
            const pos = new editorDom_js_1.PageCoordinates(this._mouseEvent.pos.x, editorPos.y + editorPos.height - horizontalScrollbarHeight - 0.1);
            const relativePos = (0, editorDom_js_1.createCoordinatesRelativeToEditor)(this._viewHelper.viewDomNode, editorPos, pos);
            mouseTarget = this._mouseTargetFactory.createMouseTarget(this._viewHelper.getLastRenderData(), editorPos, pos, relativePos, null);
        }
        if (!mouseTarget.position || mouseTarget.position.lineNumber !== edgeLineNumber) {
            if (this._position.outsidePosition === 'above') {
                mouseTarget = mouseTarget_js_1.MouseTarget.createOutsideEditor(this._position.mouseColumn, new position_js_1.Position(edgeLineNumber, 1), 'above', this._position.outsideDistance);
            }
            else {
                mouseTarget = mouseTarget_js_1.MouseTarget.createOutsideEditor(this._position.mouseColumn, new position_js_1.Position(edgeLineNumber, this._context.viewModel.getLineMaxColumn(edgeLineNumber)), 'below', this._position.outsideDistance);
            }
        }
