...
exports.PagedScreenReaderStrategy = void 0;
exports.ariaLabelForScreenReaderContent = ariaLabelForScreenReaderContent;
exports.newlinecount = newlinecount;
...
class PagedScreenReaderStrategy {
    ...
    static fromEditorSelection(model, selection, linesPerPage, trimLongText) {
        ...
        let pretextRange = selectionStartPageRange.intersectRanges(new range_js_1.Range(1, 1, selection.startLineNumber, selection.startColumn));
        if (trimLongText && model.getValueLengthInRange(pretextRange, 1 /* EndOfLinePreference.LF */) > LIMIT_CHARS) {
            const pretextStart = model.modifyPosition(pretextRange.getEndPosition(), -LIMIT_CHARS);
            pretextRange = range_js_1.Range.fromPositions(pretextStart, pretextRange.getEndPosition());
        }
        const pretext = model.getValueInRange(pretextRange, 1 /* EndOfLinePreference.LF */);
        const lastLine = model.getLineCount();
        const lastLineMaxColumn = model.getLineMaxColumn(lastLine);
        let posttextRange = selectionEndPageRange.intersectRanges(new range_js_1.Range(selection.endLineNumber, selection.endColumn, lastLine, lastLineMaxColumn));
        if (trimLongText && model.getValueLengthInRange(posttextRange, 1 /* EndOfLinePreference.LF */) > LIMIT_CHARS) {
            const posttextEnd = model.modifyPosition(posttextRange.getStartPosition(), LIMIT_CHARS);
            posttextRange = range_js_1.Range.fromPositions(posttextRange.getStartPosition(), posttextEnd);
        }
