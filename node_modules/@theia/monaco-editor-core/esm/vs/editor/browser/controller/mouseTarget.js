...
const viewLine_js_1 = require("../viewParts/viewLines/viewLine.js");
...
const cursorAtomicMoveOperations_js_1 = require("../../common/cursor/cursorAtomicMoveOperations.js");
...
class MouseTarget {
    static _deduceRage(position, range = null) {
        if (!range && position) {
            return new range_js_1.Range(position.lineNumber, position.column, position.lineNumber, position.column);
        }
        return range ?? null;
    }
...
class HitTestContext {
    ...
    static getZoneAtCoord(context, mouseVerticalOffset) {
        ...
        if (viewZoneWhitespace) {
            ...
            if (viewZoneWhitespace.afterLineNumber !== lineCount) {
                ...
                positionAfter = new position_js_1.Position(viewZoneWhitespace.afterLineNumber + 1, 1);
            }
            if (viewZoneWhitespace.afterLineNumber > 0) {
                // There are more lines above this view zone
                positionBefore = new position_js_1.Position(viewZoneWhitespace.afterLineNumber, context.viewModel.getLineMaxColumn(viewZoneWhitespace.afterLineNumber));
            }
    ...
    getFullLineRangeAtCoord(mouseVerticalOffset) {
        if (this._context.viewLayout.isAfterLines(mouseVerticalOffset)) {
            ...
            return {
                range: new range_js_1.Range(lineNumber, maxLineColumn, lineNumber, maxLineColumn),
                isAfterLines: true
            };
        }
        ...
        const maxLineColumn = this._context.viewModel.getLineMaxColumn(lineNumber);
        return {
            range: new range_js_1.Range(lineNumber, 1, lineNumber, maxLineColumn),
            isAfterLines: false
        };
    }
...
class HitTestRequest extends BareHitTestRequest {
    ...
    get targetPath() {
        if (this._targetPathCacheElement !== this.target) {
            ...
            this._targetPathCacheValue = viewPart_js_1.PartFingerprints.collect(this.target, this._ctx.viewDomNode);
        }
        return this._targetPathCacheValue;
    }
    constructor(ctx, editorPos, pos, relativePos, eventTarget) {
        super(ctx, editorPos, pos, relativePos);
        this.hitTestResult = new lazy_js_1.Lazy(() => MouseTargetFactory.doHitTest(this._ctx, this));
    ...
    _getMouseColumn(position = null) {
        if (position && position.column < this._ctx.viewModel.getLineMaxColumn(position.lineNumber)) {
            ...
            return cursorColumns_js_1.CursorColumns.visibleColumnFromColumn(this._ctx.viewModel.getLineContent(position.lineNumber), position.column, this._ctx.viewModel.model.getOptions().tabSize) + 1;
        }
...
class MouseTargetFactory {
    ...
    mouseTargetIsWidget(e) {
        const t = e.target;
        const path = viewPart_js_1.PartFingerprints.collect(t, this._viewHelper.viewDomNode);
        ...
        try {
            ...
            if (r.type === 6 /* MouseTargetType.CONTENT_TEXT */) {
                ...
                if (ctx.stickyTabStops && r.position !== null) {
                    ...
                    const range = range_js_1.Range.fromPositions(position, position).plusRange(r.range);
    ...
    static _hitTestViewLines(ctx, request) {
        ...
        if (ctx.isInTopPadding(request.mouseVerticalOffset)) {
            return request.fulfillContentEmpty(new position_js_1.Position(1, 1), EMPTY_CONTENT_AFTER_LINES);
        }
        ...
        if (ctx.isAfterLines(request.mouseVerticalOffset) || ctx.isInBottomPadding(request.mouseVerticalOffset)) {
            ...
            return request.fulfillContentEmpty(new position_js_1.Position(lineCount, maxLineColumn), EMPTY_CONTENT_AFTER_LINES);
        }
        ...
        if (ElementPath.isStrictChildOfViewLines(request.targetPath)) {
            ...
            if (ctx.viewModel.getLineLength(lineNumber) === 0) {
                ...
                return request.fulfillContentEmpty(new position_js_1.Position(lineNumber, 1), detail);
            }
            const lineWidth = ctx.getLineWidth(lineNumber);
            if (request.mouseContentHorizontalOffset >= lineWidth) {
                // TODO: This is wrong for RTL
                const detail = createEmptyContentDataInLines(request.mouseContentHorizontalOffset - lineWidth);
                const pos = new position_js_1.Position(lineNumber, ctx.viewModel.getLineMaxColumn(lineNumber));
            ...
            if (ctx.viewLinesGpu) {
                ...
                if (position) {
                    ...
                    return request.fulfillContentText(position, range_js_1.Range.fromPositions(position, position), detail);
                }
            }
        }
    ...
    static _hitTestMinimap(ctx, request) {
        if (ElementPath.isChildOfMinimap(request.targetPath)) {
            ...
            return request.fulfillScrollbar(new position_js_1.Position(possibleLineNumber, maxColumn));
        }
        return null;
    }
    ...
    static createMouseTargetFromHitTestPosition(ctx, request, spanNode, pos, injectedText) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            if (prev.offset <= request.mouseContentHorizontalOffset && request.mouseContentHorizontalOffset <= curr.offset) {
                rng = new range_js_1.Range(lineNumber, prev.column, lineNumber, curr.column);
                ...
                pos = (prevDelta < nextDelta
                    ? new position_js_1.Position(lineNumber, prev.column)
                    : new position_js_1.Position(lineNumber, curr.column));
                break;
            }
        }
    ...
    static _doHitTestWithCaretRangeFromPoint(ctx, request) {
        ...
        if (!isBelowLastLine) {
            ...
            const adjustedPage = new editorDom_js_1.PageCoordinates(request.pos.x, adjustedPageY);
    ...
    static _actualDoHitTestWithCaretRangeFromPoint(ctx, coords) {
        ...
        if (startContainer.nodeType === startContainer.TEXT_NODE) {
            ...
            if (parent3ClassName === viewLine_js_1.ViewLine.CLASS_NAME) {
            ...
            if (parent2ClassName === viewLine_js_1.ViewLine.CLASS_NAME) {
    ...
    static _doHitTestWithCaretPositionFromPoint(ctx, coords) {
        ...
        if (hitResult.offsetNode.nodeType === hitResult.offsetNode.ELEMENT_NODE) {
            ...
            const parent2ClassName = parent2 && parent2.nodeType === parent2.ELEMENT_NODE ? parent2.className : null;
            if (parent1ClassName === viewLine_js_1.ViewLine.CLASS_NAME) {
            ...
            else if (parent2ClassName === viewLine_js_1.ViewLine.CLASS_NAME) {
    ...
    static _snapToSoftTabBoundary(position, viewModel) {
        ...
        const newPosition = cursorAtomicMoveOperations_js_1.AtomicTabMoveOperations.atomicPosition(lineContent, position.column - 1, tabSize, 2 /* Direction.Nearest */);
        if (newPosition !== -1) {
            return new position_js_1.Position(position.lineNumber, newPosition + 1);
        }
        return position;
    }
