...
const encodedTokenAttributes_js_1 = require("../../common/encodedTokenAttributes.js");
const tokenization_js_1 = require("../../common/languages/supports/tokenization.js");
const themes_js_1 = require("../common/themes.js");
...
const iconsStyleSheet_js_1 = require("../../../platform/theme/browser/iconsStyleSheet.js");
...
const colorRegistry = platform_js_1.Registry.as(colorRegistry_js_1.Extensions.ColorContribution);
const themingRegistry = platform_js_1.Registry.as(themeService_js_1.Extensions.ThemingContribution);
class StandaloneTheme {
    ...
    getColors() {
        if (!this.colors) {
            ...
            for (const id in this.themeData.colors) {
                colors.set(id, color_js_1.Color.fromHex(this.themeData.colors[id]));
            }
            if (this.themeData.inherit) {
                const baseData = getBuiltinRules(this.themeData.base);
                for (const id in baseData.colors) {
                    if (!colors.has(id)) {
                        colors.set(id, color_js_1.Color.fromHex(baseData.colors[id]));
                    }
                }
            }
            this.colors = colors;
    ...
    get type() {
        switch (this.base) {
            case exports.VS_LIGHT_THEME_NAME: return theme_js_1.ColorScheme.LIGHT;
            case exports.HC_BLACK_THEME_NAME: return theme_js_1.ColorScheme.HIGH_CONTRAST_DARK;
            case exports.HC_LIGHT_THEME_NAME: return theme_js_1.ColorScheme.HIGH_CONTRAST_LIGHT;
            default: return theme_js_1.ColorScheme.DARK;
        }
    }
    get tokenTheme() {
        if (!this._tokenTheme) {
            ...
            this._tokenTheme = tokenization_js_1.TokenTheme.createFromRawTokenTheme(rules, encodedTokensColors);
        }
    ...
    getTokenStyleMetadata(type, modifiers, modelLanguage) {
        ...
        const foreground = encodedTokenAttributes_js_1.TokenMetadata.getForeground(metadata);
        const fontStyle = encodedTokenAttributes_js_1.TokenMetadata.getFontStyle(metadata);
        return {
...
function getBuiltinRules(builtinTheme) {
    switch (builtinTheme) {
        case exports.VS_LIGHT_THEME_NAME:
            return themes_js_1.vs;
        case exports.VS_DARK_THEME_NAME:
            return themes_js_1.vs_dark;
        case exports.HC_BLACK_THEME_NAME:
            return themes_js_1.hc_black;
        case exports.HC_LIGHT_THEME_NAME:
            return themes_js_1.hc_light;
    }
}
...
class StandaloneThemeService extends lifecycle_js_1.Disposable {
    constructor() {
        super();
        this._onColorThemeChange = this._register(new event_js_1.Emitter());
        this.onDidColorThemeChange = this._onColorThemeChange.event;
        this._onFileIconThemeChange = this._register(new event_js_1.Emitter());
        this.onDidFileIconThemeChange = this._onFileIconThemeChange.event;
        this._onProductIconThemeChange = this._register(new event_js_1.Emitter());
        this.onDidProductIconThemeChange = this._onProductIconThemeChange.event;
        this._environment = Object.create(null);
        this._builtInProductIconTheme = new iconsStyleSheet_js_1.UnthemedProductIconTheme();
        ...
        const iconsStyleSheet = this._register((0, iconsStyleSheet_js_1.getIconsStyleSheet)(this));
        ...
        (0, browser_js_1.addMatchMediaChangeListener)(window_js_1.mainWindow, '(forced-colors: active)', () => {
            this._onOSSchemeChanged();
        });
    }
    ...
    _onOSSchemeChanged() {
        if (this._autoDetectHighContrast) {
            const wantsHighContrast = window_js_1.mainWindow.matchMedia(`(forced-colors: active)`).matches;
            if (wantsHighContrast !== (0, theme_js_1.isHighContrast)(this._theme.type)) {
                // switch to high contrast or non-high contrast but stick to dark or light
                let newThemeName;
                if ((0, theme_js_1.isDark)(this._theme.type)) {
    ...
    _updateThemeOrColorMap() {
        ...
        for (const item of colorRegistry.getColors()) {
            ...
            if (color) {
                colorVariables.push(`${(0, colorRegistry_js_1.asCssVariableName)(item.id)}: ${color.toString()};`);
            }
        }
        ruleCollector.addRule(`.monaco-editor, .monaco-diff-editor, .monaco-component { ${colorVariables.join('\n')} }`);
        const colorMap = this._colorMapOverride || this._theme.tokenTheme.getColorMap();
        ruleCollector.addRule((0, tokenization_js_1.generateTokensCSSForColorMap)(colorMap));
        this._themeCSS = cssRules.join('\n');
        this._updateCSS();
        languages_js_1.TokenizationRegistry.setColorMap(colorMap);
