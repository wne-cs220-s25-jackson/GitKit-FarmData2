...
const stableEditorScroll_js_1 = require("../../../browser/stableEditorScroll.js");
class FormattingEdit {
    ...
    static execute(editor, _edits, addUndoStops) {
        ...
        const scrollState = stableEditorScroll_js_1.StableEditorScrollState.capture(editor);
        const edits = FormattingEdit._handleEolEdits(editor, _edits);
        if (edits.length === 1 && FormattingEdit._isFullModelReplaceEdit(editor, edits[0])) {
            // We use replace semantics and hope that markers stay put...
            editor.executeEdits('formatEditsCommand', edits.map(edit => editOperation_js_1.EditOperation.replace(range_js_1.Range.lift(edit.range), edit.text)));
        }
        else {
            editor.executeEdits('formatEditsCommand', edits.map(edit => editOperation_js_1.EditOperation.replaceMove(range_js_1.Range.lift(edit.range), edit.text)));
        }
        if (addUndoStops) {
            editor.pushUndoStop();
        }
