...
const defaultColor = new color_js_1.Color(new color_js_1.RGBA(0, 122, 204));
...
class Arrow {
    static { this._IdGenerator = new idGenerator_js_1.IdGenerator('.arrow-decoration-'); }
    constructor(_editor) {
    ...
    show(where) {
        ...
        this._decorations.set([{
                range: range_js_1.Range.fromPositions(where),
                options: {
...
class ZoneWidget {
    ...
    show(rangeOrPos, heightInLines) {
        const range = range_js_1.Range.isIRange(rangeOrPos) ? range_js_1.Range.lift(rangeOrPos) : range_js_1.Range.fromPositions(rangeOrPos);
        this._isShowing = true;
        this._showImpl(range, heightInLines);
        this._isShowing = false;
        this._positionMarkerId.set([{ range, options: textModel_js_1.ModelDecorationOptions.EMPTY }]);
    }
    updatePositionAndHeight(rangeOrPos, heightInLines) {
        if (this._viewZone) {
            rangeOrPos = range_js_1.Range.isIRange(rangeOrPos) ? range_js_1.Range.getStartPosition(rangeOrPos) : rangeOrPos;
            ...
            this._positionMarkerId.set([{
                    range: range_js_1.Range.isIRange(rangeOrPos) ? rangeOrPos : range_js_1.Range.fromPositions(rangeOrPos),
    ...
    _showImpl(where, heightInLines) {
        ...
        if (model) {
            const range = model.validateRange(new range_js_1.Range(where.startLineNumber, 1, where.endLineNumber + 1, 1));
    ...
    _initSash() {
        ...
        this._resizeSash = this._disposables.add(new sash_js_1.Sash(this.domNode, this, { orientation: 1 /* Orientation.HORIZONTAL */ }));
