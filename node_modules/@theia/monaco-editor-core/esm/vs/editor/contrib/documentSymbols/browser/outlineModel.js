...
class TreeElement {
    ...
    static getElementById(id, element) {
        ...
        const len = (0, strings_js_1.commonPrefixLength)(id, element.id);
        if (len === id.length) {
            return element;
        }
...
class OutlineGroup extends TreeElement {
    ...
    _getItemEnclosingPosition(position, children) {
        for (const [, item] of children) {
            if (!item.symbol.range || !range_js_1.Range.containsPosition(item.symbol.range, position)) {
                continue;
            }
    ...
    _updateMarker(markers, item) {
        ...
        const idx = (0, arrays_js_1.binarySearch)(markers, item.symbol.range, range_js_1.Range.compareRangesUsingStarts);
        let start;
        if (idx < 0) {
            start = ~idx;
            if (start > 0 && range_js_1.Range.areIntersecting(markers[start - 1], item.symbol.range)) {
                start -= 1;
            }
        }
        else {
        ...
        const myMarkers = [];
        let myTopSev;
        for (; start < markers.length && range_js_1.Range.areIntersecting(item.symbol.range, markers[start]); start++) {
        ...
        (0, arrays_js_1.coalesceInPlace)(markers);
    }
}
exports.OutlineGroup = OutlineGroup;
class OutlineModel extends TreeElement {
    static create(registry, textModel, token) {
        const cts = new cancellation_js_1.CancellationTokenSource(token);
        ...
        const listener = registry.onDidChange(() => {
            ...
            if (!(0, arrays_js_1.equals)(newProvider, provider)) {
                cts.cancel();
            }
        });
    ...
    _compact() {
        ...
        if (count !== 1) {
            ...
            const group = iterator_js_1.Iterable.first(this._groups.values());
    ...
    updateMarker(marker) {
        ...
        marker.sort(range_js_1.Range.compareRangesUsingStarts);
        for (const [, group] of this._groups) {
    ...
    getTopLevelSymbols() {
        ...
        for (const child of this.children.values()) {
            if (child instanceof OutlineElement) {
                ...
                roots.push(...iterator_js_1.Iterable.map(child.children.values(), child => child.symbol));
            }
        }
        return roots.sort((a, b) => range_js_1.Range.compareRangesUsingStarts(a.range, b.range));
    }
    asListOfDocumentSymbols() {
        const roots = this.getTopLevelSymbols();
        const bucket = [];
        OutlineModel._flattenDocumentSymbols(bucket, roots, '');
        return bucket.sort((a, b) => position_js_1.Position.compare(range_js_1.Range.getStartPosition(a.range), range_js_1.Range.getStartPosition(b.range)) || position_js_1.Position.compare(range_js_1.Range.getEndPosition(b.range), range_js_1.Range.getEndPosition(a.range)));
    }
...
exports.IOutlineModelService = (0, instantiation_js_1.createDecorator)('IOutlineModelService');
let OutlineModelService = class OutlineModelService {
    constructor(_languageFeaturesService, debounces, modelService) {
        ...
        this._cache = new map_js_1.LRUCache(15, 0.7);
    ...
    async getOrCreate(textModel, token) {
        ...
        if (!data || data.versionId !== textModel.getVersionId() || !(0, arrays_js_1.equals)(data.provider, provider)) {
    ...
    getCachedModels() {
        return iterator_js_1.Iterable.filter(iterator_js_1.Iterable.map(this._cache.values(), entry => entry.model), model => model !== undefined);
    }
};
exports.OutlineModelService = OutlineModelService;
exports.OutlineModelService = OutlineModelService = __decorate([
    __param(0, languageFeatures_js_1.ILanguageFeaturesService),
    __param(1, languageFeatureDebounce_js_1.ILanguageFeatureDebounceService),
    __param(2, model_js_1.IModelService)
], OutlineModelService);
(0, extensions_js_1.registerSingleton)(exports.IOutlineModelService, OutlineModelService, 1 /* InstantiationType.Delayed */);
