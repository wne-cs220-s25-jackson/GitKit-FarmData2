...
const syntaxRangeProvider_js_1 = require("../../folding/browser/syntaxRangeProvider.js");
const indentRangeProvider_js_1 = require("../../folding/browser/indentRangeProvider.js");
...
let StickyModelProvider = class StickyModelProvider extends lifecycle_js_1.Disposable {
    constructor(_editor, onProviderUpdate, _languageConfigurationService, _languageFeaturesService) {
        super();
        this._editor = _editor;
        this._modelProviders = [];
        this._modelPromise = null;
        this._updateScheduler = this._register(new async_js_1.Delayer(300));
        this._updateOperation = this._register(new lifecycle_js_1.DisposableStore());
    ...
    async update(token) {
        ...
        return await this._updateScheduler.trigger(async () => {
            ...
            (0, errors_js_1.onUnexpectedError)(error);
            return null;
        });
    }
};
...
class StickyModelCandidateProvider extends lifecycle_js_1.Disposable {
    constructor(_editor) {
        super();
        this._editor = _editor;
    ...
    computeStickyModel(token) {
        ...
        const providerModelPromise = (0, async_js_1.createCancelablePromise)(token => this.createModelFromProvider(token));
        return {
...
let StickyModelFromCandidateOutlineProvider = class StickyModelFromCandidateOutlineProvider extends StickyModelCandidateProvider {
    ...
    createModelFromProvider(token) {
        return outlineModel_js_1.OutlineModel.create(this._languageFeaturesService.documentSymbolProvider, this._editor.getModel(), token);
    }
    createStickyModel(token, model) {
        ...
        return new stickyScrollElement_js_1.StickyModel(textModel.uri, textModel.getVersionId(), stickyOutlineElement, providerID);
    }
    isModelValid(model) {
    ...
    _stickyModelFromOutlineModel(outlineModel, preferredProvider) {
        ...
        if (iterator_js_1.Iterable.first(outlineModel.children.values()) instanceof outlineModel_js_1.OutlineGroup) {
            const provider = iterator_js_1.Iterable.find(outlineModel.children.values(), outlineGroupOfModel => outlineGroupOfModel.id === preferredProvider);
            if (provider) {
        ...
        const outlineElementsArray = Array.from(outlineElements.values()).sort((element1, element2) => {
            const range1 = new stickyScrollElement_js_1.StickyRange(element1.symbol.range.startLineNumber, element1.symbol.range.endLineNumber);
            const range2 = new stickyScrollElement_js_1.StickyRange(element2.symbol.range.startLineNumber, element2.symbol.range.endLineNumber);
        ...
        const stickyOutlineElement = new stickyScrollElement_js_1.StickyElement(undefined, stickyChildren, undefined);
        return {
    ...
    _stickyModelFromOutlineElement(outlineElement, previousStartLine) {
        ...
        const range = new stickyScrollElement_js_1.StickyRange(outlineElement.symbol.selectionRange.startLineNumber, outlineElement.symbol.range.endLineNumber);
        return new stickyScrollElement_js_1.StickyElement(range, children, undefined);
    }
    ...
    _findSumOfRangesOfGroup(outline) {
        ...
        if (outline instanceof outlineModel_js_1.OutlineElement) {
...
class StickyModelFromCandidateFoldingProvider extends StickyModelCandidateProvider {
    constructor(editor) {
        super(editor);
        this._foldingLimitReporter = new folding_js_1.RangesLimitReporter(editor);
    }
    createStickyModel(token, model) {
        const foldingElement = this._fromFoldingRegions(model);
        const textModel = this._editor.getModel();
        return new stickyScrollElement_js_1.StickyModel(textModel.uri, textModel.getVersionId(), foldingElement, undefined);
    }
    isModelValid(model) {
        return model !== null;
    }
    _fromFoldingRegions(foldingRegions) {
        ...
        const stickyOutlineElement = new stickyScrollElement_js_1.StickyElement(undefined, [], undefined);
        for (let i = 0; i < length; i++) {
            ...
            const child = new stickyScrollElement_js_1.StickyElement(new stickyScrollElement_js_1.StickyRange(foldingRegions.getStartLineNumber(i), foldingRegions.getEndLineNumber(i) + 1), [], parentNode);
...
let StickyModelFromCandidateIndentationFoldingProvider = class StickyModelFromCandidateIndentationFoldingProvider extends StickyModelFromCandidateFoldingProvider {
    constructor(editor, _languageConfigurationService) {
        ...
        this.provider = this._register(new indentRangeProvider_js_1.IndentRangeProvider(editor.getModel(), this._languageConfigurationService, this._foldingLimitReporter));
    }
...
let StickyModelFromCandidateSyntaxFoldingProvider = class StickyModelFromCandidateSyntaxFoldingProvider extends StickyModelFromCandidateFoldingProvider {
    constructor(editor, onProviderUpdate, _languageFeaturesService) {
        super(editor);
        this._languageFeaturesService = _languageFeaturesService;
        const selectedProviders = folding_js_1.FoldingController.getFoldingRangeProviders(this._languageFeaturesService, editor.getModel());
        if (selectedProviders.length > 0) {
            this.provider = this._register(new syntaxRangeProvider_js_1.SyntaxRangeProvider(editor.getModel(), selectedProviders, onProviderUpdate, this._foldingLimitReporter, undefined));
        }
    }
    isProviderValid() {
