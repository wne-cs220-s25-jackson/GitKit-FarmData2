...
const viewLine_js_1 = require("../../../browser/viewParts/viewLines/viewLine.js");
...
const foldingDecorations_js_1 = require("../../folding/browser/foldingDecorations.js");
class StickyScrollWidgetState {
    ...
    equals(other) {
        return !!other
            ...
            && (0, arrays_js_1.equals)(this.startLineNumbers, other.startLineNumbers)
            && (0, arrays_js_1.equals)(this.endLineNumbers, other.endLineNumbers);
    }
    static get Empty() {
...
const _ttPolicy = (0, trustedTypes_js_1.createTrustedTypesPolicy)('stickyScrollViewLayer', { createHTML: value => value });
...
class StickyScrollWidget extends lifecycle_js_1.Disposable {
    constructor(_editor) {
        super();
        this._editor = _editor;
        this._foldingIconStore = new lifecycle_js_1.DisposableStore();
        ...
        this._rootDomNode.classList.toggle('peek', _editor instanceof embeddedCodeEditorWidget_js_1.EmbeddedCodeEditorWidget);
    ...
    _renderChildNode(index, line, foldingModel, layoutInfo) {
        ...
        const viewLineNumber = viewModel.coordinatesConverter.convertModelPositionToViewPosition(new position_js_1.Position(line, 1)).lineNumber;
        const lineRenderingData = viewModel.getViewLineRenderingData(viewLineNumber);
        const lineNumberOption = this._editor.getOption(69 /* EditorOption.lineNumbers */);
        let actualInlineDecorations;
        try {
            actualInlineDecorations = lineDecorations_js_1.LineDecoration.filter(lineRenderingData.inlineDecorations, viewLineNumber, lineRenderingData.minColumn, lineRenderingData.maxColumn);
        }
        catch (err) {
            actualInlineDecorations = [];
        }
        const renderLineInput = new viewLineRenderer_js_1.RenderLineInput(true, true, lineRenderingData.content, lineRenderingData.continuesWithWrappedLine, lineRenderingData.isBasicASCII, lineRenderingData.containsRTL, 0, lineRenderingData.tokens, actualInlineDecorations, lineRenderingData.tabSize, lineRenderingData.startVisibleColumn, 1, 1, 1, 500, 'none', true, true, null);
        const sb = new stringBuilder_js_1.StringBuilder(2000);
        const renderOutput = (0, viewLineRenderer_js_1.renderViewLine)(renderLineInput, sb);
        let newLine;
        if (_ttPolicy) {
    ...
    getEditorPositionFromNode(spanDomNode) {
        ...
        const column = (0, viewLine_js_1.getColumnOfNodeOffset)(renderedStickyLine.characterMapping, spanDomNode, 0);
        return new position_js_1.Position(renderedStickyLine.lineNumber, column);
    }
...
class StickyFoldingIcon {
    constructor(isCollapsed, foldingStartLine, foldingEndLine, dimension) {
        ...
        this.domNode.className = themables_js_1.ThemeIcon.asClassName(isCollapsed ? foldingDecorations_js_1.foldingCollapsedIcon : foldingDecorations_js_1.foldingExpandedIcon);
    }
    setVisible(visible) {
