...
const diff_js_1 = require("../../../../../base/common/diff/diff.js");
...
function computeGhostText(edit, model, mode, cursorPosition, previewSuffixLength = 0) {
    let e = (0, singleTextEditHelpers_js_1.singleTextRemoveCommonPrefix)(edit, model);
    ...
    const sourceIndentationLength = (0, strings_js_1.getLeadingWhitespace)(sourceLine).length;
    ...
    if (suggestionTouchesIndentation) {
        ...
        const suggestionAddedIndentationLength = (0, strings_js_1.getLeadingWhitespace)(e.text).length;
        ...
        const newStartPosition = startPosition.column + replacedIndentation.length <= endPosition.column
            ...
            : endPosition;
        const rangeThatDoesNotReplaceIndentation = range_js_1.Range.fromPositions(newStartPosition, endPosition);
        ...
        e = new textEdit_js_1.SingleTextEdit(rangeThatDoesNotReplaceIndentation, suggestionWithoutIndentationChange);
    }
    ...
    for (const c of changes) {
        ...
        if (nonPreviewText.length > 0) {
            parts.push(new ghostText_js_1.GhostTextPart(insertColumn, nonPreviewText, false));
        }
        if (italicText.length > 0) {
            parts.push(new ghostText_js_1.GhostTextPart(insertColumn, italicText, true));
        }
    }
    return new ghostText_js_1.GhostText(lineNumber, parts);
}
...
function smartDiff(originalValue, newValue, smartBracketMatching) {
    ...
    return new diff_js_1.LcsDiff({ getElements: () => elements1 }, { getElements: () => elements2 }).ComputeDiff(false).changes;
}
