...
const languageSelector_js_1 = require("../../../common/languageSelector.js");
...
const highlightDecorations_js_1 = require("./highlightDecorations.js");
const textualHighlightProvider_js_1 = require("./textualHighlightProvider.js");
const ctxHasWordHighlights = new contextkey_js_1.RawContextKey('hasWordHighlights', false);
...
function getOccurrencesAcrossMultipleModels(registry, model, position, token, otherModels) {
    ...
    return (0, async_js_1.first)(orderedByScore.map(provider => () => {
        const filteredModels = otherModels.filter(otherModel => {
            return (0, model_js_1.shouldSynchronizeModel)(otherModel);
        }).filter(otherModel => {
            return (0, languageSelector_js_1.score)(provider.selector, otherModel.uri, otherModel.getLanguageId(), true, undefined, undefined) > 0;
        });
...
class OccurenceAtPositionRequest {
    ...
    get result() {
        if (!this._result) {
            this._result = (0, async_js_1.createCancelablePromise)(token => this._compute(this._model, this._selection, this._wordSeparators, token));
        }
        return this._result;
    }
    _getCurrentWordRange(model, selection) {
        ...
        if (word) {
            return new range_js_1.Range(selection.startLineNumber, word.startColumn, selection.startLineNumber, word.endColumn);
        }
        return null;
    }
...
(0, editorExtensions_js_1.registerModelAndPositionCommand)('_executeDocumentHighlights', async (accessor, model, position) => {
    const languageFeaturesService = accessor.get(languageFeatures_js_1.ILanguageFeaturesService);
    const map = await getOccurrencesAtPosition(languageFeaturesService.documentHighlightProvider, model, position, cancellation_js_1.CancellationToken.None);
    return map?.get(model.uri);
});
let WordHighlighter = class WordHighlighter {
    static { WordHighlighter_1 = this; }
    static { this.storedDecorationIDs = new map_js_1.ResourceMap(); }
    ...
    constructor(editor, providers, multiProviders, contextKeyService, textModelService, codeEditorService, configurationService, logService) {
        this.toUnhook = new lifecycle_js_1.DisposableStore();
        this.workerRequestTokenId = 0;
        this.workerRequestCompleted = false;
        this.workerRequestValue = new map_js_1.ResourceMap();
        this.lastCursorPositionChangeTime = 0;
        this.renderDecorationsTimer = -1;
        this.runDelayer = this.toUnhook.add(new async_js_1.Delayer(50));
        this.editor = editor;
        ...
        this.toUnhook.add(editor.onDidChangeModelContent((e) => {
            if (!(0, network_js_1.matchesScheme)(this.model.uri, 'output')) {
                this._stopAll();
            }
        }));
        ...
        this.toUnhook.add(editor.onDidBlurEditorWidget(() => {
            ...
            else if (activeEditor.getModel()?.uri.scheme === network_js_1.Schemas.vscodeNotebookCell && this.editor.getModel()?.uri.scheme !== network_js_1.Schemas.vscodeNotebookCell) { // switched tabs from non-nb to nb
                this._stopAll();
            }
        }));
    ...
    _getSortedHighlights() {
        return (this.decorations.getRanges()
            .sort(range_js_1.Range.compareRangesUsingStarts));
    }
    moveNext() {
        ...
        try {
            ...
            if (word) {
                ...
                (0, aria_js_1.alert)(`${lineContent}, ${newIndex + 1} of ${highlights.length} for '${word.word}'`);
            }
        }
        finally {
            this._ignorePositionChangeEvent = false;
    ...
    _removeAllDecorations(preservedModel) {
        ...
        for (const editor of currentEditors) {
            if (!editor.hasModel() || (0, resources_js_1.isEqual)(editor.getModel().uri, preservedModel)) {
                continue;
            }
    ...
    _stopSingular() {
        ...
        if (this.editor.hasTextFocus()) {
            if (this.editor.getModel()?.uri.scheme !== network_js_1.Schemas.vscodeNotebookCell && WordHighlighter_1.query?.modelInfo?.modelURI.scheme !== network_js_1.Schemas.vscodeNotebookCell) { // clear query if focused non-nb editor
    ...
    _onPositionChanged(e) {
        ...
        if (e.reason !== 3 /* CursorChangeReason.Explicit */ && this.editor.getModel()?.uri.scheme !== network_js_1.Schemas.vscodeNotebookCell) {
            this._stopAll();
            return;
        }
        this._run();
    ...
    getOtherModelsToHighlight(model) {
        ...
        // notebook case
        const isNotebookEditor = model.uri.scheme === network_js_1.Schemas.vscodeNotebookCell;
        if (isNotebookEditor) {
            const currentModels = [];
            const currentEditors = this.codeEditorService.listCodeEditors();
            for (const editor of currentEditors) {
                const tempModel = editor.getModel();
                if (tempModel && tempModel !== model && tempModel.uri.scheme === network_js_1.Schemas.vscodeNotebookCell) {
        ...
        const currentEditors = this.codeEditorService.listCodeEditors();
        for (const editor of currentEditors) {
            if (!(0, editorBrowser_js_1.isDiffEditor)(editor)) {
                continue;
            }
    ...
    async _run(multiFileConfigChange, noDelay) {
        ...
        if ((0, resources_js_1.isEqual)(this.editor.getModel().uri, WordHighlighter_1.query.modelInfo?.modelURI)) { // only trigger new worker requests from the primary model that initiated the query
            // case d)
        ...
        else if (this.model.uri.scheme === network_js_1.Schemas.vscodeNotebookCell) {
                ...
                if (newDocumentHighlights) {
                    for (const highlight of newDocumentHighlights) {
                        ...
                        newDecorations.push({
                            ...
                            options: (0, highlightDecorations_js_1.getHighlightDecorationOptions)(highlight.kind)
                        });
                    }
                }
...
WordHighlighter = WordHighlighter_1 = __decorate([
    __param(4, resolverService_js_1.ITextModelService),
    __param(5, codeEditorService_js_1.ICodeEditorService),
    __param(6, configuration_js_1.IConfigurationService),
    __param(7, log_js_1.ILogService)
], WordHighlighter);
let WordHighlighterContribution = class WordHighlighterContribution extends lifecycle_js_1.Disposable {
    ...
    constructor(editor, contextKeyService, languageFeaturesService, codeEditorService, textModelService, configurationService, logService) {
        ...
        const createWordHighlighterIfPossible = () => {
            if (editor.hasModel() && !editor.getModel().isTooLargeForTokenization() && editor.getModel().uri.scheme !== network_js_1.Schemas.accessibleView) {
        ...
        this._register(editor.onDidChangeModel((e) => {
            if (this._wordHighlighter) {
                if (!e.newModelUrl && e.oldModelUrl?.scheme !== network_js_1.Schemas.vscodeNotebookCell) { // happens when switching tabs to a notebook that has focus in the cell list, no new model URI (this also doesn't make it to the wordHighlighter, bc no editor.hasModel)
...
exports.WordHighlighterContribution = WordHighlighterContribution = WordHighlighterContribution_1 = __decorate([
    ...
    __param(2, languageFeatures_js_1.ILanguageFeaturesService),
    __param(3, codeEditorService_js_1.ICodeEditorService),
    __param(4, resolverService_js_1.ITextModelService),
    __param(5, configuration_js_1.IConfigurationService),
    __param(6, log_js_1.ILogService)
], WordHighlighterContribution);
class WordHighlightNavigationAction extends editorExtensions_js_1.EditorAction {
...
class TriggerWordHighlightAction extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
...
(0, editorExtensions_js_1.registerEditorContribution)(WordHighlighterContribution.ID, WordHighlighterContribution, 0 /* EditorContributionInstantiation.Eager */); // eager because it uses `saveViewState`/`restoreViewState`
(0, editorExtensions_js_1.registerEditorAction)(NextWordHighlightAction);
(0, editorExtensions_js_1.registerEditorAction)(PrevWordHighlightAction);
(0, editorExtensions_js_1.registerEditorAction)(TriggerWordHighlightAction);
(0, editorFeatures_js_1.registerEditorFeature)(textualHighlightProvider_js_1.TextualMultiDocumentHighlightFeature);
