...
const findController_js_1 = require("../../find/browser/findController.js");
...
const highlightDecorations_js_1 = require("../../wordHighlighter/browser/highlightDecorations.js");
...
function announceCursorChange(previousCursorState, cursorState) {
    ...
    if (cursorDiff.length >= 1) {
        ...
        (0, aria_js_1.status)(msg);
    }
}
class InsertCursorAbove extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
            ...
            menuOpts: {
                menuId: actions_js_1.MenuId.MenubarSelectionMenu,
                group: '3_multi',
    ...
    run(accessor, editor, args) {
        ...
        viewModel.setCursorStates(args.source, 3 /* CursorChangeReason.Explicit */, cursorMoveCommands_js_1.CursorMoveCommands.addCursorUp(viewModel, previousCursorState, useLogicalLine));
        viewModel.revealTopMostCursor(args.source);
        announceCursorChange(previousCursorState, viewModel.getCursorStates());
    }
}
exports.InsertCursorAbove = InsertCursorAbove;
class InsertCursorBelow extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
    ...
    run(accessor, editor, args) {
        ...
        viewModel.model.pushStackElement();
        const previousCursorState = viewModel.getCursorStates();
        viewModel.setCursorStates(args.source, 3 /* CursorChangeReason.Explicit */, cursorMoveCommands_js_1.CursorMoveCommands.addCursorDown(viewModel, previousCursorState, useLogicalLine));
        viewModel.revealBottomMostCursor(args.source);
        announceCursorChange(previousCursorState, viewModel.getCursorStates());
    }
}
exports.InsertCursorBelow = InsertCursorBelow;
class InsertCursorAtEndOfEachLineSelected extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
    ...
    getCursorsForSelection(selection, model, result) {
        ...
        for (let i = selection.startLineNumber; i < selection.endLineNumber; i++) {
            ...
            result.push(new selection_js_1.Selection(i, currentLineMaxColumn, i, currentLineMaxColumn));
        }
        if (selection.endColumn > 1) {
            result.push(new selection_js_1.Selection(selection.endLineNumber, selection.endColumn, selection.endLineNumber, selection.endColumn));
        }
    }
    run(accessor, editor) {
        if (!editor.hasModel()) {
...
class InsertCursorAtEndOfLineSelected extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
    ...
    run(accessor, editor) {
        ...
        for (let i = selections[0].startLineNumber; i <= lineCount; i++) {
            newSelections.push(new selection_js_1.Selection(i, selections[0].startColumn, i, selections[0].endColumn));
        }
        const viewModel = editor._getViewModel();
        const previousCursorState = viewModel.getCursorStates();
        if (newSelections.length > 0) {
...
class InsertCursorAtTopOfLineSelected extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
...
class MultiCursorSession {
    static create(editor, findController) {
        ...
        if (s.isEmpty()) {
            ...
            currentMatch = new selection_js_1.Selection(s.startLineNumber, word.startColumn, s.startLineNumber, word.endColumn);
        }
        else {
    ...
    _getNextMatch() {
        ...
        return new selection_js_1.Selection(nextMatch.range.startLineNumber, nextMatch.range.startColumn, nextMatch.range.endLineNumber, nextMatch.range.endColumn);
    }
    ...
    _getPreviousMatch() {
        ...
        return new selection_js_1.Selection(previousMatch.range.startLineNumber, previousMatch.range.startColumn, previousMatch.range.endLineNumber, previousMatch.range.endColumn);
    }
...
class MultiCursorSelectionController extends lifecycle_js_1.Disposable {
    ...
    constructor(editor) {
        super();
        this._sessionDispose = this._register(new lifecycle_js_1.DisposableStore());
        this._editor = editor;
    ...
    _expandEmptyToWord(model, selection) {
        ...
        return new selection_js_1.Selection(selection.startLineNumber, word.startColumn, selection.startLineNumber, word.endColumn);
    }
    ...
    selectAll(findController) {
        ...
        if (matches.length > 0) {
            ...
            this._setSelections(matches.map(m => new selection_js_1.Selection(m.range.startLineNumber, m.range.startColumn, m.range.endLineNumber, m.range.endColumn)));
        }
    }
...
class MultiCursorSelectionControllerAction extends editorExtensions_js_1.EditorAction {
    run(accessor, editor) {
        ...
        if (viewModel) {
            const previousCursorState = viewModel.getCursorStates();
            const findController = findController_js_1.CommonFindController.get(editor);
            if (findController) {
                this._run(multiCursorController, findController);
            }
            else {
                const newFindController = accessor.get(instantiation_js_1.IInstantiationService).createInstance(findController_js_1.CommonFindController, editor);
...
class MoveSelectionToNextFindMatchAction extends MultiCursorSelectionControllerAction {
    constructor() {
        super({
            ...
            kbOpts: {
                kbExpr: editorContextKeys_js_1.EditorContextKeys.focus,
                primary: (0, keyCodes_js_1.KeyChord)(2048 /* KeyMod.CtrlCmd */ | 41 /* KeyCode.KeyK */, 2048 /* KeyMod.CtrlCmd */ | 34 /* KeyCode.KeyD */),
                weight: 100 /* KeybindingWeight.EditorContrib */
            }
        });
    }
...
class CompatChangeAll extends MultiCursorSelectionControllerAction {
    constructor() {
        super({
            ...
            precondition: contextkey_js_1.ContextKeyExpr.and(editorContextKeys_js_1.EditorContextKeys.writable, editorContextKeys_js_1.EditorContextKeys.editorTextFocus),
            kbOpts: {
                kbExpr: editorContextKeys_js_1.EditorContextKeys.editorTextFocus,
...
class SelectionHighlighterState {
    ...
    findMatches() {
        if (this._cachedFindMatches === null) {
            ...
            this._cachedFindMatches.sort(range_js_1.Range.compareRangesUsingStarts);
        }
        return this._cachedFindMatches;
    }
}
let SelectionHighlighter = class SelectionHighlighter extends lifecycle_js_1.Disposable {
    ...
    constructor(editor, _languageFeaturesService) {
        ...
        this.updateSoon = this._register(new async_js_1.RunOnceScheduler(() => this._update(), 300));
        this.state = null;
    ...
    _setState(newState) {
        ...
        for (let i = 0, j = 0, len = allMatches.length, lenJ = selections.length; i < len;) {
            ...
            if (j >= lenJ) {
                ...
                const cmp = range_js_1.Range.compareRangesUsingStarts(match, selections[j]);
                if (cmp < 0) {
                    // match is before sel
                    if (selections[j].isEmpty() || !range_js_1.Range.areIntersecting(match, selections[j])) {
                        matches.push(match);
                    }
                    i++;
                }
        ...
        const decorations = matches.map(r => {
            return {
                range: r,
                options: (0, highlightDecorations_js_1.getSelectionHighlightDecorationOptions)(hasSemanticHighlights)
            };
        });
...
class FocusNextCursor extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
...
class FocusPreviousCursor extends editorExtensions_js_1.EditorAction {
    constructor() {
        super({
...
(0, editorExtensions_js_1.registerEditorContribution)(MultiCursorSelectionController.ID, MultiCursorSelectionController, 4 /* EditorContributionInstantiation.Lazy */);
(0, editorExtensions_js_1.registerEditorContribution)(SelectionHighlighter.ID, SelectionHighlighter, 1 /* EditorContributionInstantiation.AfterFirstRender */);
(0, editorExtensions_js_1.registerEditorAction)(InsertCursorAbove);
(0, editorExtensions_js_1.registerEditorAction)(InsertCursorBelow);
(0, editorExtensions_js_1.registerEditorAction)(InsertCursorAtEndOfEachLineSelected);
(0, editorExtensions_js_1.registerEditorAction)(AddSelectionToNextFindMatchAction);
(0, editorExtensions_js_1.registerEditorAction)(AddSelectionToPreviousFindMatchAction);
(0, editorExtensions_js_1.registerEditorAction)(MoveSelectionToNextFindMatchAction);
(0, editorExtensions_js_1.registerEditorAction)(MoveSelectionToPreviousFindMatchAction);
(0, editorExtensions_js_1.registerEditorAction)(SelectHighlightsAction);
(0, editorExtensions_js_1.registerEditorAction)(CompatChangeAll);
(0, editorExtensions_js_1.registerEditorAction)(InsertCursorAtEndOfLineSelected);
(0, editorExtensions_js_1.registerEditorAction)(InsertCursorAtTopOfLineSelected);
(0, editorExtensions_js_1.registerEditorAction)(FocusNextCursor);
(0, editorExtensions_js_1.registerEditorAction)(FocusPreviousCursor);
