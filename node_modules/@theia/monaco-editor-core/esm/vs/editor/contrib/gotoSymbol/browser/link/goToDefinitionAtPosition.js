...
const clickLinkGesture_js_1 = require("./clickLinkGesture.js");
...
const goToCommands_js_1 = require("../goToCommands.js");
...
let GotoDefinitionAtPositionEditorContribution = class GotoDefinitionAtPositionEditorContribution {
    ...
    constructor(editor, textModelResolverService, languageService, languageFeaturesService) {
        ...
        this.toUnhookForKeyboard = new lifecycle_js_1.DisposableStore();
        this.currentWordAtPosition = null;
        this.previousPromise = null;
        this.editor = editor;
        this.linkDecorations = this.editor.createDecorationsCollection();
        const linkGesture = new clickLinkGesture_js_1.ClickLinkGesture(editor);
    ...
    async startFindDefinition(position) {
        ...
        const state = new editorState_js_1.EditorState(this.editor, 4 /* CodeEditorStateFlag.Position */ | 1 /* CodeEditorStateFlag.Value */ | 2 /* CodeEditorStateFlag.Selection */ | 8 /* CodeEditorStateFlag.Scroll */);
        if (this.previousPromise) {
            this.previousPromise.cancel();
            this.previousPromise = null;
        }
        this.previousPromise = (0, async_js_1.createCancelablePromise)(token => this.findDefinition(position, token));
        let results;
        try {
        ...
        const linkRange = results[0].originSelectionRange
            ? range_js_1.Range.lift(results[0].originSelectionRange)
            : new range_js_1.Range(position.lineNumber, word.startColumn, position.lineNumber, word.endColumn);
        // Multiple results
        if (results.length > 1) {
            let combinedRange = linkRange;
            for (const { originSelectionRange } of results) {
                if (originSelectionRange) {
                    combinedRange = range_js_1.Range.plusRange(combinedRange, originSelectionRange);
                }
            }
            this.addDecoration(combinedRange, new htmlContent_js_1.MarkdownString().appendText(nls.localize('multipleResults', "Click to show {0} definitions.", results.length)));
        }
        else {
            // Single result
            ...
            this.textModelResolverService.createModelReference(result.uri).then(ref => {
                ...
                this.addDecoration(linkRange, previewValue ? new htmlContent_js_1.MarkdownString().appendCodeblock(languageId ? languageId : '', previewValue) : undefined);
                ref.dispose();
            });
        }
    }
    ...
    isEnabled(mouseEvent, withKey) {
        return this.editor.hasModel()
            ...
            && !(mouseEvent.target.detail.injectedText?.options instanceof textModel_js_1.ModelDecorationInjectedTextOptions)
    ...
    findDefinition(position, token) {
        ...
        return (0, goToSymbol_js_1.getDefinitionsAtPosition)(this.languageFeaturesService.definitionProvider, model, position, false, token);
    }
    ...
    isInPeekEditor(accessor) {
        ...
        return peekView_js_1.PeekContext.inPeekEditor.getValue(contextKeyService);
    }
    dispose() {
...
(0, editorExtensions_js_1.registerEditorContribution)(GotoDefinitionAtPositionEditorContribution.ID, GotoDefinitionAtPositionEditorContribution, 2 /* EditorContributionInstantiation.BeforeFirstInteraction */);
