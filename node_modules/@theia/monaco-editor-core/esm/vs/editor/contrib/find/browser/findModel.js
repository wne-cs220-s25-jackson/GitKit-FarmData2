...
const textModelSearch_js_1 = require("../../../common/model/textModelSearch.js");
const findDecorations_js_1 = require("./findDecorations.js");
const replaceAllCommand_js_1 = require("./replaceAllCommand.js");
const replacePattern_js_1 = require("./replacePattern.js");
const contextkey_js_1 = require("../../../../platform/contextkey/common/contextkey.js");
exports.CONTEXT_FIND_WIDGET_VISIBLE = new contextkey_js_1.RawContextKey('findWidgetVisible', false);
exports.CONTEXT_FIND_WIDGET_NOT_VISIBLE = exports.CONTEXT_FIND_WIDGET_VISIBLE.toNegated();
// Keep ContextKey use of 'Focussed' to not break when clauses
exports.CONTEXT_FIND_INPUT_FOCUSED = new contextkey_js_1.RawContextKey('findInputFocussed', false);
exports.CONTEXT_REPLACE_INPUT_FOCUSED = new contextkey_js_1.RawContextKey('replaceInputFocussed', false);
exports.ToggleCaseSensitiveKeybinding = {
...
class FindModelBoundToEditorModel {
    constructor(editor, state) {
        this._toDispose = new lifecycle_js_1.DisposableStore();
        this._editor = editor;
        this._state = state;
        this._isDisposed = false;
        this._startSearchingTimer = new async_js_1.TimeoutTimer();
        this._decorations = new findDecorations_js_1.FindDecorations(editor);
        this._toDispose.add(this._decorations);
        this._updateDecorationsScheduler = new async_js_1.RunOnceScheduler(() => {
    ...
    dispose() {
        ...
        (0, lifecycle_js_1.dispose)(this._startSearchingTimer);
    ...
    research(moveCursor, newFindScope) {
        ...
        if (findScopes !== null) {
            findScopes = findScopes.map(findScope => {
                if (findScope.startLineNumber !== findScope.endLineNumber) {
                    ...
                    return new range_js_1.Range(findScope.startLineNumber, 1, endLineNumber, this._editor.getModel().getLineMaxColumn(endLineNumber));
                }
                return findScope;
            });
        }
        ...
        if (currentMatchesPosition === 0 && findMatches.length > 0) {
            ...
            const matchAfterSelection = (0, arraysFind_js_1.findFirstIdxMonotonousOrArrLen)(findMatches.map(match => match.range), range => range_js_1.Range.compareRangesUsingStarts(range, editorSelection) >= 0);
    ...
    _moveToPrevMatch(before, isRecursed = false) {
        ...
        const model = this._editor.getModel();
        let position = new position_js_1.Position(lineNumber, column);
    ...
    _getReplacePattern() {
        if (this._state.isRegex) {
            return (0, replacePattern_js_1.parseReplaceString)(this._state.replaceString);
        }
        return replacePattern_js_1.ReplacePattern.fromStaticValue(this._state.replaceString);
    }
    replace() {
        if (!this._hasMatches()) {
            return;
        ...
        if (nextMatch) {
            if (selection.equalsRange(nextMatch.range)) {
                ...
                const command = new replaceCommand_js_1.ReplaceCommand(selection, replaceString);
                this._executeEditorCommand('replace', command);
                this._decorations.setStartPosition(new position_js_1.Position(selection.startLineNumber, selection.startColumn + replaceString.length));
                this.research(true);
            }
            else {
                this._decorations.setStartPosition(this._editor.getPosition());
    ...
    _largeReplaceAll() {
        const searchParams = new textModelSearch_js_1.SearchParams(this._state.searchString, this._state.isRegex, this._state.matchCase, this._state.wholeWord ? this._editor.getOption(136 /* EditorOption.wordSeparators */) : null);
        ...
        const command = new replaceCommand_js_1.ReplaceCommandThatPreservesSelection(fullModelRange, resultText, this._editor.getSelection());
    ...
    _regularReplaceAll(findScopes) {
        ...
        const command = new replaceAllCommand_js_1.ReplaceAllCommand(this._editor.getSelection(), matches.map(m => m.range), replaceStrings);
        this._executeEditorCommand('replaceAll', command);
    }
    selectAllMatches() {
        if (!this._hasMatches()) {
        ...
        let selections = matches.map(m => new selection_js_1.Selection(m.range.startLineNumber, m.range.startColumn, m.range.endLineNumber, m.range.endColumn));
