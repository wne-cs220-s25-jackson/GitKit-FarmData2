...
const blockCommentCommand_js_1 = require("./blockCommentCommand.js");
class LineCommentCommand {
    ...
    static _analyzeLines(type, insertSpace, model, lines, startLineNumber, ignoreEmptyLines, ignoreFirstLine, languageConfigurationService) {
        ...
        for (let i = 0, lineCount = lines.length; i < lineCount; i++) {
            ...
            if (shouldRemoveComments && !blockCommentCommand_js_1.BlockCommentCommand._haystackHasNeedleAtOffset(lineContent, lineData.commentStr, lineContentStartOffset)) {
                if (type === 0 /* Type.Toggle */) {
    ...
    _executeLineComments(model, builder, data, s) {
        ...
        const cursorPosition = new position_js_1.Position(s.positionLineNumber, s.positionColumn);
        for (let i = 0, len = ops.length; i < len; i++) {
            builder.addEditOperation(ops[i].range, ops[i].text);
            if (range_js_1.Range.isEmpty(ops[i].range) && range_js_1.Range.getStartPosition(ops[i].range).equals(cursorPosition)) {
    ...
    _attemptRemoveBlockComment(model, s, startToken, endToken) {
        ...
        if (startTokenIndex !== -1 && endTokenIndex !== -1) {
            return blockCommentCommand_js_1.BlockCommentCommand._createRemoveBlockCommentOperations(new range_js_1.Range(startLineNumber, startTokenIndex + startToken.length + 1, endLineNumber, endTokenIndex + 1), startToken, endToken);
        }
        return null;
    }
    /**
    ...
    _executeBlockComment(model, builder, s) {
        ...
        if (!ops) {
            if (s.isEmpty()) {
                ...
                ops = blockCommentCommand_js_1.BlockCommentCommand._createAddBlockCommentOperations(new range_js_1.Range(s.startLineNumber, firstNonWhitespaceIndex + 1, s.startLineNumber, lineContent.length + 1), startToken, endToken, this._insertSpace);
            }
            else {
                ops = blockCommentCommand_js_1.BlockCommentCommand._createAddBlockCommentOperations(new range_js_1.Range(s.startLineNumber, model.getLineFirstNonWhitespaceColumn(s.startLineNumber), s.endLineNumber, model.getLineMaxColumn(s.endLineNumber)), startToken, endToken, this._insertSpace);
            }
            if (ops.length === 1) {
    ...
    getEditOperations(model, builder) {
        ...
        if (s.startLineNumber === s.endLineNumber && this._ignoreFirstLine) {
            builder.addEditOperation(new range_js_1.Range(s.startLineNumber, model.getLineMaxColumn(s.startLineNumber), s.startLineNumber + 1, 1), s.startLineNumber === model.getLineCount() ? '' : '\n');
            this._selectionId = builder.trackSelection(s);
            return;
        }
    ...
    computeCursorState(model, helper) {
        ...
        return new selection_js_1.Selection(result.selectionStartLineNumber, result.selectionStartColumn + this._deltaColumn, result.positionLineNumber, result.positionColumn + this._deltaColumn);
    }
    /**
    ...
    static _createRemoveLineCommentsOperations(lines, startLineNumber) {
        ...
        for (let i = 0, len = lines.length; i < len; i++) {
            ...
            res.push(editOperation_js_1.EditOperation.delete(new range_js_1.Range(startLineNumber + i, lineData.commentStrOffset + 1, startLineNumber + i, lineData.commentStrOffset + lineData.commentStrLength + 1)));
        }
        return res;
    }
    /**
    ...
    _createAddLineCommentsOperations(lines, startLineNumber) {
        ...
        for (let i = 0, len = lines.length; i < len; i++) {
            ...
            res.push(editOperation_js_1.EditOperation.insert(new position_js_1.Position(startLineNumber + i, lineData.commentStrOffset + 1), lineData.commentStr + afterCommentStr));
        }
        return res;
    }
