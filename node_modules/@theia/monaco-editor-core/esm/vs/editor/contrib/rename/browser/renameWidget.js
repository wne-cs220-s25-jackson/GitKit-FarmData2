...
require("./renameWidget.css");
const domFontInfo = require("../../../browser/config/domFontInfo.js");
...
/** for debugging */
const _sticky = false;
exports.CONTEXT_RENAME_INPUT_VISIBLE = new contextkey_js_1.RawContextKey('renameInputVisible', false, nls.localize('renameInputVisible', "Whether the rename input widget is visible"));
exports.CONTEXT_RENAME_INPUT_FOCUSED = new contextkey_js_1.RawContextKey('renameInputFocused', false, nls.localize('renameInputFocused', "Whether the rename input widget is focused"));
let RenameWidget = class RenameWidget {
    constructor(_editor, _acceptKeybindings, _themeService, _keybindingService, contextKeyService, _logService) {
        ...
        this._beforeFirstInputFieldEditSW = new stopwatch_js_1.StopWatch();
    ...
    _updateStyles(theme) {
        ...
        const widgetShadowColor = theme.getColor(colorRegistry_js_1.widgetShadow);
        const widgetBorderColor = theme.getColor(colorRegistry_js_1.widgetBorder);
        this._domNode.style.backgroundColor = String(theme.getColor(colorRegistry_js_1.editorWidgetBackground) ?? '');
        this._domNode.style.boxShadow = widgetShadowColor ? ` 0 0 8px 2px ${widgetShadowColor}` : '';
        this._domNode.style.border = widgetBorderColor ? `1px solid ${widgetBorderColor}` : '';
        this._domNode.style.color = String(theme.getColor(colorRegistry_js_1.inputForeground) ?? '');
        const border = theme.getColor(colorRegistry_js_1.inputBorder);
        this._inputWithButton.domNode.style.backgroundColor = String(theme.getColor(colorRegistry_js_1.inputBackground) ?? '');
        this._inputWithButton.input.style.backgroundColor = String(theme.getColor(colorRegistry_js_1.inputBackground) ?? '');
    ...
    _updateFont() {
        ...
        (0, types_js_1.assertType)(this._label !== undefined, 'RenameWidget#_updateFont: _label must not be undefined given _domNode is defined');
    ...
    afterRender(position) {
        ...
        (0, types_js_1.assertType)(this._renameCandidateListView);
        (0, types_js_1.assertType)(this._nPxAvailableAbove !== undefined);
        (0, types_js_1.assertType)(this._nPxAvailableBelow !== undefined);
    ...
    getInput(where, currentName, supportPreview, requestRenameCandidates, cts) {
        ...
        this._renameCts = cts;
        const disposeOnDone = new lifecycle_js_1.DisposableStore();
        this._nRenameSuggestionsInvocations = 0;
        this._hadAutomaticRenameSuggestionsInvocation = false;
        ...
        disposeOnDone.add((0, lifecycle_js_1.toDisposable)(() => {
        ...
        disposeOnDone.add((0, lifecycle_js_1.toDisposable)(() => this._candidates.clear()));
        const inputResult = new async_js_1.DeferredPromise();
        ...
        this._currentAcceptInput = (wantsPreview) => {
            ...
            (0, types_js_1.assertType)(this._renameCandidateListView !== undefined);
    ...
    _requestRenameCandidates(currentName, isManuallyTriggered) {
        ...
        (0, types_js_1.assertType)(this._renameCts);
        if (this._inputWithButton.buttonState !== 'stop') {
            this._renameCandidateProvidersCts = new cancellation_js_1.CancellationTokenSource();
            const triggerKind = isManuallyTriggered ? languages_js_1.NewSymbolNameTriggerKind.Invoke : languages_js_1.NewSymbolNameTriggerKind.Automatic;
    ...
    _getSelection(where, currentName) {
        (0, types_js_1.assertType)(this._editor.hasModel());
        const selection = this._editor.getSelection();
        let start = 0;
        let end = currentName.length;
        if (!range_js_1.Range.isEmpty(selection) && !range_js_1.Range.spansMultipleLines(selection) && range_js_1.Range.containsRange(where, selection)) {
    ...
    async _updateRenameCandidates(candidates, currentName, token) {
        ...
        trace('start');
        const namesListResults = await (0, async_js_1.raceCancellation)(Promise.allSettled(candidates), token);
        this._inputWithButton.setSparkleButton();
        if (namesListResults === undefined) {
            trace('returning early - received updateRenameCandidates results - undefined');
            return;
        }
        const newNames = namesListResults.flatMap(namesListResult => namesListResult.status === 'fulfilled' && (0, types_js_1.isDefined)(namesListResult.value)
...
class RenameCandidateListView {
    ...
    constructor(parent, opts) {
        ...
        this._listWidget.style((0, defaultStyles_js_1.getListStyles)({
            listInactiveFocusForeground: colorRegistry_js_1.quickInputListFocusForeground,
            listInactiveFocusBackground: colorRegistry_js_1.quickInputListFocusBackground,
        }));
    }
    dispose() {
    ...
    static _createListWidget(container, candidateViewHeight, fontInfo) {
        ...
        return new listWidget_js_1.List('NewSymbolNameCandidates', container, virtualDelegate, [renderer], {
...
class InputWithButton {
    constructor() {
        ...
        this._onDidInputChange = new event_js_1.Emitter();
    ...
    get domNode() {
        if (!this._domNode) {
            ...
            this._disposables.add((0, hoverDelegate2_js_1.getBaseLayerHoverDelegate)().setupDelayedHover(this._buttonNode, () => ({
    ...
    get input() {
        (0, types_js_1.assertType)(this._inputNode);
        return this._inputNode;
    }
    get button() {
        (0, types_js_1.assertType)(this._buttonNode);
    ...
    setSparkleButton() {
        ...
        this._sparkleIcon ??= (0, iconLabels_js_1.renderIcon)(codicons_js_1.Codicon.sparkle);
    ...
    setStopButton() {
        ...
        this._stopIcon ??= (0, iconLabels_js_1.renderIcon)(codicons_js_1.Codicon.stopCircle);
        dom.clearNode(this.button);
...
class RenameCandidateView {
    ...
    constructor(parent, fontInfo) {
        ...
        this._icon = (0, iconLabels_js_1.renderIcon)(codicons_js_1.Codicon.sparkle);
    ...
    _updateIcon(value) {
        const isAIGenerated = !!value.tags?.includes(languages_js_1.NewSymbolNameTag.AIGenerated);
