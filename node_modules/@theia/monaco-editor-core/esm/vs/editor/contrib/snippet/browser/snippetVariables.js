...
const labels_js_1 = require("../../../../base/common/labels.js");
const path = require("../../../../base/common/path.js");
    ...
    resolve(variable) {
        ...
        if (name === 'SELECTION' || name === 'TM_SELECTED_TEXT') {
            ...
            if (value && isMultiline && variable.snippet) {
                ...
                const lineLeadingWhitespace = (0, strings_js_1.getLeadingWhitespace)(line, 0, this._selection.startColumn - 1);
                let varLeadingWhitespace = lineLeadingWhitespace;
                variable.snippet.walk(marker => {
                    if (marker === variable) {
                        return false;
                    }
                    if (marker instanceof snippetParser_js_1.Text) {
                        varLeadingWhitespace = (0, strings_js_1.getLeadingWhitespace)((0, strings_js_1.splitLines)(marker.value).pop());
                    }
                    return true;
                });
                const whitespaceCommonLength = (0, strings_js_1.commonPrefixLength)(varLeadingWhitespace, lineLeadingWhitespace);
...
class ModelBasedVariableResolver {
    ...
    resolve(variable) {
        ...
        if (name === 'TM_FILENAME') {
            ...
            return this._labelService.getUriLabel((0, resources_js_1.dirname)(this._model.uri));
        }
...
class ClipboardBasedVariableResolver {
    ...
    resolve(variable) {
        ...
        // and when enabled
        if (this._spread) {
            const lines = clipboardText.split(/\r\n|\n|\r/).filter(s => !(0, strings_js_1.isFalsyOrWhitespace)(s));
...
exports.CommentBasedVariableResolver = CommentBasedVariableResolver = __decorate([
    __param(2, languageConfigurationRegistry_js_1.ILanguageConfigurationService)
...
class WorkspaceBasedVariableResolver {
    ...
    resolve(variable) {
        ...
        const workspaceIdentifier = (0, workspace_js_1.toWorkspaceIdentifier)(this._workspaceService.getWorkspace());
        if ((0, workspace_js_1.isEmptyWorkspaceIdentifier)(workspaceIdentifier)) {
            return undefined;
        }
    ...
    _resolveWorkspaceName(workspaceIdentifier) {
        if ((0, workspace_js_1.isSingleFolderWorkspaceIdentifier)(workspaceIdentifier)) {
            return path.basename(workspaceIdentifier.uri.path);
        }
        let filename = path.basename(workspaceIdentifier.configPath.path);
        if (filename.endsWith(workspace_js_1.WORKSPACE_EXTENSION)) {
            filename = filename.substr(0, filename.length - workspace_js_1.WORKSPACE_EXTENSION.length - 1);
        }
        return filename;
    }
    _resoveWorkspacePath(workspaceIdentifier) {
        if ((0, workspace_js_1.isSingleFolderWorkspaceIdentifier)(workspaceIdentifier)) {
            return (0, labels_js_1.normalizeDriveLetter)(workspaceIdentifier.uri.fsPath);
        }
        ...
        return (folderpath ? (0, labels_js_1.normalizeDriveLetter)(folderpath) : '/');
    }
}
...
class RandomBasedVariableResolver {
    resolve(variable) {
        ...
        if (name === 'RANDOM') {
            ...
            return (0, uuid_js_1.generateUuid)();
        }
        return undefined;
    }
}
